...
this.params.scale+"'")}return this};a.Image.prototype.mkPrimaryImage=function(){var d,b,c,e,f,g,h,l,j,m,n,r,k,q,p,s,t,u,w,z=this,y=this,x=this,A=!1,v=255;if(!this.primary||!this.psColors)return this;if(a.globalOpts.rgb.mode&&(this===a.globalOpts.rgb.rim||this===a.globalOpts.rgb.gim||this===a.globalOpts.rgb.bim))A=!0;A&&(z=a.globalOpts.rgb.rim,y=a.globalOpts.rgb.gim,x=a.globalOpts.rgb.bim);d=this.primary;b=d.sect;if(!d.img||d.img.width!==b.width||d.img.height!==b.height)d.img=this.display.context.createImageData(b.width,
b.height);d=d.img;this.params.maskInvert?(p=this.params.alpha2,s=this.params.alpha1):(p=this.params.alpha1,s=this.params.alpha2);e=b.y1-1;for(g=0;e>=b.y0;e--,g++){k=e*this.raw.width;l=g*b.zoom;c=b.x0;for(f=0;c<b.x1;c++,f++){A?(z&&(t=z.colorData[k+c]),y&&(u=y.colorData[k+c]),x&&(w=x.colorData[k+c])):n=this.colorData[k+c];this.maskData&&(v=0<this.maskData[k+c]?p:s);h=f*b.zoom;for(j=0;j<b.zoom;j++){m=Math.floor(l+j);q=m*b.width;for(m=0;m<b.zoom;m++)r=Math.floor(h+m),r=4*(q+r),A?(d.data[r]=z?z.psColors[t][0]:
...
Math.abs(c.dval/k["cdelt"+b]);return c.dval};d=d.trim();if(!d.match(/(\(|\{|#|;|\n)/))return d;try{k=JSON.parse(a.wcsinfo(this.wcs))}catch(A){k={cdelt1:1,cdelt2:1,crot:0,imflip:0}}l=this.getWCSSys();j="physical";this.setWCSSys(j);m="image"!==j&&"physical"!==j;f=d.split(/\n|;/);for(d=0;d<f.length;d++)if("#"!==f[d].trim().substr(0,1)){n=!1;g=f[d];h=void 0;c={opts:{},args:[],isregion:0};c.cmd=0<=g.indexOf("(")?g.split("(")[0].trim().toLowerCase():0<=g.indexOf("{")?g.split("{")[0].trim().toLowerCase():
0<=g.indexOf("#")?g.split("#")[0].trim().toLowerCase():g.trim().toLowerCase();c.cmd&&(c.isregion=0<=c.cmd.search(p));c.comment=g.split("#")[1];c.comment&&(c.comment=c.comment.trim().toLowerCase());if((h=u.exec(g))&&h[1])try{c.opts=JSON.parse(h[1].trim())}catch(B){c.opts={}}if((h=t.exec(g))&&h[1])c.args=h[1].split(w);c.isregion&&0===c.cmd.indexOf("-")&&(c.cmd=c.cmd.slice(1),c.comment=c.comment?c.comment+",exclude":"exclude");h=c;q=h.args.length;if(h.isregion){g=$.extend(!0,{},h.opts);g.shape=h.cmd;

...
b),a.Fabric.addPolygonAnchors(k.dlayer,b)),a.resetPolygonCenter(b)}b.rescaleBorder();b.setCoords();a.Fabric._updateShape.call(w,c,b,y,"update");a.Fabric._handleChildText.call(w,c,b,d)}),(void 0===d.redraw||d.redraw)&&l.renderAll(),this};a.Fabric.refreshShapes=function(c){var b,d,e,f,g,h,k,l,n,p=!1,q=this;if(n=this.getShapeLayer(c))return"main"===this.display.layers[c].dtype&&(p=!0),p?(e=this.binning.bin,f=this.rgb.sect.zoom,g=this.binning.obin/this.rgb.sect.ozoom*f/e,h=this.binning.obin/this.rgb.sect.ozoom*
...
break;case "pts":a.pub.pts?a.pub.pts.forEach(function(a){c&&(c+=", ");c+=sprintf("%s, %s",a.x.toFixed(2),a.y.toFixed(2))}):a.pub.imstr&&(c=a.pub.imstr.replace(/^.*\(/,"").replace(/\)$/,""));break;case "fontFamily":a.getFontFamily&&(c=a.getFontFamily());break;case "fontSize":a.getFontSize&&(c=a.getFontSize());break;case "fontStyle":a.getFontStyle&&(c=a.getFontStyle());break;case "fontWeight":a.getFontWeight&&(c=a.getFontWeight());break;case "strokeWidth":c=a.params.sw1;break;case "strokeDashes":a.strokeDashArray?
...
function(c,b,d){var e,f,g,h,k=d.length,l={},n=function(a){return"number"===typeof a&&0!==a%1?a.toFixed(2):String(a)},p=function(a,b,c){return"remove"===b?"selected"===c:"childtext"===b?0<a.params.children.length?!1:""!==c:"strokeDashes"===b?JSON.stringify(a.strokeDashArray)!==JSON.stringify(c):"tags"!==b&&""===c?!1:"misc"===b&&""!==c?!0:"angle"===b?a.angle!==-parseFloat(c):void 0!==a.pub[b]&&n(a.pub[b])!==c||void 0!==a.params[b]&&n(a.params[b])!==c||void 0!==a[b]&&n(a[b])!==c?!0:!1},q=function(b){return"true"===
b?!0:"false"===b?!1:a.isNumber(b)?parseFloat(b):b};for(e=0;e<k;e++)switch(f=d[e].name,g=d[e].value,f){case "text":"text"===c.type&&p(c,f,g)&&(l[f]=q(g));break;case "strokeDashes":g=g.trim().split(/\s+/);p(c,f,g)&&(l.strokeDashArray=0===g.length?[]:g.map(function(a){return parseInt(a,10)}));break;case "childtext":"text"!==c.type&&p(c,f,g)&&(l.text=q(g));break;case "x":p(c,f,g)&&(l[f]=q(g),void 0===l.y&&(l.y=c.pub.y));break;case "y":p(c,f,g)&&(l[f]=q(g),void 0===l.x&&(l.x=c.pub.x));break;case "misc":if(g.trim())try{h=
...
k&&(l+=k);1<b&&this.displayMessage("regions",l);return l};a.Regions.copyRegions=function(c,b){var d,e,f=[];if("object"===typeof c)f.push(c);else if("all"===c)for(d=0;d<a.images.length;d++)this!==a.images[d]&&f.push(a.images[d]);else(d=a.lookupImage(c))&&f.push(d);if(f.length){b||(e=this.listRegions("selected",{mode:1}));e||(e=this.listRegions(b,{mode:1}));for(d=0;d<f.length;d++)f[d].addShapes("regions",e);return this}};a.Regions.parseRegions=function(c){var b=[],d,e,f,g,h,k,l,n,p,q,r,m=/(annulus)|(box)|(circle)|(ellipse)|(line)|(polygon)|(point)|(text)/,
...
0<=a.indexOf("{")?e.cmd=a.split("{")[0].trim().toLowerCase():0<=a.indexOf("#")?e.cmd=a.split("#")[0].trim().toLowerCase():e.cmd=a.trim().toLowerCase();e.cmd&&(e.isregion=0<=e.cmd.search(m));b=a.trim().split(z);if((c=v.exec(b[0]))&&c[0])try{e.opts=JSON.parse(c[0].trim())}catch(F){e.opts={}}e.comment=b[1];e.comment&&(d=x(e.comment.trim().toLowerCase()),void 0!==d._comment&&(e.comment=d._comment,delete d._comment));d&&(e.opts=$.extend({},d,e.opts));(c=u.exec(a))&&c[1]&&(e.args=c[1].split(w));e.isregion&&
0===e.cmd.indexOf("-")&&(e.cmd=e.cmd.slice(1),e.comment=e.comment?e.comment+",exclude":"exclude");return e},B=function(b){b=a.saostrtod(b);var c=String.fromCharCode(a.saodtype());switch(c){case '"':b/=3600;break;case "'":b/=60;break;case "r":b*=180/Math.PI}return{dval:b,dtype:c}},C=function(b,c){var d,e;e=B(b);d=B(c);if(":"===e.dtype||":"===d.dtype)p=!0;n||p?(":"===e.dtype&&"galactic"!==l&&"ecliptic"!==l&&(e.dval*=15),d=a.wcs2pix(this.raw.wcs,e.dval,d.dval).split(/ +/),e=parseFloat(d[0]),d=parseFloat(d[1])):
"physical"===l?(d=this.logicalToImagePos({x:e.dval,y:d.dval}),e=d.x,d=d.y):(e=e.dval,d=d.dval);return[e,d]},A=function(a,b){var c=B(a),d=this.raw.wcsinfo||{cdelt1:1,cdelt2:1};(n||p)&&c.dtype&&"."!==c.dtype&&(c.dval=Math.abs(c.dval/d["cdelt"+b]));return c.dval},E=function(a){a=B(a);var b=this.raw.wcsinfo||{crot:0};(n||p)&&b.crot&&(a.dval+=b.crot);return a.dval};c=c.trim();if(!c.match(/(\(|\{|#|;|\n)/))return c;k=this.getWCSSys();l="physical";this.setWCSSys(l);n="image"!==l&&"physical"!==l;f=c.split(/\n|;/);
for(c=0;c<f.length;c++)if("#"!==f[c].trim().substr(0,1))if(p=!1,h=D(f[c]),r=h.args.length,h.isregion){g=$.extend(!0,{},h.opts);g.shape=h.cmd;2<=r&&(q=C.call(this,h.args[0],h.args[1]),g.x=q[0],g.y=q[1]);switch(h.cmd){case "annulus":g.radii=[];for(d=2;d<r;d++)g.radii.push(A.call(this,h.args[d],1));break;case "box":3<=r&&(g.width=A.call(this,h.args[2],1));4<=r&&(g.height=A.call(this,h.args[3],2));5<=r&&(g.angle=E.call(this,h.args[4]));break;case "circle":3<=r&&(g.radius=A.call(this,h.args[2],1));break;
case "ellipse":3<=r&&(g.r1=A.call(this,h.args[2],1));4<=r&&(g.r2=A.call(this,h.args[3],2));5<=r&&(g.angle=E.call(this,h.args[4]));break;case "line":case "polygon":g.pts=[];for(e=d=0;d<r;d+=2,e++)q=C.call(this,h.args[d],h.args[d+1]),g.pts[e]={x:q[0],y:q[1]};delete g.x;delete g.y;break;case "text":3<=r&&(g.text=h.args[2].replace(/^['"]/,"").replace(/["']$/,""))}h.comment&&(g.tags=h.comment);b.push(g)}else h.cmd.match(t)?(this.setWCSSys(h.cmd),l=this.getWCSSys(),n="image"!==l&&"physical"!==l):"remove"!==
...
b,a.fits.options);void 0===d&&(d=a.Load);e.onload=function(a){var e=new Image,f,k,l;e.src=a.target.result;e.onload=function(){var a,g,h,r=0;a=document.createElement("canvas");g=a.getContext("2d");var m=e.height,t=e.width;a.width=t;a.height=m;g.drawImage(e,0,0);f=g.getImageData(0,0,t,m).data;k=new Float32Array(m*t);for(g=0;g<m;g++)for(a=0;a<t;a++)h=.299*f[r]+.587*f[r+1]+.114*f[r+2],k[(m-g)*t+a]=h,r+=4;l={head:{},filename:c.name,filedata:k,naxis:2,axis:[0,t,m],bitpix:-32,data:k};l.dmin=Number.MAX_VALUE;
l.dmax=Number.MIN_VALUE;for(r=0;r<m*t;r++)l.dmin=Math.min(l.dmin,l.data[r]),l.dmax=Math.max(l.dmax,l.data[r]);d(l,b)}};e.readAsDataURL(c)};a.lookupColormap=function(c){var b;c||(c=a.imageOpts.colormap);if(c)for(b=0;b<a.colormaps.length;b++)if(a.colormaps[b].name===c)return a.colormaps[b];a.error("unknown colormap '"+c+"'")};a.lookupCommand=function(c){var b,d;if(c)for(d=c.toLowerCase(),b=0;b<a.commands.length;b++)if(c=a.commands[b],c.name===d||c.alias===d||c.alias2===d)return c;return null};a.error=

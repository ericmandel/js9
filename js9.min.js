...
v)k(h,w);else if(h.params.tags)for(p=0;p<h.params.tags.length;p++)q=h.params.tags[p],v.match(/^\/.*\/$/)&&q.match(new RegExp(v.slice(1,-1)))?k(h,w):v===q&&k(h,w)}}return this};a.Fabric.selectShapes=function(c,b,d){var e=[];var f=this.getShapeLayer(c);var h=f.canvas;if(f){d=d||{};a.isNull(d.saveselection)&&(d.saveselection=!0);if("reset"===b)return delete f.selection,!1!==d.activateselection&&(h.discardActiveObject(),h.renderAll()),this;this._selectShapes(c,b,d,function(a){0>$.inArray(a,e)&&e.push(a)});
!1!==d.activateselection&&(h.discardActiveObject(),c=new fabric.ActiveSelection(e,{canvas:h}),h.setActiveObject(c),h.renderAll());return this}};a.Fabric.updateShapes=function(a,b,d,e){var c=this;this._selectShapes(a,b,null,function(b,f){c._updateShape(a,b,f,d,e)});return this};a.Fabric._updateMultiDialogs=function(a){var b=this;$("form[class*='regionsConfigForm']").each(function(c,e){c=$(e).data("multi");var d=$(e).data("winid");e=$(e).data("im");c&&d&&e===b&&e.initRegionsForm(null,{winid:d,multi:c,
setmode:a})})};a.Fabric._updateShape=function(c,b,d,e,f){var h,g={},k=this.layers[c],l=/^(child||export|unexport|move|mouseout)$/;var m=function(a){return a.toFixed(2)};var n=function(b,c,d,e,f,g,h){var k=a.pix2wcs(b,d.x,d.y).trim().split(/\s+/);h.rastr=k[0];h.decstr=k[1];h.wcssys=k[2];var l=a.strtoscaled(k[0]);a.isHMS(k[2],l.dtype)&&(l.dval*=15);var m=a.strtoscaled(k[1]);h.ra=l.dval;h.dec=m.dval;if(!1!==g.updateWCS&&(g.updateWCS||c.opts.updateWCS)){h.wcsstr=a.reg2wcs(b,e).replace(/;$/,"");"line"===
d.shape&&f&&(h.wcsstr=h.wcsstr.replace(/} *$/,f+"}"));k=h.wcsstr.replace(/.*\(/,"").replace(/\).*/,"").split(",");for(b=0;b<k.length;b++)k[b]=k[b].trim();h.wcsposstr=[k[0],k[1]];switch(d.shape){case "annulus":h.wcssizestr=[k[k.length-1]];break;case "box":case "cross":h.wcssizestr=[k[2],k[3]];break;case "circle":h.wcssizestr=[k[2]];break;case "ellipse":h.wcssizestr=[k[2],k[3]];break;case "line":case "polygon":for(h.wcspts=[],b=0;b<k.length;b+=2)l=a.strtoscaled(k[b]),a.isHMS(h.wcssys,l.dtype)&&(l.dval*=
15),m=a.strtoscaled(k[b+1]),h.wcspts.push({ra:l.dval,dec:m.dval})}}};d=d||{};f=f||{};e=e||"update";var p="main"!==this.display.layers[c].dtype||b.params.preservedcoords?1:this.rgb.sect.zoom;g.mode=e;g.id=b.params.id;g.shape=b.params.shape;g.layer=c;g.color=b.color||b.stroke;g.tags=b.params.tags;g.sticky=b.params.sticky;g.preservedcoords=b.params.preservedcoords;g.parent=b.params.parent?b.params.parent.obj.params.id:null;g.child=b.params.children&&b.params.children.length?b.params.children[0].id:null;
var q=b.getCenterPoint();var r={x:0,y:0};d.group&&(r=d.group.getCenterPoint(),q={x:q.x+r.x,y:q.y+r.y},d.group.angle&&(q=a.rotatePoint(q,d.group.angle,r)));g.dx=q.x;g.dy=q.y;q=this.displayToImagePos(q);g.x=q.x;g.y=q.y;g.lcs=this.imageToLogicalPos(q);"export"!==e&&b.params.wcsconfig&&("image"===b.params.wcsconfig.wcssys?h="image":"physical"===b.params.wcsconfig.wcssys&&(h="physical"));if("image"===h||"image"===this.params.wcssys&&"physical"!==h){g.imsys="image";var v=g.x;var w=g.y;var u=1}else g.imsys=
g.lcs.sys,v=g.lcs.x,w=g.lcs.y,u=Math.sqrt(Math.pow(this.lcs.physical.reverse[0][0],2)+Math.pow(this.lcs.physical.reverse[0][1],2));g.angle=-b.angle;d.group&&(g.angle-=d.group.angle);h=g.angle;if(!g.parent)switch(g.shape){case "box":case "cross":case "ellipse":case "text":if(a.notNull(this.params.transformScale)&&(g.angle/=this.params.transformScale),a.notNull(this.params.transformAngle)&&(g.angle-=this.params.transformAngle),this.raw.wcsinfo)this.raw.wcsinfo.crot&&(g.angle-=this.raw.wcsinfo.crot);
else if(a.notNull(this.raw.header.LTM1_1)||a.notNull(this.raw.header.LTM1_2)){try{var t=Math.atan((this.raw.header.LTM1_2||0)/(this.raw.header.LTM1_1||0))}catch(A){t=0}t&&(t=180*-t/Math.PI,g.angle-=t)}}for(;0>g.angle;)g.angle+=360;for(;360<=g.angle;)g.angle-=360;q=b.scaleX/p;t=b.scaleY/p;d.group&&(q*=d.group.scaleX,t*=d.group.scaleY);switch(g.shape){case "annulus":g.shape="annulus";g.radii=[];"image"!==g.imsys&&(g.lcs.radii=[]);g.imstr="annulus("+m(v)+","+m(w)+",";var x="annulus "+g.x+" "+g.y+" ";
r=b.getObjects();var y=r.length;for(d=0;d<y;d++)h=r[d].radius*q,t=h*u,g.imstr+=m(t),x+=g.x+" "+g.y+" "+(g.x+h)+" "+g.y+" ",g.imstr=d===y-1?g.imstr+")":g.imstr+",",g.radii.push(h),"image"!==g.imsys&&g.lcs.radii.push(t);break;case "box":case "cross":g.width=b.width*q;g.height=b.height*t;t=g.width*u;x=g.height*u;"image"!==g.imsys&&(g.lcs.width=t,g.lcs.height=x);g.imstr=g.shape+"("+m(v)+","+m(w)+","+m(t)+","+m(x)+","+g.angle.toFixed(4)+")";x=g.shape+" "+g.x+" "+g.y+" "+g.x+" "+g.y+" "+(g.x+g.width)+" "+
g.y+" "+g.x+" "+g.y+" "+g.x+" "+(g.y+g.height)+" "+g.angle*Math.PI/180;break;case "circle":g.radius=b.radius*q;t=g.radius*u;"image"!==g.imsys&&(g.lcs.radius=t);g.imstr="circle("+m(v)+","+m(w)+","+m(t)+")";x="circle "+g.x+" "+g.y+" "+g.x+" "+g.y+" "+(g.x+g.radius)+" "+g.y;break;case "ellipse":g.r1=b.width*q/2;g.r2=b.height*t/2;t=g.r1*u;x=g.r2*u;"image"!==g.imsys&&(g.lcs.r1=t,g.lcs.r2=x);g.imstr="ellipse("+m(v)+","+m(w)+","+m(t)+","+m(x)+","+g.angle.toFixed(4)+")";x="ellipse "+g.x+" "+g.y+" "+g.x+" "+
g.y+" "+(g.x+g.r1)+" "+g.y+" "+g.x+" "+g.y+" "+g.x+" "+(g.y+g.r2)+" "+g.angle*Math.PI/180;break;case "point":g.width=b.width*q;g.height=b.height*t;t=g.width*u;x=g.height*u;"image"!==g.imsys&&(g.lcs.width=t,g.lcs.height=x);g.imstr="point("+m(v)+","+m(w)+")";x="point "+g.x+" "+g.y;break;case "line":case "polygon":g.imstr=g.shape+"(";x=g.shape+" ";g.pts=[];"image"!==g.imsys&&(g.lcs.pts=[]);for(d=0;d<b.points.length;d++)0<d&&(g.imstr+=",",x+=" "),t=this.displayToImagePos({x:r.x+b.left+b.points[d].x*b.scaleX,
y:r.y+b.top+b.points[d].y*b.scaleY}),t=a.rotatePoint(t,h,{x:g.x,y:g.y}),"image"===g.imsys?g.imstr+=m(t.x)+","+m(t.y):(u=this.imageToLogicalPos(t),q=u.x,u=u.y,g.imstr+=m(q)+","+m(u),g.lcs.pts.push({x:q,y:u})),x+=t.x+" "+t.y,g.pts.push(t),"line"===g.shape&&(0===d?y=0:(q=g.pts[d-1],y+=Math.sqrt((t.x-q.x)*(t.x-q.x)+(t.y-q.y)*(t.y-q.y))));if("line"===g.shape&&a.notNull(y)){g.imstr+=') {"size":'+m(y)+',"units":"pixels"';if(2===g.pts.length){for(t=180*Math.atan2(g.pts[1].y-g.pts[0].y,g.pts[1].x-g.pts[0].x)/
Math.PI;0>t;)t+=360;var B=',"posang":'+t.toFixed(4)+',"posunits":"degrees"';g.imstr+=B}g.imstr+="}"}else g.imstr+=")";g.angle=0;break;case "text":g.imstr="text("+m(v)+","+m(w)+',"'+b.text+'",'+g.angle.toFixed(4)+")",x="text "+g.x+" "+g.y+' "'+b.text+'" '+g.angle*Math.PI/180,g.text=b.text}this.raw.wcs&&0<this.raw.wcs&&(n(this.raw.wcs,k,g,x,B,f,g),b.params.wcsconfig&&b.params.wcsconfig.wcssys&&(m=a.globalOpts.xeqPlugins,a.globalOpts.xeqPlugins=!1,r=this.getWCSSys(),a.notWCS(b.params.wcsconfig.wcssys)||
(this.setWCSSys(b.params.wcsconfig.wcssys,!1),n(this.raw.wcs,k,g,x,B,f,b.params.wcsconfig)),g.wcsconfig=$.extend(!0,{},b.params.wcsconfig),this.setWCSSys(r,!1),a.globalOpts.xeqPlugins=m));g.data=b.params.data;b.set("pub",g);b.params.winid&&($(b.params.winid).is(":visible")?this.initRegionsForm(b):b.params.winid=null);if(f.nocb)return g;if(!b.params.parent&&!e.match(l)){if(this.params.xeqonchange&&k.show&&k.opts.onchange)try{this.params.xeqonchange=!1,a.xeqByName(k.opts.onchange,window,this,g)}catch(A){a.log("error in onchange: %s [%s]\n%s",
this.id,A.message,a.strace(A))}finally{this.params.xeqonchange=!0}this.xeqPlugins("shape","on"+c+"change",g)}if("regions"===c&&a.globalOpts.regToClipboard){switch(e){case "update":d=g.parent||g.id;break;default:d=null}if(a.notNull(d)){try{var z=this.listRegions(d,{mode:1,includedcoords:!0})}catch(A){z=null}z&&(a.clipboard=z)}}"regions"===c&&"wcs"===e&&this._updateMultiDialogs(!0);return g};a.Fabric.removeShapes=function(c,b,d){var e=this,f,h={mode:1,includedcoords:!0},g=[];if(f=this.getShapeLayer(c)){var k=
f.canvas;d=d||{};"regions"===c&&a.globalOpts.unremoveReg&&(this.regstack.push(this.listRegions(b,h,c)),this.regstack.length>a.globalOpts.unremoveReg&&(this.regstack=this.regstack.slice(0,a.globalOpts.unremoveReg)));this._selectShapes(c,b,d,function(a,b){if(!(!1===a.params.removable&&!d.overrideRemovable||a.params.sticky&&!1===d.sticky)){e._updateShape(c,a,b,"remove");a.params.winid&&(a.params.winid.close(),a.params.winid=null);if(a.params.parent){var f=a.params.parent.obj;for(b=f.params.children.length-
...
f);e[b].addShapes(c,d)}return this}}};a.Fabric._addPolygonPoint=function(c,b,d){var e=Number.MAX_VALUE;if(b&&b.points){d=a.eventToDisplayPos(d);var f=b.getCenterPoint().x;var h=b.getCenterPoint().y;f=(d.x-f)/b.get("scaleX");var g=(d.y-h)/b.get("scaleY");for(h=-b.get("angle")*Math.PI/180;h>2*Math.PI;)h-=2*Math.PI;d=Math.cos(h)*f-Math.sin(h)*g;g=Math.sin(h)*f+Math.cos(h)*g;f=b.points;for(h=0;h<f.length;h++){var k=f[h];var l=f[(h+1)%f.length];var m=k.x<l.x?k.x:l.x;var n=k.y<l.y?k.y:l.y;var p=k.x>l.x?
k.x:l.x;var q=k.y>l.y?k.y:l.y;var r=k.x-l.x;k=k.y-l.y;var v=Math.sqrt(r*r+k*k);0!==v&&(r/=v,k/=v);v=d-l.x;var w=g-l.y;v=v*r+w*k;r=l.x+r*v;l=l.y+k*v;r<m?r=m:r>p&&(r=p);l<n?l=n:l>q&&(l=q);m=(r-d)*(r-d)+(l-g)*(l-g);if(m<e){e=m;var u=r;var t=l;var x=h===f.length?0:h}}c=this.getShapeLayer(c);a.Fabric.removePolygonAnchors(c.dlayer,b);f.splice(x+1,0,{x:u,y:t});if(1===fabric.major_version)c.canvas.setActiveObject(b);else{switch(b.type){case "polyline":case "polygon":a.Fabric.addPolygonAnchors(c.dlayer,b),
c.dlayer.canvas.renderAll()}b.polyparams?c.dlayer.params.sel=b.polyparams.polygon:b.params&&(c.dlayer.params.sel=b)}}};a.Fabric._removePolygonPoint=function(c,b){if(b&&b.polyparams){var d=b.polyparams.polygon;var e=d.points;if(!("polygon"===d.type&&3>=e.length||"polyline"===d.type&&2>=e.length)){var f=b.polyparams.point;delete b.polyparams.point;c=this.getShapeLayer(c);a.Fabric.removePolygonAnchors(c.dlayer,d);e.splice(f,1);a.resetPolygonCenter(d);c.canvas.setActiveObject(d)}}};a.Fabric.addPolygonAnchors=

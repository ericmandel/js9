...
"galactic"!==x&&"ecliptic"!==x&&(y*=15);if(e=n(y,B)){var z=u();z={id:c.toString(),shape:p,x:e.x,y:e.y,width:z.width,height:z.height,radius:z.radius,r1:z.r1,r2:z.r2,angle:0,data:{ra:y,dec:B}};if(!1!==b.save&&!1!==a.globalOpts.catalogs.save)for(e=0;e<=t.length;e++)t[e]&&(z.data[t[e]]=q[c][e]);b.color&&(z.color=b.color);m[l++]=z}}this.setWCSSys(r,!1);return m}};a.Image.prototype.loadCatalog=function(c){for(var b=[],d=0;d<arguments.length;++d)b[d-0]=arguments[d];var e=$jscomp.makeIterator(b);d=e.next().value;
...
(this.worker=new Worker(c),d())};a.WebWorker.prototype.msgHandler=function(c){var b=a.helper;var d=c.data;switch(d.cmd){case "progress":a.progress(d.result.value,d.result.max);break;case "initsocketio":this.sockinit=!0;for(c=0;c<this.handlers.length;c++)if(b=this.handlers[c],b.id===d.id){b.func(d.result);this.handlers.splice(c,1);break}break;case "uploadFITS":for(c=0;c<this.handlers.length;c++)if(b=this.handlers[c],b.id===d.id){b.func(d.result);this.handlers.splice(c,1);break}break;case "connect_error":case "connect_timeout":1<
a.DEBUG&&a.log("JS9 worker socketio: "+d.cmd);break;case "disconnect":a.waiting(!1);d.result?a.worker.postMessage("initsocketio",[b.url,b.sockver,b.pageid],function(){a.error(d.result)}):1<a.DEBUG&&a.log("JS9 worker socketio: disconnect");break;case "error":a.error(d.result||"in web worker")}};a.WebWorker.prototype.postMessage=function(c,b,d,e){var f=c+a.uniqueID(),h={id:f,cmd:c,args:b};d&&(b=b||[],this.handlers.push({id:f,cmd:c,args:b,func:d}));e?this.worker.postMessage(h,e):this.worker.postMessage(h)};
a.WebWorker.prototype.socketio=function(c){var b=a.helper;a.worker.sockinit?c&&c():a.worker.postMessage("initsocketio",[b.url,b.sockver,b.pageid],function(b){"OK"===b?c&&c():a.error("can't init  socket.io for JS9 worker: "+b)})};a.WebWorker.prototype.terminate=function(){this.worker.terminate()};fabric.major_version=parseFloat(fabric.version.split(".")[0]);fabric.minor_version=parseFloat(fabric.version.split(".")[1]);fabric.patch_version=parseFloat(fabric.version.split(".")[2]);a.Fabric={};a.Fabric.elements=
...
b.target,"moving")});k.canvas.on("object:rotating",function(b){b.target&&a.Fabric.updateChildren(k,b.target,"rotating")});1===fabric.major_version?(k.canvas.on("object:selected",function(a){a.target&&g(k,a.target)}),k.canvas.on("before:selection:cleared",function(a){a.target&&h(k,a.target)})):(k.canvas.on("selection:created",function(a){a.target&&g(k,a.target)}),k.canvas.on("selection:updated",function(b){b=k.canvas.getActiveObject();if("activeSelection"===b.type){var d,e=k.canvas.getActiveObjects();
for(d=0;d<e.length;d++){var h=e[d];if(h.params){if(h.params.parent&&h.params.parent.obj){var l=h.params.parent.obj;0>$.inArray(l,e)&&b.addWithUpdate(l)}if(h.params.children)for(l=0;l<h.params.children.length;l++){var m=h.params.children[l].obj;0>$.inArray(m,e)&&b.addWithUpdate(m)}switch(h.type){case "polyline":case "polygon":a.Fabric.removePolygonAnchors(k,h)}h&&k.display.image&&k.display.image.updateShapes(c,h,"select")}}k.canvas.renderAll();f(1)}else g(k,b)}),k.canvas.on("before:selection:cleared",
...
h.canvas;var l=this.display.layers[c];if(a.isNull(b))return h.show;if(!0===b||"true"===b){if(!d.local&&(h.show=!0,this!==this.display.image))return;h.show&&(k.selection=h.opts.canvas.selection);h.json&&h.show&&k.loadFromJSON(h.json,function(){var b,d;a.Fabric.updateChildren(h.dlayer,null,"objects");e.resize&&(k.getObjects().forEach(function(a){a.left+=e.resize.left;a.top+=e.resize.top;a.setCoords()}),k.calcOffset());h.zindex=Math.abs(h.zindex);l.divjq.css("z-index",h.zindex);for(b in e.layers)if(e.layers.hasOwnProperty(b)&&
c!==b&&e.layers[b].show){var f=e.display.layers[b];f.divjq.css("z-index")<h.zindex&&(d=f.canvas.getActiveObject())&&(a.Fabric.removePolygonAnchors(f,d),f.canvas.discardActiveObject())}});for(f in this.layers)this.layers.hasOwnProperty(f)&&this.layers[f].json&&g++;g||(this.resize=null);this.xeqPlugins("shape","onshapelayershow",c)}else{if(!d.local&&this!==this.display.image){h.show=!1;return}if(h.show){b=k.getObjects();for(f=b.length;f--;)g=b[f],g.params&&(g.params.winid&&(g.params.winid.close(),g.params.winid=
null),g.params.anchors&&a.Fabric.removePolygonAnchors(h.dlayer,g));b=k.toJSON(h.dlayer.el);h.json=JSON.stringify(b);k.selection=!1;l&&(h.zindex=-Math.abs(h.zindex),l.divjq.css("z-index",h.zindex));k.clear()}d.local||(h.show=!1);this.xeqPlugins("shape","onshapelayerhide",c)}return this}};a.Fabric.displayShapeLayers=function(){var a;if(this!==this.display.image){if(this.display.image&&this.display.image.layers)for(a in this.display.image.layers)this.display.image.layers.hasOwnProperty(a)&&this.display.image.showShapeLayer(a,

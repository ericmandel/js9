...
(a.ra*=15)),"string"===typeof a.dec&&(a.dec=b.saostrtod(a.dec)),f=b.wcs2pix(this.raw.wcs,a.ra,a.dec).trim().split(/ +/),a=parseFloat(f[0]),c=parseFloat(f[1]),d&&(this.setWCSSys(d),b.globalOpts.xeqPlugins=h)));this.mkSection(a,c);if(this.display.blendMode)for(d=0;d<b.images.length;d++)h=b.images[d],h!==this&&h.display===this.display&&h.blend.active&&(f=h.raw.width/2,g=h.raw.height/2,0!==arguments.length&&(f-=k-a,g-=l-c),b.Image.prototype.mkSection.call(h,f,g));this.displayImage("rgb");for(e in this.layers)this.layers.hasOwnProperty(e)&&
...
parseFloat(a);this.params.scalemax=parseFloat(c);this.params.scaleclipping="user";break;default:f(a),"zscale"!==a&&"zmax"!==a&&(this.params.scalemin=parseFloat(c),this.params.scalemax=parseFloat(d),this.params.scaleclipping="user")}this.displayImage("colors")}b.globalOpts.extendedPlugins&&this.xeqPlugins("image","onsetscale");return this}};b.Image.prototype.getParam=function(a){return a?"all"===a?this.params:this.params[a]:null};b.Image.prototype.setParam=function(a,b){var c,e;if(!a)return null;if("all"===
a&&"object"===typeof b){$.extend(!0,this.params,b);if(b.colormap||b.contrast||b.bias)e=this.getColormap(),b.colormap=b.colormap||e.colormap,b.contrast=b.contrast||e.contrast,b.bias=b.bias||e.bias,this.setColormap(b.colormap,b.contrast,b.bias);if(b.scale||b.scalemin||b.scalemax)e=this.getScale(),b.scale=b.scale||e.scale,b.scalemin=b.scalemin||e.scalemin,b.scalemax=b.scalemax||e.scalemax,this.setScale(b.scale,b.scalemin,b.scalemax);b.invert&&(this.params.invert=b.invert,this.displayImage("colors"));
b.zoom&&this.setZoom(b.zoom);b.wcssys&&this.setWCSSys(b.wcssys);b.wcsunits&&this.setWCSUnits(b.wcsunits);return this.params}if("disable"===a){$.isArray(b)||(b=[b]);for(c=0;c<b.length;c++)e=$.inArray(b[c],this.params.disable),0>e&&this.params.disable.push(b[c]);return this.params.disable}if("enable"===a){$.isArray(b)||(b=[b]);for(c=0;c<b.length;c++)e=$.inArray(b[c],this.params.disable),0<=e&&this.params.disable.splice(e,1);return this.params.disable}c=this.params[a];this.params[a]=b;switch(a){case "colormap":this.setColormap(b);
break;case "invert":this.displayImage("colors");break;case "contrast":e=this.getColormap();this.setColormap(e.colormap,b,e.bias);break;case "bias":e=this.getColormap();this.setColormap(e.colormap,e.contrast,b);break;case "scale":this.setScale(b);break;case "scalemin":e=this.getScale();this.setScale("user",b,e.scalemax);break;case "scalemax":e=this.getScale();this.setScale("user",e.scalemin,b);break;case "scaleclipping":e=this.getScale();this.setScale(b,e.scalemin,e.scalemax);break;case "wcssys":this.setWCSSys(b);
break;case "wcsunits":this.setWCSUnits(b);break;case "zoom":this.setZoom(b)}return c};b.Image.prototype.dataminmax=function(a,b){var c,e,f=isNaN(this.params.scalemin)||void 0===this.params.scalemin,g=isNaN(this.params.scalemax)||void 0===this.params.scalemax;if("dataminmax"===this.params.scaleclipping){if(this.raw.dmin===this.params.scalemin||void 0===this.raw.dmin)f=!0;if(this.raw.dmax===this.params.scalemax||void 0===this.raw.dmax)g=!0}if(void 0!==a&&void 0!==b)this.raw.dmin=a,this.raw.dmax=b;else if(this.raw.dmin=
...
f[a].trim();else if($.isArray(c.tags))for(a=0;a<c.tags.length;a++)m.tags[a]=c.tags[a].trim();void 0!==c.angle&&(n.angle=-c.angle);void 0!==c.x&&void 0!==c.y&&(f=this.imageToDisplayPos(c),n.left=f.x,n.top=f.y);void 0!==c.px&&void 0!==c.py&&(f=this.logicalToDisplayPos({x:c.px,y:c.py}),n.left=f.x,n.top=f.y);"string"===typeof c.wcs&&(f=c.wcs.trim().split(/ +/),c.ra=f[0],c.dec=f[1],3<=f.length&&(c.wcssys=f[2]));void 0!==c.ra&&void 0!==c.dec&&0<this.raw.wcs&&(c.wcssys?(g=this.getWCSSys(),e=b.globalOpts.xeqPlugins,
b.globalOpts.xeqPlugins=!1,this.setWCSSys(c.wcssys)):c._wcssys&&(g=this.getWCSSys(),e=b.globalOpts.xeqPlugins,b.globalOpts.xeqPlugins=!1,this.setWCSSys(c._wcssys),delete c._wcssys),"string"===typeof c.ra&&(c.ra=b.saostrtod(c.ra),":"===String.fromCharCode(b.saodtype())&&"galactic"!==this.params.wcssys&&"ecliptic"!==this.params.wcssys&&(c.ra*=15)),"string"===typeof c.dec&&(c.dec=b.saostrtod(c.dec)),f=b.wcs2pix(this.raw.wcs,c.ra,c.dec).trim().split(/ +/),g&&(this.setWCSSys(g),b.globalOpts.xeqPlugins=
e),f=this.imageToDisplayPos({x:parseFloat(f[0]),y:parseFloat(f[1])}),n.left=f.x,n.top=f.y);void 0!==c.left&&(n.left=c.left);void 0!==c.top&&(n.top=c.top);void 0===n.left&&(n.left=d&&void 0!==d.left?d.left:this.display.canvasjq.attr("width")/2-1);void 0===n.top&&(n.top=d&&void 0!==d.top?d.top:this.display.canvasjq.attr("height")/2-1+1);c.dx&&(n.left+=c.dx);c.dy&&(n.top-=c.dy);switch(c.shape){case "annulus":m.radii=[];if(void 0!==c.radii)if("string"===typeof c.radii)for(m.radii=c.radii.replace(/ /g,
"").split(","),e=a=0;a<m.radii.length;a++)""!==m.radii[a]&&(m.radii[e++]=this.wcs2imlen(m.radii[a]));else m.radii=c.radii;else for(c.ireg&&b.SCALEIREG&&(c.iradius&&(c.iradius/=h),c.oradius&&(c.oradius/=h)),h=(c.oradius-c.iradius)/c.nannuli,g=c.nannuli+1,a=0;a<g;a++)e=c.iradius+h*a,m.radii.push(e);break;case "box":c.ireg&&b.SCALEIREG&&(c.width&&(c.width/=h),c.height&&(c.height/=h));break;case "circle":c.ireg&&b.SCALEIREG&&c.radius&&(c.radius/=h);break;case "ellipse":c.ireg&&b.SCALEIREG&&(c.r1&&(c.r1/=
h),c.r2&&(c.r2/=h));break;case "point":switch(c.ptshape){case "box":c.width=2*c.ptsize;c.height=2*c.ptsize;break;case "circle":c.radius=c.ptsize;break;case "ellipse":c.rx=c.ptsize,c.ry=c.ptsize/2}c.lockRotation=!0;c.lockScalingX=!0;c.lockScalingY=!0;c.lockUniScaling=!0;c.hasControls=!1;c.hasRotatingPoint=!1;c.hasBorders=!0;break;case "line":case "polygon":if(c.pts&&c.pts.length){if("string"===typeof c.pts){f=c.pts.replace(/ /g,"").split(",");g=f.length;if("string"===typeof f[0])for(a=0;a<g;a++)f[a]=
parseFloat(f[a]);c.pts=[];for(e=a=0;a<g;a+=2,e++)c.pts[e]={x:f[a],y:f[a+1]}}g=c.pts.length;for(a=0;a<g;a++)c.pts[a]=this.imageToDisplayPos(c.pts[a]);c.left&&c.top?e={x:c.left,y:c.top}:(e=b.centerPolygon(c.pts),n.left=e.x,n.top=e.y);c.points=[];for(a=0;a<g;a++)f={x:(c.pts[a].x-e.x)/h,y:(c.pts[a].y-e.y)/h},c.points.push(f)}else d&&d.points&&d.points.length||("polygon"===c.shape&&c.polypoints?c.points=c.polypoints:"line"===c.shape&&c.linepoints&&(c.points=c.linepoints));if(c.ireg&&b.SCALEIREG)for(g=
...
c.params.winid&&c.params.winid.close();if(c.params.parent)for(h=c.params.parent.obj,g=h.params.children.length-1;0<=g;g--)if(c===h.params.children[g].obj){h.params.children.splice(g,1);break}for(g=0;g<c.params.children.length;g++)h=c.params.children[g].obj,f.remove(h);f.remove(c)}}),f.getActiveGroup()&&f.discardActiveGroup(),f.renderAll(),this};b.Fabric.getShapes=function(a,c,d){var e=[],f={};if("string"===typeof d)try{d=JSON.parse(d)}catch(g){b.error("can't parse getShapes opts: "+d,g)}d=d||{};if("regions"===
...
switch(c.params.shape){case "text":g.opts.stroke&&(g.opts.fill=g.opts.stroke),g.opts.strokeWidth=0}c.set(g.opts);c.params=$.extend(!1,{},c.params,g.params);d.strokeWidth&&(c.params.sw1=d.strokeWidth);switch(c.params.shape){case "annulus":if(d.radii&&d.radii.length){q=c.get("stroke");c.forEachObject(function(a){p.push(a)});m=p.length;for(e=0;e<m;e++)c.remove(p[e]),l.remove(p[e]);m=c.params.radii.length;for(e=t=0;e<m;e++)f=new fabric.Circle({radius:c.params.radii[e],stroke:q}),t=Math.max(t,c.params.radii[e]),

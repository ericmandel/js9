...
f)}},a.TIMEOUT)):a.error("WindowToPDF is only available for the JS9 desktop app")});a.mkPublic("SaveDir",function(c){for(var b=[],d=0;d<arguments.length;++d)b[d-0]=arguments[d];b=a.parsePublicArgs(b);var e={cmd:"savedir"};window.electron?(void 0!==b.argv[0]?(e.dirname=b.argv[0],b.argv[1]&&(e.opts=b.argv[1])):a.error("SaveDir requires a directory name"),window.setTimeout(function(){try{window.electron.sendMsg(e)}catch(f){a.error("could not set save directory",f)}},a.TIMEOUT)):a.error("SaveDir is only available for the JS9 desktop app")});

...
n(a.params[b])!==c||void 0!==a[b]&&n(a[b])!==c?!0:!1},q=function(b){return"true"===b?!0:"false"===b?!1:a.isNumber(b)?parseFloat(b):b};for(e=0;e<k;e++)switch(f=d[e].name,g=d[e].value,f){case "text":"text"===c.type&&p(c,f,g)&&(l[f]=q(g));break;case "strokeDashes":g=g.trim().split(/\s+/);p(c,f,g)&&(l.strokeDashArray=0===g.length?[]:g.map(function(a){return parseInt(a,10)}));break;case "childtext":"text"!==c.type&&p(c,f,g)&&(l.text=q(g));break;case "x":p(c,f,g)&&(l[f]=q(g),void 0===l.y&&(l.y=c.pub.y));
break;case "y":p(c,f,g)&&(l[f]=q(g),void 0===l.x&&(l.x=c.pub.x));break;case "misc":if(g.trim())try{h=JSON.parse(g),$.extend(l,h)}catch(r){a.error("invalid json: "+g)}break;default:p(c,f,g)&&(l[f]=q(g))}this.changeShapes(c.pub.layer,c,l);a.Regions.initConfigForm.call(this,c,b)};a.Regions.listRegions=function(c,b,d){var e,f,g,h,k,l="",n="none",p=!1,q=[],r=[];g={};var m=function(a,b){var c,d,e,f={},g=a.params,h=g.children,k=g.exports;for(c=0;c<k.length;c++)if(e=k[c],("text"!==e||"text"===a.type)&&"textOpts"!==
...
a.Regions.parseRegions=function(c){var b=[],d,e,f,g,h,k,l,n,p,q,r,m=/(annulus)|(box)|(circle)|(ellipse)|(line)|(polygon)|(point)|(text)/,t=/(fk4)|(fk5)|(icrs)|(galactic)|(ecliptic)|(image)|(physical)/,u=/\(\s*([^)]+?)\s*\)/,v=/(\{.*\})/,w=/\s*,\s*/,z=/#(?![a-zA-Z0-9]{6}['"])/,y=function(a){return"0"===a?!1:!0},x=function(a){for(var b,c,d,e={},f=/([a-zA-Z][a-zA-Z0-9_]*)\s*=\s*(\d+(\s*\d+)*|[^ '"{]+|['"{][^'"}]*['"}])/g,g={color:function(a){return{color:a}},dash:function(a){if(a)return{strokeDashArray:[3,
...
else e[c]=b;if(a=a.replace(f,""))e._comment=a.trim();return e},D=function(a){var b,c,d,e={opts:{},args:[],isregion:0};0<=a.indexOf("(")?e.cmd=a.split("(")[0].trim().toLowerCase():0<=a.indexOf("{")?e.cmd=a.split("{")[0].trim().toLowerCase():0<=a.indexOf("#")?e.cmd=a.split("#")[0].trim().toLowerCase():e.cmd=a.trim().toLowerCase();e.cmd&&(e.isregion=0<=e.cmd.search(m));b=a.trim().split(z);if((c=v.exec(b[0]))&&c[0])try{e.opts=JSON.parse(c[0].trim())}catch(F){e.opts={}}e.comment=b[1];e.comment&&(d=x(e.comment.trim().toLowerCase()),
void 0!==d._comment&&(e.comment=d._comment,delete d._comment));d&&(e.opts=$.extend({},d,e.opts));(c=u.exec(a))&&c[1]&&(e.args=c[1].split(w));e.isregion&&0===e.cmd.indexOf("-")&&(e.cmd=e.cmd.slice(1),e.comment=e.comment?e.comment+",exclude":"exclude");return e},B=function(b){b=a.saostrtod(b);var c=String.fromCharCode(a.saodtype());switch(c){case '"':b/=3600;break;case "'":b/=60;break;case "r":b*=180/Math.PI}return{dval:b,dtype:c}},C=function(b,c){var d,e;e=B(b);d=B(c);if(":"===e.dtype||":"===d.dtype)p=
...
return a.dval};c=c.trim();if(!c.match(/(\(|\{|#|;|\n)/))return c;k=this.getWCSSys();l="physical";this.setWCSSys(l);n="image"!==l&&"physical"!==l;f=c.split(/\n|;/);for(c=0;c<f.length;c++)if("#"!==f[c].trim().substr(0,1))if(p=!1,h=D(f[c]),r=h.args.length,h.isregion){g=$.extend(!0,{},h.opts);g.shape=h.cmd;2<=r&&(q=C.call(this,h.args[0],h.args[1]),g.x=q[0],g.y=q[1]);switch(h.cmd){case "annulus":g.radii=[];for(d=2;d<r;d++)g.radii.push(A.call(this,h.args[d],1));break;case "box":3<=r&&(g.width=A.call(this,
h.args[2],1));4<=r&&(g.height=A.call(this,h.args[3],2));5<=r&&(g.angle=E.call(this,h.args[4]));break;case "circle":3<=r&&(g.radius=A.call(this,h.args[2],1));break;case "ellipse":3<=r&&(g.r1=A.call(this,h.args[2],1));4<=r&&(g.r2=A.call(this,h.args[3],2));5<=r&&(g.angle=E.call(this,h.args[4]));break;case "line":case "polygon":g.pts=[];for(e=d=0;d<r;d+=2,e++)q=C.call(this,h.args[d],h.args[d+1]),g.pts[e]={x:q[0],y:q[1]};delete g.x;delete g.y;break;case "text":3<=r&&(g.text=h.args[2].replace(/^['"]/,"").replace(/["']$/,
...
a.fits.options.waiting=a.waiting;return b};a.handleFITSFile=function(c,b,d){a.fits.handleFITSFile?a.fits.handleFITSFile(c,b,d):a.error("no FITS module available to process FITS file")};a.handleImageFile=function(c,b,d){var e=new FileReader;b=$.extend(!0,{},b,a.fits.options);void 0===d&&(d=a.Load);e.onload=function(a){var e=new Image,f,k,l;e.src=a.target.result;e.onload=function(){var a,g,h,r=0;a=document.createElement("canvas");g=a.getContext("2d");var m=e.height,t=e.width;a.width=t;a.height=m;g.drawImage(e,
0,0);f=g.getImageData(0,0,t,m).data;k=new Float32Array(m*t);for(g=0;g<m;g++)for(a=0;a<t;a++)h=.299*f[r]+.587*f[r+1]+.114*f[r+2],k[(m-g)*t+a]=h,r+=4;l={head:{},filename:c.name,filedata:k,naxis:2,axis:[0,t,m],bitpix:-32,data:k};l.dmin=Number.MAX_VALUE;l.dmax=Number.MIN_VALUE;for(r=0;r<m*t;r++)l.dmin=Math.min(l.dmin,l.data[r]),l.dmax=Math.max(l.dmax,l.data[r]);d(l,b)}};e.readAsDataURL(c)};a.lookupColormap=function(c){var b;c||(c=a.imageOpts.colormap);if(c)for(b=0;b<a.colormaps.length;b++)if(a.colormaps[b].name===

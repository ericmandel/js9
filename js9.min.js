...
this.params.zscalesamples,this.params.zscaleline);Module._free(c);c=b.trim().split(" ");this.params.z1=parseFloat(c[0]);this.params.z2=parseFloat(c[1]);d&&(this.params.scalemin=this.params.z1,this.params.scalemax=this.params.z2);return this};a.Colormap=function(d,b,c,e){this.name=d;switch(arguments.length-1){case 1:this.type="lut";this.colors=b;break;case 3:this.type="sao";this.vertices=[];this.vertices[0]=b;this.vertices[1]=c;this.vertices[2]=e;break;default:a.error("colormap requires name and 1 or 3 array arg")}a.colormaps.push(this);
1<a.DEBUG&&a.log("JS9 colormap:  %s",this.name)};a.Colormap.prototype.mkColorCell=function(d){var b,c=a.COLORSIZE,e=[0,0,0];switch(this.type){case "sao":var f,g;d/=c;for(f=0;3>f;f++){g=this.vertices[f];b=g.length;for(c=0;c<b&&!(g[c][0]>d);c++);c=0===c?g[0][1]:c===b?g[b-1][1]:(b=(g[c][1]-g[c-1][1])/(g[c][0]-g[c-1][0]))?b*(d-g[c-1][0])+g[c-1][1]:g[c][1];e[f]=255*c}break;case "lut":f=this.colors.length;d=Math.floor(d*f/c+0.5);0<=d&&d<f&&(e[0]=255*this.colors[d][0],e[1]=255*this.colors[d][1],e[2]=255*
this.colors[d][2]);break;default:a.error("unknown colormap type")}return e};a.Display=function(d){this.divjq=d instanceof jQuery?d:"object"===typeof d?$(d):$("#"+d);this.divjq.attr("id")||this.divjq.attr("id",a.DEFID);this.id=this.divjq.attr("id");this.divjq.addClass("JS9");this.width=this.divjq.attr("data-width");this.width||(this.width=a.WIDTH);this.divjq.css("width",this.width);this.width=parseInt(this.divjq.css("width"),10);this.height=this.divjq.attr("data-height");this.height||(this.height=
a.HEIGHT);this.divjq.css("height",this.height);this.height=parseInt(this.divjq.css("height"),10);this.canvas=document.createElement("canvas");this.canvasjq=$(this.canvas).addClass("JS9Image").css("z-index",a.ZINDEX).attr("width",this.width).attr("height",this.height);this.displayConjq=$("<div>").addClass("JS9Container").css("z-index",a.ZINDEX).attr("tabindex","0").append(this.canvasjq).appendTo(this.divjq);this.context=this.canvas.getContext("2d");a.ANTIALIAS||(this.context.imageSmoothingEnabled=
...
this.divjq.on("mouseup touchend",this,function(b){return a.mouseUpCB(b)});this.divjq.on("mouseout",this,function(b){return a.mouseOutCB(b)});this.divjq.on("keypress",this,function(b){return a.keyPressCB(b)});this.divjq.on("keydown",this,function(b){return a.keyDownCB(b)});this.divjq.on("dragenter",this,function(b){return a.dragenter(this.id,b.originalEvent)});this.divjq.on("dragover",this,function(b){return a.dragover(this.id,b.originalEvent)});this.divjq.on("dragexit",this,function(b){return a.dragexit(this.id,
b.originalEvent)});this.divjq.on("drop",this,function(b){return a.dragdrop(this.id,b.originalEvent,a.NewFITSImage)});this.divjq.append('<div style="visibility:hidden; position:relative; top:-50;left:-50"> <input type="file" id="openLocalFile-'+this.id+'" multiple="true" onchange="javascript:for(var i=0; i<this.files.length; i++){JS9.Load(this.files[i], {display:\''+this.id+"'}); }\"> </div>");this.divjq.append('<div style="visibility:hidden; position:relative; top:-50;left:-50"> <input type="file" id="openLocalRegions-'+

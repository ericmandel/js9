...
d,e)}});return this};a.Image.prototype.starbaseToShapes=function(d,b){var c,e,f,g,h,j,k,m,n,q;m=a.globalOpts.catalogs.ras;k=a.globalOpts.catalogs.decs;var l=[];n=function(a,d,b,c){var e,f;if(void 0!==c)f=c;else{f=-1;for(e=0;e<b.length;e++){for(c=0;c<d.length;c++)if(b[e].toLowerCase()===d[c].toLowerCase()){f=a[d[c]];break}if(0<=f)break}}return f};if(d&&d.data&&d.headline){j=d.data;f=d.headline;b=b||{};m=n(d,f,m,b.xcol);0>m&&a.error("xcol not specified");n=n(d,f,k,b.ycol);0>n&&a.error("ycol not specified");
f=b.shape||"circle";switch(f){case "box":k=function(){return{width:b.width||7,height:b.height||7}};break;case "circle":k=function(){return{radius:b.radius||3.5}};break;case "ellipse":k=function(){return{width:b.width||7,height:b.height||7}};break;default:k=function(){return{width:b.width||7,height:b.height||7}}}q=this.getWCSSys();c=b.wcssys?b.wcssys:"ICRS";this.setWCSSys(c);for(e=c=0;c<j.length;c++)g=void 0,g=(g=a.wcs2pix(this.raw.wcs,15*j[c][m],j[c][n]).trim().split(/ +/))&&g.length?{x:parseFloat(g[0]),
y:parseFloat(g[1])}:null,g&&(h=k.call(this,j[c][1],j[c][1]),g={id:c.toString(),shape:f,x:g.x,y:g.y,width:h.width,height:h.height,radius:h.radius,angle:0,data:{ra:15*j[c][m],dec:j[c][n]}},b.color&&(g.color=b.color),l[e++]=g);this.setWCSSys(q);return l}};a.Image.prototype.loadCatalog=function(d,b,c){var e,f,g=$.extend(!0,{},a.Catalogs.opts);a.globalOpts.catalogs.tooltip&&(g.tooltip=a.globalOpts.catalogs.tooltip);c=c||{};c.color=c.color||a.globalOpts.catalogs.color||"#00FF00";c.wcssys=c.wcssys||a.globalOpts.catalogs.wcssys;
f=new a.Starbase(b,{convFuncs:{def:a.saostrtod},units:c.units||a.globalOpts.catalogs.units,skip:c.skip||a.globalOpts.catalogs.skip});(!f||!f.data||!f.data.length)&&a.error("no objects found in catalog");e=this.starbaseToShapes(f,c);e.length?(d=d||"catalog_"+a.uniqueID(),this.display.newShapeLayer(d,g),this.removeShapes(d),this.addShapes(d,e,c),this.layers[d].catalog=b,this.layers[d].starbase=f):a.error("no catalog objects found");return this};a.Image.prototype.saveCatalog=function(d,b){var c;d=d||
this.activeShapeLayer();if(!this.layers[d]||!this.layers[d].catalog)d&&"undefined"!==d?a.error("no catalog available: "+d):a.error("no active catalog available");c=new Blob([this.layers[d].catalog],{type:"text/plain;charset=utf-8"});b=b||d+".cat";window.hasOwnProperty("saveAs")?saveAs(c,b):a.error("no saveAs function available to save catalog");return b};a.Command=function(d){for(var b in d)d.hasOwnProperty(b)&&(this[b]=d[b]);d.name||a.error("command has no name");!d.get&&!d.set&&a.error("command requires get and/or set routine");
...
return null===a.helper.connected?"notConfigured":a.helper.connected?(d=sprintf("connected %s %s",a.helper.type,a.helper.url),a.helper.pageid&&(d+="<p>"+a.helper.pageid),d):sprintf("notConnected %s",a.helper.type)};a.Helper.prototype.connect=function(d){var b=this;d&&(this.type=d);if(this.socket){try{this.socket.disconnect()}catch(c){a.log("warning: can't disconnect from socket")}this.socket=null}this.url=a.globalOpts.helperURL?0<=a.globalOpts.helperURL.search(/:\/\//)?a.globalOpts.helperURL:a.globalOpts.helperProtocol+
a.globalOpts.helperURL:document.domain?a.globalOpts.helperProtocol+document.domain:a.globalOpts.helperProtocol+"localhost";switch(this.type){case "none":this.connected=null;$(document).trigger("JS9:ready",{type:"none",status:"OK"});a.Preload(!0);break;case "get":case "post":a.globalOpts.helperCGI||a.error("cgi script name missing for helper");this.url+="/"+a.globalOpts.helperCGI;this.helper=this.connected=!0;a.DEBUG&&a.log("JS9 helper: connect: "+this.type);$(document).trigger("JS9:ready",{type:"get",
status:"OK"});a.Preload(!0);break;case "sock.io":case "nodejs":a.globalOpts.helperPort||a.error("port missing for helper");this.url+=":"+a.globalOpts.helperPort;this.sockurl=this.url+"/socket.io/socket.io.js";$.ajax({url:this.sockurl,dataType:"script",timeout:a.globalOpts.htimeout,success:function(){var d,c;b.socket=io.connect(b.url,{reconnection:!0,reconnectionDelay:1E4,reconnectionDelayMax:1E4,reconnectionAttempts:6,timeout:a.globalOpts.htimeout});b.socket.on("connect",function(){b.connected=!0;
b.helper=!0;c=[];for(d=0;d<a.displays.length;d++)c.push(a.displays[d].id);b.socket.emit("displays",{displays:c},function(a){b.pageid=a});$(document).trigger("JS9:ready",{type:"socket.io",status:"OK"});a.Preload(!0);a.DEBUG&&a.log("JS9 helper: connect: "+b.type)});b.socket.on("connect_error",function(){b.connected=!1;b.helper=!1;a.Preload(!0);1<a.DEBUG&&a.log("JS9 helper: connect error")});b.socket.on("connect_timeout",function(){b.connected=!1;b.helper=!1;a.Preload(!0);1<a.DEBUG&&a.log("JS9 helper: connect timeout")});
b.socket.on("disconnect",function(){b.connected=!1;b.helper=!1;1<a.DEBUG&&a.log("JS9 helper: disconnect")});b.socket.on("reconnect",function(){b.connected=!0;b.helper=!0;1<a.DEBUG&&a.log("JS9 helper: reconnect")});b.socket.on("msg",a.msgHandler)},error:function(d,c,g){b.connected=!1;b.helper=!1;$(document).trigger("JS9:ready",{type:"socket.io",status:"error"});a.Preload(!0);a.DEBUG&&a.log("JS9 helper: connect failure: "+c+" "+g)}});break;default:a.error("unknown helper type: "+this.type)}};a.Helper.prototype.send=
function(d,b,c){if(!this.connected)return null;b&&"object"===typeof b?(b.cookie=document.cookie,a.globalOpts.dataPath&&!b.dataPath&&(b.dataPath=a.globalOpts.dataPath+":.")):b={dataPath:"."};a.TOROOT&&(b.dataPath+=":"+a.TOROOT);switch(this.type){case "get":case "post":b.key=d;a.DEBUG&&a.log("JS9 cgi helper [%s, %s]: %s",this.type,JSON.stringify(b),this.url);$.ajax({url:this.url,type:this.type.toUpperCase(),data:b,dataType:"text",success:function(d){"string"===typeof d&&0<=d.search(a.analOpts.epattern)&&
...
!0,f.opts.lockRotation=!0,f.opts.lockScalingX=!0,f.opts.lockScalingY=!0,f.opts.lockUniScaling=!0,f.opts.selectable=!1,f.opts.evented=!1,f.opts.usekeyboard=!1);f.opts.selectable&&(f.opts.canvas.selection=!0);if(f.opts.onmousedown||f.opts.onmouseup||f.opts.onmousemove||f.opts.tooltip||f.opts.onmouseover||f.opts.onmouseout){f.opts.evented=!0;if(f.opts.onmousedown)f.canvas.on("mouse:down",function(d){if(f.display.image&&d.target)"main"===f.dtype&&(f.display.image.clickInRegion=!0),f.opts.onmousedown.call(this,
f.display.image,d.target.pub,d.e,d.target);else if(this._selection=this.selection)this.selection=a.specialKey(d.e)});else f.canvas.on("mouse:down",function(d){if(this._selection=this.selection)this.selection=a.specialKey(d.e)});if(f.opts.onmouseup)f.canvas.on("mouse:up",function(a){f.display.image&&a.target&&f.opts.onmouseup.call(this,f.display.image,a.target.pub,a.e,a.target);this.selection=this._selection||this.selection});else f.canvas.on("mouse:up",function(){this.selection=this._selection||this.selection});
...
function(d){f.display.image&&d.target&&a.tooltip(d.target.left,d.target.top,f.opts.tooltip,f.display.image,d.target.pub,d.e,d.target)}),f.canvas.on("mouse:out",function(d){f.display.image&&d.target&&a.tooltip(d.target.left,d.target.top,null,f.display.image,d.target.pub,d.e,d.target)}))}else f.canvas.on("mouse:down",function(d){if(this._selection=this.selection)this.selection=a.specialKey(d.e)}),f.canvas.on("mouse:up",function(){this.selection=this._selection||this.selection});if(f.opts.ongroupcreate&&

...
"auto";g=c.leftMargin||B.leftMargin||0;h=c.topMargin||B.topMargin||0;switch(f){case "auto":y=1;w=0;break;case "horizontal":y=1;w=0;break;case "vertical":y=0;w=1;break;default:y=1,w=0}k=43;l=0;n=$("#"+a);m=$("#"+a+"Menubar");0<m.length&&(m.isactive="visible"===m.closest(".JS9PluginContainer").css("visibility"));q=$("#"+a+"Toolbar");0<q.length&&(q.isactive="visible"===q.closest(".JS9PluginContainer").css("visibility"));t=$("#"+a+"Colorbar");0<t.length&&(t.isactive="visible"===t.closest(".JS9PluginContainer").css("visibility"));
0<n.length&&(r=n.width(),u=n.height(),p=n.offset().top-$(window).scrollTop(),x=n.offset().left-$(document).scrollLeft(),m.isactive&&(u+=m.height(),p-=m.height()),q.isactive&&(u+=q.height(),p-=q.height()),t.isactive&&(u+=t.height(),p-=t.height(),p+=7))},z=function(a,b){var c,d;a&&(0<n.length&&(c="",m.isactive&&(c+=sprintf("<div class='JS9Menubar' id='%sMenubar' data-width=%s></div>",b,n.width())),q.isactive&&(c+=sprintf("<div class='JS9Toolbar' id='%sToolbar' data-width=%s></div>",b,n.width())),c+=
sprintf("<div class='JS9' id='%s' data-width=%s data-height=%s></div>",b,n.width()-C,n.height()-C),t.isactive&&(c+=sprintf("<div style='margin-top: 2px;'><div class='JS9Colorbar' id='%sColorbar' data-width=%s></div></div>",b,n.width()))),"auto"===f&&x+r*(y+.5)>window.innerWidth&&(w++,y=0),d=sprintf("width=%s,height=%s,top=%s,left=%s,resize=1,scolling=1",r,u,p+(u+h+k)*w,x+(r+g+l)*y),"auto"===f||"horizontal"===f?y++:"vertical"===f&&w++);return{id:b,html:c,winopts:d}},E=function(f){var g,h,k;b.images.length>
f&&(g=b.images[f],g.displayImage("all"),c===g.display?void 0===d?(d=b.images[f].display.id,D(d,a),E(f+1)):(e="string"===typeof a.idbase?k=a.idbase+v++:d.replace(A,"")+"_sep"+b.uniqueID(),$("#dhtmlwindowholder").arrive("#"+e,{onceOnly:!0},function(){window.setTimeout(function(){g.moveToDisplay(e);E(f+1)},0)}),h=z(d,e),k&&(h.id=k),b.LoadWindow(null,h)):E(f+1))};if("string"===typeof a)try{a=JSON.parse(a)}catch(G){b.error("can't parse displaySeparate opts: "+a,G)}a=a||{};E(0)};b.Display.prototype.nextImage=
...
{type:"get",status:"OK"});break;case "sock.io":case "nodejs":b.globalOpts.helperPort||b.error("port missing for helper");this.url=this.url.replace(/:[0-9][0-9]*$/,"")+":"+b.globalOpts.helperPort;this.sockurl=this.url+"/socket.io/socket.io.js";window.isElectron?(this.aliveurl=this.url+"/alive",h(this.aliveurl,this.sockurl,c)):g(this.sockurl);break;default:b.error("unknown helper type: "+this.type)}};b.Helper.prototype.send=function(a,c,d){if(!this.connected)return null;if(c&&"object"===typeof c){try{c.cookie=
document.cookie}catch(e){delete c.cookie}b.globalOpts.dataPath&&!c.dataPath&&(c.dataPath=b.globalOpts.dataPath+":.")}else c={dataPath:"."};b.TOROOT&&(c.dataPath+=":"+b.TOROOT);switch(this.type){case "get":case "post":c.key=a;b.helper.pageid&&(c.pageid=b.helper.pageid);b.DEBUG&&b.log("JS9 cgi helper [%s, %s]: %s",this.type,JSON.stringify(c),this.url);$.ajax({url:this.url,type:this.type.toUpperCase(),data:c,dataType:"text",success:function(a){"string"===typeof a&&0<=a.search(b.analOpts.epattern)&&b.log(a);
...
for(b in this.layers)this.layers.hasOwnProperty(b)&&(d=this.layers[b],d!==f&&d.zindex===f.zindex&&"main"===d.dlayer.dtype&&(d.zindex=e,d.dlayer.divjq.css("z-index",d.zindex)));this.xeqPlugins("shape","onshapelayeractive",a)}a=this}else{for(b in this.layers)this.layers.hasOwnProperty(b)&&(d=this.layers[b],"main"===d.dlayer.dtype&&(void 0===f||d.zindex>f)&&(f=d.zindex,e=b));a=e}return a};b.Fabric._parseShapeOptions=function(a,c,d){var e,f,g,h,k,l,n={},m={};h="main"===this.display.layers[a].dtype?this.rgb.sect.zoom:
1;if(c.remove)return{remove:c.remove};m.tags=[];if(c.tags)if("string"===typeof c.tags)for(f=c.tags.toLowerCase().split(","),a=0;a<f.length;a++)m.tags[a]=f[a].trim();else if($.isArray(c.tags))for(a=0;a<c.tags.length;a++)m.tags[a]=c.tags[a].trim();void 0!==c.angle&&(n.angle=-c.angle);void 0!==c.x&&void 0!==c.y&&(f=this.imageToDisplayPos(c),n.left=f.x,n.top=f.y);void 0!==c.px&&void 0!==c.py&&(f=this.logicalToDisplayPos({x:c.px,y:c.py}),n.left=f.x,n.top=f.y);void 0!==c.ra&&void 0!==c.dec&&0<this.raw.wcs&&
("string"===typeof c.ra&&(c.ra=b.saostrtod(c.ra),":"===String.fromCharCode(b.saodtype())&&"galactic"!==this.params.wcssys&&"ecliptic"!==this.params.wcssys&&(c.ra*=15)),"string"===typeof c.dec&&(c.dec=b.saostrtod(c.dec)),c.wcssys?(g=this.getWCSSys(),e=b.globalOpts.xeqPlugins,b.globalOpts.xeqPlugins=!1,this.setWCSSys(c.wcssys)):c._wcssys&&(g=this.getWCSSys(),e=b.globalOpts.xeqPlugins,b.globalOpts.xeqPlugins=!1,this.setWCSSys(c._wcssys),delete c._wcssys),f=b.wcs2pix(this.raw.wcs,c.ra,c.dec).trim().split(/ +/),
g&&(this.setWCSSys(g),b.globalOpts.xeqPlugins=e),f=this.imageToDisplayPos({x:parseFloat(f[0]),y:parseFloat(f[1])}),n.left=f.x,n.top=f.y);void 0!==c.left&&(n.left=c.left);void 0!==c.top&&(n.top=c.top);void 0===n.left&&(n.left=d&&void 0!==d.left?d.left:this.display.canvasjq.attr("width")/2-1);void 0===n.top&&(n.top=d&&void 0!==d.top?d.top:this.display.canvasjq.attr("height")/2-1+1);c.dx&&(n.left+=c.dx);c.dy&&(n.top-=c.dy);switch(c.shape){case "annulus":m.radii=[];if(void 0!==c.radii)if("string"===typeof c.radii)for(m.radii=
c.radii.replace(/ /g,"").split(","),e=a=0;a<m.radii.length;a++)""!==m.radii[a]&&(m.radii[e++]=this.wcs2imlen(m.radii[a]));else m.radii=c.radii;else for(c.ireg&&b.SCALEIREG&&(c.iradius&&(c.iradius/=h),c.oradius&&(c.oradius/=h)),h=(c.oradius-c.iradius)/c.nannuli,g=c.nannuli+1,a=0;a<g;a++)e=c.iradius+h*a,m.radii.push(e);break;case "box":c.ireg&&b.SCALEIREG&&(c.width&&(c.width/=h),c.height&&(c.height/=h));break;case "circle":c.ireg&&b.SCALEIREG&&c.radius&&(c.radius/=h);break;case "ellipse":c.ireg&&b.SCALEIREG&&
(c.r1&&(c.r1/=h),c.r2&&(c.r2/=h));break;case "point":switch(c.ptshape){case "box":c.width=2*c.ptsize;c.height=2*c.ptsize;break;case "circle":c.radius=c.ptsize;break;case "ellipse":c.rx=c.ptsize,c.ry=c.ptsize/2}c.lockRotation=!0;c.lockScalingX=!0;c.lockScalingY=!0;c.lockUniScaling=!0;c.hasControls=!1;c.hasRotatingPoint=!1;c.hasBorders=!0;break;case "line":case "polygon":if(c.pts&&c.pts.length){if("string"===typeof c.pts){f=c.pts.replace(/ /g,"").split(",");g=f.length;if("string"===typeof f[0])for(a=
0;a<g;a++)f[a]=parseFloat(f[a]);c.pts=[];for(e=a=0;a<g;a+=2,e++)c.pts[e]={x:f[a],y:f[a+1]}}g=c.pts.length;for(a=0;a<g;a++)c.pts[a]=this.imageToDisplayPos(c.pts[a]);c.left&&c.top?e={x:c.left,y:c.top}:(e=b.centerPolygon(c.pts),n.left=e.x,n.top=e.y);c.points=[];for(a=0;a<g;a++)f={x:(c.pts[a].x-e.x)/h,y:(c.pts[a].y-e.y)/h},c.points.push(f)}else d&&d.points&&d.points.length||("polygon"===c.shape&&c.polypoints?c.points=c.polypoints:"line"===c.shape&&c.linepoints&&(c.points=c.linepoints));if(c.ireg&&b.SCALEIREG)for(g=
c.points.length,a=0;a<g;a++)c.points[a].x/=h,c.points[a].y/=h}for(k in c)if(c.hasOwnProperty(k))switch(k){case "tags":case "x":case "y":case "px":case "py":case "dx":case "dy":case "ra":case "dec":case "pts":case "left":case "top":case "angle":case "radii":case "ireg":break;case "type":case "originX":case "originY":case "width":case "height":case "scaleX":case "scaleY":case "flipX":case "flipY":case "opacity":case "cornerSize":case "transparentCorners":case "hoverCursor":case "padding":case "borderColor":case "cornerColor":case "centeredScaling":case "centeredRotation":case "fill":case "fillRule":case "backgroundColor":case "stroke":case "strokeWidth":case "strokeDashArray":case "strokeLineCap":case "strokeLineJoin":case "strokeMiterLimit":case "shadow":case "borderOpacityWhenMoving":case "borderScaleFactor":case "borderDashArray":case "transformMatrix":case "minScaleLimit":case "selectable":case "evented":case "visible":case "hasControls":case "hasBorders":case "hasRotatingPoint":case "rotatingPointOffset":case "perPixelTargetFind":case "includeDefaultValues":case "clipTo":case "lockMovementX":case "lockMovementY":case "lockRotation":case "lockScalingX":case "lockScalingY":case "lockUniScaling":case "radius":case "rx":case "ry":case "points":case "selectionLineWidth":case "fontFamily":case "fontSize":case "fontStyle":case "fontWeight":case "text":case "textDecoration":case "textAlign":case "lineHeight":case "textBackgroundColor":case "textOpts":n[k]=

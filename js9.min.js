...
typeof c&&0>c.search(/ERROR/))&&(g="JS9 ERROR: "),alert(g+(c+h))),d))throw e;};a.eventToDisplayPos=function(a,b){var d,e,f,g,h;a||(a=window.event);a.target?e=a.target:a.srcElement&&(e=a.srcElement);3===e.nodeType&&(e=e.parentNode);b=b||$(e).offset();(h=a.originalEvent.touches)?h.length?(d=h[0].pageX,f=h[0].pageY):a.originalEvent.changedTouches&&a.originalEvent.changedTouches.length&&(d=a.originalEvent.changedTouches[0].pageX,f=a.originalEvent.changedTouches[0].pageY):(d=a.pageX,f=a.pageY);e=b.left+
1;g=b.top+1;f={x:Math.floor(d-e),y:Math.floor(f-g)};if(h&&h.length){f.touches=[{x:f.x,y:f.y}];for(d=1;d<h.length;d++)f.touches[d]={x:Math.floor(h[d].pageX-e),y:Math.floor(h[d].pageY-g)}}return f};a.rotatePoint=function(a,b,d){var e;d=d||{x:0,y:0};b=Math.PI*b/180;e=Math.cos(b);b=Math.sin(b);return{x:e*(a.x-d.x)-b*(a.y-d.y)+d.x,y:b*(a.x-d.x)+e*(a.y-d.y)+d.y}};a.log=function(){if(void 0!==window.console&&void 0!==window.console.log)try{console.log.apply(console,arguments)}catch(a){Function.prototype.bind.call(console.log,
console).apply(console,arguments)}};a.isNumber=function(a){return!isNaN(parseFloat(a))&&isFinite(a)};a.notNull=function(a){return void 0!==a&&null!==a};a.cardpars=function(c){var b;if("="===c[8])return b=c.slice(0,8).trim(),c=c.slice(10).replace(/\'/g," ").replace(/\/.*/,"").trim(),"T"===c?c=!0:"F"===c?c=!1:a.isNumber(c)&&(c=parseFloat(c)),[b,c]};a.raw2FITS=function(a,b){var d,e,f,g=!1,h="";if(!a)return h;if(a.card)for(d=0;d<a.card.length;d++)e=a.card[d],h+=e,"END "===e.substring(0,4)&&(g=!0),b&&
...
b){var d,e,f,g;e=Array.prototype.slice.call(arguments,2);d=typeof c;switch(d){case "function":return c.apply(b,e);case "string":f=c.split(".");g=f.pop();for(d=0;d<f.length;d++)b=b[f[d]];return b[g].apply(b,e);default:a.error("unknown function type: "+d)}};a.loadPrefs=function(c,b){$.ajax({url:c,dataType:"json",async:!1,success:function(b){var c,f,g;for(g in b)if(b.hasOwnProperty(g)&&a.hasOwnProperty(g)&&(f=typeof a[g],c=typeof b[g],f===c||"string"===c))switch(f){case "object":$.isArray(b[g])?a[g]=
b[g]:$.extend(!0,a[g],b[g]);break;case "number":case "string":a[g]=b[g]}},error:function(d,e,f){b&&(a.CHROMEFILEWARNING&&"Chrome"===a.BROWSER[0]&&""===document.domain&&f&&f.message&&f.message.match(/Failed to execute 'send' on 'XMLHttpRequest'/)?(alert("When using the file:// URI, Chrome must be run with the --allow-file-access-from-files switch to permit JS9 to access the preference file (and data files)."),a.CHROMEFILEWARNING=!1):a.log("JS9 prefs file not available: %s",c))}})};a.isTypedArray=function(a){a=

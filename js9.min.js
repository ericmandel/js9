...
d,y:c.top+b},c.angle,{x:c.left,y:c.top}):(f.x=c.left+d,f.y=c.top+b);if(5<=fabric.version.split(".")[1])for(d/=c.scaleX,e=b/c.scaleY,b=0;b<c.points.length;b++)c.points[b].x-=d,c.points[b].y-=e;c.left=f.x;c.top=f.y;c.setCoords()};a.Fabric.print=function(c){var b,d,e,f,g,h=0,k=sprintf("width=%s,height=%s,menubar=1,toolbar=1,status=0,scrollbars=1,resizable=1",this.display.canvasjq.attr("width"),this.display.canvasjq.attr("height"));c=c||{};f=this.display.canvas.toDataURL("image/png");b+="<html><body style='padding: 0px; margin: 0px' onload='window.print(); return false'>";
g=sprintf("<div style='position:absolute; left:%spx; top:%spx'>",0,h);b+=sprintf("%s<img src='%s'></div>",g,f);for(d in this.layers)this.layers.hasOwnProperty(d)&&(f=this.layers[d],"main"===f.dlayer.dtype&&f.show&&(b+=sprintf("%s%s</div>",g,f.dlayer.canvas.toSVG())));(void 0===c.colorbar||c.colorbar)&&(c=this.display.pluginInstances.JS9Colorbar)&&c.isActive()&&(h+=2,f=c.colorbarjq[0].toDataURL("image/png"),h+=this.display.height,g=sprintf("<div style='position:absolute; left:%spx; top:%spx'>",0,h),
b+=sprintf("%s<img src='%s'></div>",g,f),c.textjq&&c.textjq[0]&&(f=c.textjq[0].toDataURL("image/png"),h+=c.colorbarjq.height()+1,g=sprintf("<div style='position:absolute; left:%spx; top:%spx'>",0,h),b+=sprintf("%s<img style='width:%spx;'src='%s'></div>",g,this.display.width,f)));b+="</body></html>";(e=window.open("data:text/html,<html><body><script>window.addEventListener('message', function(ev){document.documentElement.innerHTML=ev.data; window.setTimeout(function(){window.print()}, 250);},false)\x3c/script><p>waiting for image ...</body></html>",
this.id,k))?e.document?(e.document.open(),e.document.write(b),e.document.close()):e.postMessage?window.setTimeout(function(){e.postMessage(b,"*")},250):a.error("no method available for drawing image into print window"):a.error("could not create print window (check your pop-up blockers)")};a.Fabric.initGraphics=function(){var c;a.Display.prototype.newShapeLayer=a.Fabric.newShapeLayer;a.Image.prototype.addShapes=a.Fabric.addShapes;a.Image.prototype.selectShapes=a.Fabric.selectShapes;a.Image.prototype.updateShapes=

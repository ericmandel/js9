...
break;default:j(d,f,g)&&(l[f]=m(g))}this.changeShapes(d.pub.layer,d,l);a.Regions.initConfigForm.call(this,d,b)};a.Regions.listRegions=function(a,b){var c,e,f,g,h,l,j="",m="none",n=!1,q=[];void 0===b&&(b=2);q=this.getShapes("regions",a);f=q.length;if(b)for(c=0;c<f;c++)if(e=q[c],"source,include"!==e.tags.join(",")){n=!0;break}for(c=0;c<f;c++)e=q[c],h=e.tags.join(","),l=0<=h.indexOf("exclude")?"-":"",n&&(g=" # "+h),e.wcsstr&&"image"!==this.params.wcssys&&"physical"!==this.params.wcssys?("wcs"!==m&&("none"!==
m&&(j+="; "),j+=this.params.wcssys,m="wcs"),j+="; "+l+e.wcsstr):e.imstr&&(m!==e.imsys&&("none"!==m&&(j+="; "),j+=e.imsys,m=e.imsys),j+="; "+l+e.imstr),n&&(j+=g);1<b&&this.displayMessage("regions",j);return j};a.Regions.parseRegions=function(d){var b=[],c,e,f,g,h,l,j,m,n,q,k,r=/(annulus)|(box)|(circle)|(ellipse)|(polygon)|(point)|(text)/,p=/(fk4)|(fk5)|(icrs)|(galactic)|(ecliptic)|(image)|(physical)/,s=/\(\s*([^)]+?)\s*\)/,t=/(\{[^}]*\})/,u=/\s*,\s*/,v=function(b){b=a.saostrtod(b);var c=String.fromCharCode(a.saodtype());
switch(c){case '"':b/=3600;break;case "'":b/=60;break;case "r":b*=180/Math.PI}return{dval:b,dtype:c}},w=function(b,c){var d,e;e=v(b);d=v(c);m?(":"===e.dtype&&("galactic"!==j&&"ecliptic"!==j)&&(e.dval*=15),d=a.wcs2pix(this.wcs,e.dval,d.dval).split(/ +/),e=parseFloat(d[0])-1,d=parseFloat(d[1])-1):"physical"===j?(d=this.logicalToImagePos({x:e.dval,y:d.dval}),e=d.x,d=d.y):(e=e.dval,d=d.dval);return[e,d]},x=function(a,b){var c=v(a);m&&(c.dtype&&"."!==c.dtype)&&(c.dval=Math.abs(c.dval/q["cdelt"+b]));return c.dval},
y=function(a){a=v(a);m&&(a.dval+=q.crot,q.imflip&&(a.dval=-a.dval));return a.dval};d=d.trim();if(!d.match(/(\C|\{|#|;|\n)/))return d;try{q=JSON.parse(a.wcsinfo(this.wcs))}catch(A){q={cdelt1:1,cdelt2:1,crot:0,imflip:0}}l=this.getWCSSys();j="physical";this.setWCSSys(j);m="image"!==j&&"physical"!==j;f=d.split(/\n|;/);for(d=0;d<f.length;d++)if("#"!==f[d].trim().substr(0,1)){g=f[d];h=void 0;c={opts:{},args:[],isregion:0};c.cmd=0<=g.indexOf("(")?g.split("(")[0].trim().toLowerCase():0<=g.indexOf("{")?g.split("{")[0].trim().toLowerCase():
0<=g.indexOf("#")?g.split("#")[0].trim().toLowerCase():g.trim().toLowerCase();c.cmd&&(c.isregion=0<=c.cmd.search(r));c.comment=g.split("#")[1];c.comment&&(c.comment=c.comment.trim().toLowerCase());if((h=t.exec(g))&&h[1])try{c.opts=JSON.parse(h[1].trim())}catch(B){c.opts={}}if((h=s.exec(g))&&h[1])c.args=h[1].split(u);c.isregion&&0===c.cmd.indexOf("-")&&(c.cmd=c.cmd.slice(1),c.comment=c.comment?c.comment+",exclude":"exclude");h=c;k=h.args.length;if(h.isregion){g=$.extend(!0,{},h.opts);g.shape=h.cmd;
2<=k&&(n=w.call(this,h.args[0],h.args[1]),g.x=n[0],g.y=n[1]);switch(h.cmd){case "annulus":g.radii=[];for(c=2;c<k;c++)g.radii.push(x.call(this,h.args[c],1));break;case "box":3<=k&&(g.width=x.call(this,h.args[2],1));4<=k&&(g.height=x.call(this,h.args[3],2));5<=k&&(g.angle=y.call(this,h.args[4]));break;case "circle":3<=k&&(g.radius=x.call(this,h.args[2],1));break;case "ellipse":3<=k&&(g.r1=x.call(this,h.args[2],1));4<=k&&(g.r2=x.call(this,h.args[3],2));5<=k&&(g.angle=y.call(this,h.args[4]));break;case "polygon":g.pts=
[];for(e=c=0;c<k;c+=2,e++)n=w.call(this,h.args[c],h.args[c+1]),g.pts[e]={x:n[0],y:n[1]};delete g.x;delete g.y;break;case "text":3<=k&&(c=g,e=h.args[2].replace(/^['"]/,"").replace(/['"]$/,""),c.text=e)}h.comment&&(g.tags=h.comment);b.push(g)}else h.cmd.match(p)?(this.setWCSSys(h.cmd),j=this.getWCSSys(),m="image"!==j&&"physical"!==j):("remove"===h.cmd||"delete"===h.cmd)&&b.push({remove:!0})}this.setWCSSys(l);return b};a.Regions.saveRegions=function(d){var b=sprintf("# Region file format: JS9 version 1.0");

...
f.length||b.error("no regions found: "+a);d="";for(a=0;a<f.length;a++)g=f[a],c.params.wcssys?(d||(d=g.wcssys||""),d+=sprintf("; %s",g.wcsstr)):(d||(d=g.imsys||""),d+=sprintf("; %s",g.imstr));return d||e};this.raw.hdu&&this.raw.hdu.fits&&this.raw.hdu.fits.vfile||b.error("no virtual file available for regcnts: "+this.id);for(d=0;d<arguments.length;d++)if(e=arguments[d],"string"===typeof e&&"{"===e.charAt(0))try{arguments[d]=JSON.parse(e)}catch(n){b.error("can't parse json arg in regcnts: "+e,n)}switch(arguments.length){case 0:break;
...
b.fits.options.table.xdim;h=d.ydim||b.fits.options.image.ydim||b.fits.options.table.ydim;k=d.bin||b.fits.options.image.bin||b.fits.options.table.bin;l=d.binMode||b.globalOpts.binMode;l="a"===l?"a":"";m.sect=void 0!==d.xcen&&void 0!==d.ycen?sprintf("%s@%s,%s@%s,%s%s",e,d.xcen,h,d.ycen,k,l):sprintf("%s,%s,%s%s",e,h,k,l);e=g.toLowerCase().split(/[>,]/);for(g=0;g<e.length;g++)switch(e[g]){case "size":e[g+1]&&(b.isNumber(e[g+1])&&(m.maxsize=1E6*parseFloat(e[g+1])),g++)}break;default:b.error("unknown FITS representation type: "+

...
d,y:c.top+b},c.angle,{x:c.left,y:c.top}):(f.x=c.left+d,f.y=c.top+b);if(5<=fabric.version.split(".")[1])for(d/=c.scaleX,e=b/c.scaleY,b=0;b<c.points.length;b++)c.points[b].x-=d,c.points[b].y-=e;c.left=f.x;c.top=f.y;c.setCoords()};a.Fabric.print=function(c){var b,d,e,f,g,h,k=0,l=sprintf("width=%s,height=%s,menubar=1,toolbar=1,status=0,scrollbars=1,resizable=1",this.display.canvasjq.attr("width"),this.display.canvasjq.attr("height"));c=c||{};f=this.display.canvas.toDataURL("image/png");b="<html><body style='padding: 0px; margin: 0px' onload='window.print(); return false'>";

...
h.savepng={name:"save image as PNG"};h.close={name:"close image"};h["sep"+f++]="------";h.lite={name:"new JS9 light window"};h.xnew={name:"new JS9 separate window"};return{callback:function(b){d().forEach(function(d){var c,g=d.image;switch(b){case "close":g&&g.closeImage();break;case "header":g&&(g.raw.header?g.displayAnalysis("text",a.raw2FITS(g.raw,!0),"FITSHeader_"+g.id):a.error("no FITS header for "+g.id));break;case "lite":a.LoadWindow(null,null,"light");break;case "xnew":a.LoadWindow(null,null,
"new");break;case "pageid":d="<center><p>Usage: js9 -pageid [pageid] ...<p>"+a.helper.pageid||"none</center>";a.lightWin("fileid"+a.uniqueID(),"inline",d,"page ID",a.lightOpts[a.LIGHTWIN].lineWin);break;case "open":a.OpenFileMenu(d);break;case "savefits":g&&(d=g.id.replace(/png/,"fits").replace(/.gz$/,"").replace(/\[.*\]/,""),g.saveFITS(d));break;case "savepng":g&&(d=g.id.replace(/fits/,"png").replace(/.gz$/,"").replace(/\[.*\]/,""),g.savePNG(d));break;case "print":g&&g.print();break;default:for(c=
0;c<a.images.length;c++)if(g=a.images[c],d.id===g.display.id&&g.id===b){g.displayImage("display");g.clearMessage();break}}})},items:h}}});$("#viewMenu"+g.id).on("mousedown",function(a){a.preventDefault();$("#viewMenu"+g.id).contextMenu()});$.contextMenu({selector:"#viewMenu"+g.id,zIndex:a.MENUZINDEX,events:{hide:b},build:function(){var b,c,g,e,f,h="",p=0,s=0,t={},u=d()[0],w=u.image;for(b=0;b<a.plugins.length;b++)if(c=a.plugins[b],g=c.name,c.opts.menuItem&&"view"===c.opts.menu&&(e=u.pluginInstances[g],
...
d.params.z1,d.params.scalemax=d.params.z2,$.contextMenu.setInputValues(b,{scalemin:String(d.params.scalemin),scalemax:String(d.params.scalemax)}),d.displayImage("colors"),!1;default:d.setScale(a)}})},events:{show:function(a){var b=g.display.image,d={};b&&(d.scalemin=String(b.params.scalemin),d.scalemax=String(b.params.scalemax));$.contextMenu.setInputValues(a,d)},hide:function(a){var b=g.display.image;b&&a.edited&&(delete a.edited,a=$.contextMenu.getInputValues(a),p(b,a))}},items:h}}});$("#colorMenu"+
...
b))})},items:f}}});$("#analysisMenu"+g.id).on("mousedown",function(a){a.preventDefault();$("#analysisMenu"+g.id).contextMenu()});$.contextMenu({selector:"#analysisMenu"+g.id,zIndex:a.MENUZINDEX,events:{hide:b},build:function(){var b,c,g,e,f,h=0,p=0,s={};f=d()[0];var t=f.image,u="";for(b=0;b<a.plugins.length;b++)if(c=a.plugins[b],e=c.name,c.opts.menuItem&&"analysis"===c.opts.menu&&(g=f.pluginInstances[e],!g||g.winHandle))c.xclass!==u&&(s["sep"+p++]="------",s["sep"+p++]={name:c.xclass+" Plugins:"},
...
this.type);a.Preload(!0);break;case "sock.io":case "nodejs":a.globalOpts.helperPort||a.error("port missing for helper");this.url+=":"+a.globalOpts.helperPort;this.sockurl=this.url+"/socket.io/socket.io.js";$.ajax({url:this.sockurl,dataType:"script",success:function(){var c,d;b.socket=io.connect(b.url);b.socket.on("connect",function(){b.connected=!0;b.helper=!0;d=[];for(c=0;c<a.displays.length;c++)d.push(a.displays[c].id);b.socket.emit("displays",{displays:d},function(a){b.pageid=a});a.Preload(!0);
a.DEBUG&&a.log("JS9 helper: connect: "+b.type)});b.socket.on("disconnect",function(){b.connected=!1;b.helper=!1;1<a.DEBUG&&a.log("JS9 helper: disconnect")});b.socket.on("reconnect",function(){b.connected=!0;b.helper=!0;1<a.DEBUG&&a.log("JS9 helper: reconnect")});b.socket.on("msg",function(b,c){var d,e,f,n=[],r=b.cmd,k=b.id;a.globalOpts.alerts=!1;if(a.publics[r]){$.isArray(b.args)||(b.args=[b.args]);n=$.extend(!0,[],b.args);k&&n.push({display:k});try{f=a.publics[r].apply(null,n)}catch(q){f=sprintf("ERROR: %s",
q.message)}c&&c(f);a.globalOpts.alerts=!0}else if(!r||"#"===r)c&&c(""),a.globalOpts.alerts=!0;else{d=a.lookupCommand(r);e=a.lookupDisplay(k);if(d&&e)switch(d.getDisplayInfo(e),b.args?n=$.extend(!0,[],b.args):b.paramlist&&(n=b.paramlist.split(/ +/)),d.getWhich(n)){case "get":try{f=d.get(n)||""}catch(p){f="ERROR: "+p.message}break;case "set":try{f=d.set(n)||"OK"}catch(s){f="ERROR: "+s.message}break;default:f=sprintf("ERROR: unknown cmd type for '%s'",r)}else d||(f=sprintf("ERROR: unknown cmd '%s'",
r)),e||(f=sprintf("ERROR: unknown display (%s)",k));a.globalOpts.alerts=!0;c&&c(f)}})},error:function(b,c,d){a.DEBUG&&a.log("JS9 helper: connect failure: "+c+" "+d)}});break;default:a.error("unknown helper type: "+this.type)}};a.Helper.prototype.send=function(d,b,c){if(!this.connected)return null;b&&"object"===typeof b?(b.cookie=document.cookie,a.globalOpts.dataPath&&!b.dataPath&&(b.dataPath=a.globalOpts.dataPath+":.")):b={dataPath:"."};a.TOROOT&&(b.dataPath+=":"+a.TOROOT);switch(this.type){case "get":case "post":b.key=
d;$.ajax({url:this.url,type:this.type.toUpperCase(),data:b,dataType:"text",success:function(b){"string"===typeof b&&0<=b.search(a.analOpts.epattern)&&a.log(b);c&&c(b)},error:function(b,c,d){a.DEBUG&&a.log("JS9 helper: "+this.type+" failure: "+c+" "+d)}});break;case "sock.io":case "nodejs":a.helper.socket.emit(d,b,c)}return this};a.Fabric={};a.Fabric.elements="cornerSize cornerColor borderColor transparentCorners selectionLineWidth centeredScaling hasControls hasRotatingPoint hasBorders params pub".split(" ");
...
for(d=0;d<f.length;d++)r.tags[d]=f[d].trim()}else if($.isArray(b.tags))for(d=0;d<b.tags.length;d++)r.tags[d]=b.tags[d].trim();void 0!==b.angle&&(n.angle=-b.angle);void 0!==b.x&&void 0!==b.y&&(e=this.imageToDisplayPos(b),n.left=e.x,n.top=e.y);void 0!==b.left&&(n.left=b.left);void 0!==b.top&&(n.top=b.top);void 0===n.left&&(n.left=c&&void 0!==c.left?c.left:this.display.canvasjq.attr("width")/2-1);void 0===n.top&&(n.top=c&&void 0!==c.top?c.top:this.display.canvasjq.attr("height")/2-1+1);b.dx&&(n.left+=
b.dx);b.dy&&(n.top-=b.dy);switch(b.shape){case "annulus":r.radii=[];if(void 0!==b.radii)if("string"===typeof b.radii){r.radii=b.radii.replace(/ /g,"").split(",");for(e=d=0;d<r.radii.length;d++)""!==r.radii[d]&&(r.radii[e++]=parseInt(r.radii[d],10))}else r.radii=b.radii;else{b.ireg&&a.SCALEIREG&&(b.iradius&&(b.iradius/=l),b.oradius&&(b.oradius/=l));h=(b.oradius-b.iradius)/b.nannuli;l=b.nannuli+1;for(d=0;d<l;d++)f=b.iradius+h*d,r.radii.push(f)}break;case "box":b.ireg&&a.SCALEIREG&&(b.width&&(b.width/=

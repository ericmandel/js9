...
break;case "none":break;default:a.error("unknown analysis result type: "+e.rtype)}}});return this}a.error("could not find analysis task: "+d)}};a.Image.prototype.displayAnalysis=function(d,b,c){var e,f,g,h,j,k,m,n=a.lightOpts[a.LIGHTWIN],q=function(a,d){var c,b,e;if(!a.text)return null;b=a.x||0;if("%Y"===a.y.toUpperCase())for(c=1;c<d.length-1;c++){if(d[c][0]>b){e=Math.max(d[c-1][1],d[c][1],d[c+1][1]);break}}else e=a.y;return{x:b,y:e}},l=function(d,c,b){var e,f,g,h=b.data,k=b.annotations.color||a.plotOpts.annotateColor;
d.find(".plotAnnotation").remove();for(e=0;e<b.annotations.data.length;e++)f=b.annotations.data[e],g=q(f,h),g=c.pointOffset({x:g.x,y:g.y}),0>g.left||g.left>d.width()||(f=sprintf("<div class='plotAnnotation' style='position: absolute; left: %spx; top:%spx; color: %s; font-size: small'>%s</div>",g.left,g.top+-25,k,"&darr;"+f.text),d.append(f))};c=c||{};j=c.winformat;c.divid&&0<$("#"+c.divid).length&&(k=$("#"+c.divid));h=c.title||"";this&&!h&&(c=this.fitsFile||this.id,c=c.split("/").reverse()[0],h="AnalysisResults: "+
c+sprintf(a.IDFMT,this.display.id));c="Analysis_"+a.uniqueID();switch(d){case "text":b="<div class='JS9Analysis'></div>"+("<pre class='JS9AnalysisText'>"+(b||"")+"</pre>");b+="</div>";k?k.html(b):(j=j||n.textWin,e=a.lightWin(c,"inline",b,h,j));break;case "plot":try{f=JSON.parse(b)}catch(p){a.error("can't plot return data: "+b,p)}if(!f)return;b=sprintf("<div id='%s' class='JS9Analysis'><div id='%sPlot' class='JS9Plot' ></div></div>",c,c);k?k.html(b):(j=j||n.plotWin,e=a.lightWin(c,"inline",b,h,j));
g=$("#"+c+" #"+c+"Plot");k&&(g.css("width",k.css("width")),g.css("height",k.css("height")),g.css("margin",0));if(f.data)switch(a.globalOpts.plotLibrary){case "plotly":j=$.extend(!0,{},a.plotOpts,f.opts);f.label&&(j.title=f.label);m={x:[],y:[],type:"scatter"};3<=f.data[0].length&&(m.error_y={type:"data",array:[],visible:!0},f.points&&f.points.yerr&&f.points.yerr.color&&(m.error_y.color=f.points.yerr.color));for(b=0;b<f.data.length;b++)m.x.push(f.data[b][0]),m.y.push(f.data[b][1]),m.error_y&&m.error_y.array&&
m.error_y.array.push(f.data[b][2]);if(a.plotOpts.annotate&&f.annotations){b=j;var n=f,r,s;c=n.data;h=n.annotations.color||a.plotOpts.annotateColor;var t=[];for(d=0;d<n.annotations.data.length;d++)if(r=n.annotations.data[d],s=q(r,c))r={x:s.x,y:s.y,xref:"x",yref:"y",text:r.text,arrowcolor:h,font:{color:h},showarrow:!0,arrowhead:2,ax:0,ay:-30},t.push(r);b.annotations=t}try{Plotly.newPlot(g.attr("id"),[m],j)}catch(v){a.error("can't plot data (plotly)",v)}break;default:j=$.extend(!0,{},a.plotOpts,f.opts);
a.plotOpts.annotate&&f.annotations&&(j.zoomStack.func=function(a){l(g,a,f)});f.color=f.color||j.color;try{m=$.plot(g,[f],j)}catch(u){a.error("can't plot data (flot)",u)}a.plotOpts.annotate&&f.annotations&&l(g,m,f)}break;case "params":case "textline":k?a.allinone?k.html(b):$.ajax({url:b,cache:!1,dataType:"text",success:function(a){k.html(a)}}):(j="params"===d?j||n.paramWin:j||n.dpathWin,e=a.allinone?"inline":"ajax",e=a.lightWin(c,e,b,h,j))}return e};a.Image.prototype.loadAuxFile=function(d,b){var c=
this,e,f,g,h,j=a.auxFiles.length,k=[],m=function(){c.aux[f.name]=f;a.Image.prototype.mkOffScreenCanvas.call(h);a.Image.prototype.mkRawDataFromPNG.call(h);if(b)try{a.xeqByName(b,window,c,f)}catch(d){a.error("in aux mask onload callback",d)}a.DEBUG&&a.log("JS9 %s: %s dims(%d,%d) min/max(%d,%d)",h.type,h.file,h.raw.width,h.raw.height,h.raw.dmin,h.raw.dmax)},n=function(d){c.aux[f.name]=f;f.regions=d;if(b)try{a.xeqByName(b,window,c,f)}catch(e){a.error("in aux region onload callback",e)}},q=function(d){c.aux[f.name]=
...
(m-=j+m-a.width);if(0>=m)return!1;b=(c*a.width+b)*n;b=new Uint8Array(a.data.buffer,b,m*n);j=(k*a.width+j)*n;m=new Uint8Array(d.data.buffer,j,m*n);m.set(b)}return!0});return this};a.Image.prototype.reprojectData=function(d,b){var c=this;a.waiting(!0,this.display.divjq[0]);window.setTimeout(function(){var e={},f=!1,g,h,j,k,m,n,q,l,p,r,s,t,v,u;p=/SIMPLE|BITPIX|NAXIS|NAXIS[1-4]|AMDX|AMDY|CD[1-2]_[1-2]|CDELT[1-4]|CNPIX[1-4]|CO1_[1-9][0-9]|CO2_[1-9][0-9]|CROTA[1-4]|CRPIX[1-4]|CRVAL[1-4]|CTYPE[1-4]|CUNIT[1-4]|DATE|DATE_OBS|DC-FLAG|DEC|DETSEC|DETSIZE|EPOCH|EQUINOX|EQUINOX[a-z]|IMAGEH|IMAGEW|LATPOLE|LONGPOLE|MJD-OBS|PC00[1-4]00[1-4]|PC[1-4]_[1-4]|PIXSCALE|PIXSCAL[1-2]|PLTDECH|PLTDECM|PLTDECS|PLTDECSN|PLTRAH|PLTRAM|PLTRAS|PPO|PROJP[1-9]|PROJR0|PV[1-3]_[1-3]|PV[1-4]_[1-4]|RA|RADECSYS|SECPIX|SECPIX|SECPIX[1-2]|UT|UTMID|VELOCITY|VSOURCE|WCSAXES|WCSDEP|WCSDIM|WCSNAME|XPIXSIZE|YPIXSIZE|ZSOURCE|LTM|LTV/;
...
k&&"ecliptic"!==k)&&(e.dval*=15),c=a.wcs2pix(this.raw.wcs,e.dval,c.dval).split(/ +/),e=parseFloat(c[0]),c=parseFloat(c[1])):"physical"===k?(c=this.logicalToImagePos({x:e.dval,y:c.dval}),e=c.x,c=c.y):(e=e.dval,c=c.dval);return[e,c]},y=function(a,b){var d=u(a),c=this.raw.wcsinfo||{cdelt1:1,cdelt2:1};if((m||n)&&d.dtype&&"."!==d.dtype)d.dval=Math.abs(d.dval/c["cdelt"+b]);return d.dval},x=function(a){a=u(a);var b=this.raw.wcsinfo||{crot:0};if((m||n)&&b.crot)a.dval+=b.crot;return a.dval};d=d.trim();if(!d.match(/(\(|\{|#|;|\n)/))return d;
j=this.getWCSSys();k="physical";this.setWCSSys(k);m="image"!==k&&"physical"!==k;f=d.split(/\n|;/);for(d=0;d<f.length;d++)if("#"!==f[d].trim().substr(0,1)){n=!1;g=f[d];h=void 0;c={opts:{},args:[],isregion:0};c.cmd=0<=g.indexOf("(")?g.split("(")[0].trim().toLowerCase():0<=g.indexOf("{")?g.split("{")[0].trim().toLowerCase():0<=g.indexOf("#")?g.split("#")[0].trim().toLowerCase():g.trim().toLowerCase();c.cmd&&(c.isregion=0<=c.cmd.search(p));c.comment=g.split("#")[1];c.comment&&(c.comment=c.comment.trim().toLowerCase());
if((h=t.exec(g))&&h[1])try{c.opts=JSON.parse(h[1].trim())}catch(A){c.opts={}}if((h=s.exec(g))&&h[1])c.args=h[1].split(v);c.isregion&&0===c.cmd.indexOf("-")&&(c.cmd=c.cmd.slice(1),c.comment=c.comment?c.comment+",exclude":"exclude");h=c;l=h.args.length;if(h.isregion){g=$.extend(!0,{},h.opts);g.shape=h.cmd;2<=l&&(q=w.call(this,h.args[0],h.args[1]),g.x=q[0],g.y=q[1]);switch(h.cmd){case "annulus":g.radii=[];for(c=2;c<l;c++)g.radii.push(y.call(this,h.args[c],1));break;case "box":3<=l&&(g.width=y.call(this,
h.args[2],1));4<=l&&(g.height=y.call(this,h.args[3],2));5<=l&&(g.angle=x.call(this,h.args[4]));break;case "circle":3<=l&&(g.radius=y.call(this,h.args[2],1));break;case "ellipse":3<=l&&(g.r1=y.call(this,h.args[2],1));4<=l&&(g.r2=y.call(this,h.args[3],2));5<=l&&(g.angle=x.call(this,h.args[4]));break;case "line":case "polygon":g.pts=[];for(e=c=0;c<l;c+=2,e++)q=w.call(this,h.args[c],h.args[c+1]),g.pts[e]={x:q[0],y:q[1]};delete g.x;delete g.y;break;case "text":3<=l&&(c=g,e=h.args[2].replace(/^['"]/,"").replace(/["']$/,
""),c.text=e)}h.comment&&(g.tags=h.comment);b.push(g)}else h.cmd.match(r)?(this.setWCSSys(h.cmd),k=this.getWCSSys(),m="image"!==k&&"physical"!==k):("remove"===h.cmd||"delete"===h.cmd)&&b.push({remove:!0})}this.setWCSSys(j);return b};a.Regions.saveRegions=function(d,b){var c=sprintf("# Region file format: JS9 version 1.0"),e=this.listRegions(b,1),c=sprintf("%s\n%s\n",c,e.replace(/; */g,"\n")),c=new Blob([c],{type:"text/plain;charset=utf-8"});d=d||"js9.reg";window.hasOwnProperty("saveAs")?saveAs(c,
...
a.spinner&&a.spinner.spinner.stop():$("body").removeClass("waiting")}};a.msgHandler=function(d,b){var c,e,f,g=[],h=d.cmd,j=d.id;b&&(a.globalOpts.alerts=!1);if(a.publics[h]){$.isArray(d.args)||(d.args=[d.args]);g=$.extend(!0,[],d.args);j&&g.push({display:j});"RunAnalysis"===h&&b&&(1===g.length&&g.push(null),g.push(b),b=null);try{f=a.publics[h].apply(null,g)}catch(k){f=sprintf("ERROR: %s",k.message)}b&&(a.globalOpts.alerts=!0,b(f));return f}if(!h||"#"===h)b&&b(""),b&&(a.globalOpts.alerts=!0);else{c=
a.lookupCommand(h);e=a.lookupDisplay(j);if(c&&e)switch(c.getDisplayInfo(e),d.args?g=$.extend(!0,[],d.args):d.paramlist&&(g=d.paramlist.split(/ +/)),c.getWhich(g)){case "get":try{f=c.get(g)||""}catch(m){f="ERROR: "+m.message}break;case "set":try{f=c.set(g)||"OK"}catch(n){f="ERROR: "+n.message}break;default:f=sprintf("ERROR: unknown cmd type for '%s'",h)}else c||(f=sprintf("ERROR: unknown cmd '%s'",h)),e||(f=sprintf("ERROR: unknown display (%s)",j));b&&(a.globalOpts.alerts=!0,b(f));return f}};a.lightWin=
...
e.onload=function(a){var e=new Image,h,j,k;e.src=a.target.result;e.onload=function(){var a,f,q,l=0;a=document.createElement("canvas");f=a.getContext("2d");var p=e.height,r=e.width;a.width=r;a.height=p;f.drawImage(e,0,0);h=f.getImageData(0,0,r,p).data;j=new Float32Array(p*r);for(f=0;f<p;f++)for(a=0;a<r;a++)q=0.299*h[l]+0.587*h[l+1]+0.114*h[l+2],j[(p-f)*r+a]=q,l+=4;k={head:{},name:d.name,filedata:j,naxis:2,axis:[0,r,p],bitpix:-32,data:j};k.dmin=Number.MAX_VALUE;k.dmax=Number.MIN_VALUE;for(l=0;l<p*r;l++)k.dmin=
Math.min(k.dmin,k.data[l]),k.dmax=Math.max(k.dmax,k.data[l]);c(k,b)}};e.readAsDataURL(d)};a.lookupColormap=function(d){var b;d||(d=a.imageOpts.colormap);if(d)for(b=0;b<a.colormaps.length;b++)if(a.colormaps[b].name===d)return a.colormaps[b];a.error("unknown colormap '"+d+"'")};a.lookupCommand=function(d){var b,c;if(d){c=d.toLowerCase();for(b=0;b<a.commands.length;b++)if(d=a.commands[b],d.name===c||d.alias===c||d.alias2===c)return d}return null};a.error=function(d,b,c){var e,f,g="",h="",j=!0;a.waiting(!1);

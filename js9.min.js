...
this,h,g=[],k=function(a,b){0>$.inArray(a,g)&&(e.call(f,a,b),g.push(a))};var l=function(b){a.tmp.regSelect={layer:c,im:f,all:[]};m.getObjects().forEach(function(b){b.params&&a.tmp.regSelect.all.push(b.params.id)});try{regSelect.parse(b)}catch(t){a.error("parsing selection filter: "+b,t)}b=a.tmp.regSelect.ids;delete a.tmp.regSelect;return b};if(!this.layers||!c||!this.layers[c])return null;"function"!==typeof e&&a.error("selectShapes requires a callback");d=d||{};var m=this.layers[c].canvas;b||(b=
m.getActiveObject()&&"auto"===a.globalOpts.regWhichDefault?"selected":"all");if("string"===typeof b&&b.startsWith("["))try{b=JSON.parse(b)}catch(u){a.error("can't parse array selection")}if("string"===typeof b){if(this.regSelect){if("and"===d.prev)b="(previous) && ("+b+")";else if("or"===d.prev||"add"===d.prev)b="(previous) || ("+b+")";b=b.replace(/previous/gi,this.regSelect)}this.regSelect=b;b.match(/&|\||!/)&&(b=l(b))}$.isArray(b)||(b=[b]);for(d=0;d<b.length;d++){l=m.getObjects();var n=l.length;

...
0,e);h+=k+"<img src='"+g+"'></div>";g=$jscomp.makeIterator(Object.keys(this.layers));for(b=g.next();!b.done;b=g.next())b=b.value,b=this.layers[b],"main"===b.dlayer.dtype&&b.show&&(h+=""+k+b.dlayer.canvas.toSVG()+"</div>");(void 0===c.colorbar||c.colorbar)&&(c=this.display.pluginInstances.JS9Colorbar)&&c.isActive()&&(e+=2,g=c.colorbarjq[0].toDataURL("image/png"),e+=this.display.height,k=sprintf("<div style='position:absolute; left:%spx; top:%spx'>",0,e),h+=k+"<img src='"+g+"'></div>",c.textjq&&c.textjq[0]&&
(g=c.textjq[0].toDataURL("image/png"),e+=c.colorbarjq.height()+1,k=sprintf("<div style='position:absolute; left:%spx; top:%spx'>",0,e),h+=k+"<img style='width:"+this.display.width+"px;'src='"+g+"'></div>"));h+="</body></html>";window.electron&&(d="data:text/html,"+h);(k=window.open(d,this.id,f))?k.document?(k.document.open(),k.document.write(h),k.document.close()):a.error("no method available for drawing image into print window"):a.error("could not create print window (check your pop-up blockers)")};

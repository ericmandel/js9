...
d,e)}});return this};a.Image.prototype.starbaseToShapes=function(d,b){var c,e,f,g,h,j,k,m,n;m=a.globalOpts.catalogs.ras;e=a.globalOpts.catalogs.decs;var q=[];f=function(a,d,b,c){var e,f;if(void 0!==c)f=c;else{f=-1;for(e=0;e<b.length;e++){for(c=0;c<d.length;c++)if(b[e].toLowerCase()===d[c].toLowerCase()){f=a[d[c]];break}if(0<=f)break}}return f};if(d&&d.data&&d.headline){j=d.data;c=d.headline;b=b||{};m=f(d,c,m,b.xcol);0>m&&a.error("xcol not specified");n=f(d,c,e,b.ycol);0>n&&a.error("ycol not specified");
f=b.shape||"circle";switch(f){case "box":k=function(){return{width:b.width||7,height:b.height||7}};break;case "circle":k=function(){return{radius:b.radius||3.5}};break;case "ellipse":k=function(){return{width:b.width||7,height:b.height||7}};break;default:k=function(){return{width:b.width||7,height:b.height||7}}}for(e=c=0;c<j.length;c++)g=void 0,g=(g=a.wcs2pix(this.raw.wcs,15*j[c][m],j[c][n]).trim().split(/ +/))&&g.length?{x:parseFloat(g[0]),y:parseFloat(g[1])}:null,g&&(h=k.call(this,j[c][1],j[c][1]),
g={id:c.toString(),shape:f,x:g.x,y:g.y,width:h.width,height:h.height,radius:h.radius,angle:0,data:{ra:15*j[c][m],dec:j[c][n]}},b.color&&(g.color=b.color),q[e++]=g);return q}};a.Image.prototype.loadCatalog=function(d,b,c){var e=$.extend(!0,{},a.Catalogs.opts);a.globalOpts.catalogs.tooltip&&(e.tooltip=a.globalOpts.catalogs.tooltip);c=c||{};c.color||(c.color=a.globalOpts.catalogs.color||"#00FF00");b=new a.Starbase(b,{convFuncs:{def:a.saostrtod},units:c.units||a.globalOpts.catalogs.units,skip:c.skip||
a.globalOpts.catalogs.skip});(!b||!b.data||!b.data.length)&&a.error("no objects found in catalog table");b=this.starbaseToShapes(b,c);b.length?(d=d||"catalog_"+a.uniqueID(),this.display.newShapeLayer(d,e),this.removeShapes(d),this.addShapes(d,b,c)):a.error("no catalog objects found");return this};a.Command=function(d){for(var b in d)d.hasOwnProperty(b)&&(this[b]=d[b]);d.name||a.error("command has no name");!d.get&&!d.set&&a.error("command requires get and/or set routine");a.commands.push(this);1<
a.DEBUG&&a.log("JS9 command:  %s",this.name)};a.Command.prototype.getDisplayInfo=function(a){a&&a.id&&(this.display=a,this.image=a.image);return this};a.Command.prototype.getWhich=function(a){return this.get&&!this.set?"get":this.set&&!this.get?"set":this.which?this.which(a):0===a.length?"get":"set"};a.Helper=function(){this.helper=this.connected=!1;this.type=a.globalOpts.helperType||"sock.io";this.pageid=null;this.connect()};a.Helper.prototype.connectinfo=function(){var d;return null===a.helper.connected?
"notConfigured":a.helper.connected?(d=sprintf("connected %s %s",a.helper.type,a.helper.url),a.helper.pageid&&(d+="<p>"+a.helper.pageid),d):sprintf("notConnected %s",a.helper.type)};a.Helper.prototype.connect=function(d){var b=this;d&&(this.type=d);if(this.socket){try{this.socket.disconnect()}catch(c){a.log("warning: can't disconnect from socket")}this.socket=null}this.url=a.globalOpts.helperURL?0<=a.globalOpts.helperURL.search(/:\/\//)?a.globalOpts.helperURL:a.globalOpts.helperProtocol+a.globalOpts.helperURL:
document.domain?a.globalOpts.helperProtocol+document.domain:a.globalOpts.helperProtocol+"localhost";switch(this.type){case "none":this.connected=null;$(document).trigger("JS9:ready",{type:"none",status:"OK"});a.Preload(!0);break;case "get":case "post":a.globalOpts.helperCGI||a.error("cgi script name missing for helper");this.url+="/"+a.globalOpts.helperCGI;this.helper=this.connected=!0;a.DEBUG&&a.log("JS9 helper: connect: "+this.type);$(document).trigger("JS9:ready",{type:"get",status:"OK"});a.Preload(!0);
break;case "sock.io":case "nodejs":a.globalOpts.helperPort||a.error("port missing for helper");this.url+=":"+a.globalOpts.helperPort;this.sockurl=this.url+"/socket.io/socket.io.js";$.ajax({url:this.sockurl,dataType:"script",timeout:a.globalOpts.htimeout,success:function(){var d,c;b.socket=io.connect(b.url,{reconnection:!0,reconnectionDelay:1E4,reconnectionDelayMax:1E4,reconnectionAttempts:6,timeout:a.globalOpts.htimeout});b.socket.on("connect",function(){b.connected=!0;b.helper=!0;c=[];for(d=0;d<
a.displays.length;d++)c.push(a.displays[d].id);b.socket.emit("displays",{displays:c},function(a){b.pageid=a});$(document).trigger("JS9:ready",{type:"socket.io",status:"OK"});a.Preload(!0);a.DEBUG&&a.log("JS9 helper: connect: "+b.type)});b.socket.on("connect_error",function(){b.connected=!1;b.helper=!1;a.Preload(!0);1<a.DEBUG&&a.log("JS9 helper: connect error")});b.socket.on("connect_timeout",function(){b.connected=!1;b.helper=!1;a.Preload(!0);1<a.DEBUG&&a.log("JS9 helper: connect timeout")});b.socket.on("disconnect",
function(){b.connected=!1;b.helper=!1;1<a.DEBUG&&a.log("JS9 helper: disconnect")});b.socket.on("reconnect",function(){b.connected=!0;b.helper=!0;1<a.DEBUG&&a.log("JS9 helper: reconnect")});b.socket.on("msg",a.msgHandler)},error:function(d,c,g){b.connected=!1;b.helper=!1;$(document).trigger("JS9:ready",{type:"socket.io",status:"error"});a.Preload(!0);a.DEBUG&&a.log("JS9 helper: connect failure: "+c+" "+g)}});break;default:a.error("unknown helper type: "+this.type)}};a.Helper.prototype.send=function(d,
b,c){if(!this.connected)return null;b&&"object"===typeof b?(b.cookie=document.cookie,a.globalOpts.dataPath&&!b.dataPath&&(b.dataPath=a.globalOpts.dataPath+":.")):b={dataPath:"."};a.TOROOT&&(b.dataPath+=":"+a.TOROOT);switch(this.type){case "get":case "post":b.key=d;a.DEBUG&&a.log("JS9 cgi helper [%s, %s]: %s",this.type,JSON.stringify(b),this.url);$.ajax({url:this.url,type:this.type.toUpperCase(),data:b,dataType:"text",success:function(d){"string"===typeof d&&0<=d.search(a.analOpts.epattern)&&a.log(d);

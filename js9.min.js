...
d,k)}if("regions"===c&&("text"===d.format||"csv"===d.format||"regions"===d.format)){d.mode=d.mode||1;b=this.listRegions(b,d);if("csv"===d.format){var h=b.split(";");c=0;for(b="";c<h.length;c++)if(h[c])if(h[c].toLowerCase().match(a.WCSEXP))d.includewcs&&(b+=h[c].trim()+"\n");else{var g=h[c].replace(/\(/,",").replace(/\).*/,"").trim();b+=g+"\n"}}else"regions"===d.format&&(b=b.replace(/ *; */g,"\n"));return b}this._selectShapes(c,b,function(a){e=a.pub||{};d.includeObj&&(e.obj=a);f.push(e)});return f};
a.Fabric.changeShapes=function(c,b,d){var e=this,f,h,g,k,l,m,n,p,q,r,v,w=[],t=[];if((l=this.getShapeLayer(c))&&d){if("string"===typeof d)try{d=JSON.parse(d)}catch(x){a.error("can't parse shape opts: "+d,x)}if(this.display.image!==this)this.delayedShapes=this.delayedShapes||[],this.delayedShapes.push({layer:c,shape:b,mode:"change",opts:d});else{var u=l.canvas;var y=u.getActiveObject();this._selectShapes(c,b,function(b,B){p="main"!==e.display.layers[c].dtype||b.params.preservedcoords?1:e.rgb.sect.zoom;
d.radii&&(b.params.radii=[]);d.tags&&(b.params.tags=[]);k=$.extend(!0,{},b.params,d);g=e._parseShapeOptions(c,k,b);if(g.remove){if(!0===g.remove||"true"===g.remove)g.remove="all";if(!1!==g.remove&&"false"!==g.remove){e.removeShapes(c,g.remove||"all");return}}if(g.opts.send)switch(g.opts.send){case "front":u.sendToFront(y);u.discardActiveObject();break;case "back":u.sendToBack(y),u.discardActiveObject()}q=e._exportShapeOptions(d).filter(function(a){return!b.params.exports.hasOwnProperty(a)});g.params.exports=
...
k.x:l.x;var q=k.y>l.y?k.y:l.y;var r=k.x-l.x;k=k.y-l.y;var v=Math.sqrt(r*r+k*k);0!==v&&(r/=v,k/=v);v=d-l.x;var w=g-l.y;v=v*r+w*k;r=l.x+r*v;l=l.y+k*v;r<m?r=m:r>p&&(r=p);l<n?l=n:l>q&&(l=q);m=(r-d)*(r-d)+(l-g)*(l-g);if(m<e){e=m;var t=r;var u=l;var y=h===f.length?0:h}}c=this.getShapeLayer(c);a.Fabric.removePolygonAnchors(c.dlayer,b);f.splice(y+1,0,{x:t,y:u});if(1===fabric.major_version)c.canvas.setActiveObject(b);else{switch(b.type){case "polyline":case "polygon":a.Fabric.addPolygonAnchors(c.dlayer,b),

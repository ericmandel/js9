...
y:parseFloat(g[1])}:null,g&&(h=k.call(this,j[c][1],j[c][1]),g={id:c.toString(),shape:f,x:g.x,y:g.y,width:h.width,height:h.height,radius:h.radius,angle:0,data:{ra:15*j[c][m],dec:j[c][n]}},b.color&&(g.color=b.color),l[e++]=g);this.setWCSSys(q);return l}};a.Image.prototype.loadCatalog=function(d,b,c){var e=$.extend(!0,{},a.Catalogs.opts);a.globalOpts.catalogs.tooltip&&(e.tooltip=a.globalOpts.catalogs.tooltip);c=c||{};c.color=c.color||a.globalOpts.catalogs.color||"#00FF00";c.wcssys=c.wcssys||a.globalOpts.catalogs.wcssys;
b=new a.Starbase(b,{convFuncs:{def:a.saostrtod},units:c.units||a.globalOpts.catalogs.units,skip:c.skip||a.globalOpts.catalogs.skip});(!b||!b.data||!b.data.length)&&a.error("no objects found in catalog table");b=this.starbaseToShapes(b,c);b.length?(d=d||"catalog_"+a.uniqueID(),this.display.newShapeLayer(d,e),this.removeShapes(d),this.addShapes(d,b,c)):a.error("no catalog objects found");return this};a.Command=function(d){for(var b in d)d.hasOwnProperty(b)&&(this[b]=d[b]);d.name||a.error("command has no name");
...
function(d){if(f.display.image&&d.target)"main"===f.dtype&&(f.display.image.clickInRegion=!0),f.opts.onmousedown.call(this,f.display.image,d.target.pub,d.e,d.target);else if(this._selection=this.selection)this.selection=a.specialKey(d.e)});else f.canvas.on("mouse:down",function(d){if(this._selection=this.selection)this.selection=a.specialKey(d.e)});if(f.opts.onmouseup)f.canvas.on("mouse:up",function(a){f.display.image&&a.target&&f.opts.onmouseup.call(this,f.display.image,a.target.pub,a.e,a.target);
...
function(a){f.display.image&&a.target&&f.opts.onmouseout.call(this,f.display.image,a.target.pub,a.e,a.target)});f.opts.tooltip&&(f.canvas.on("mouse:over",function(d){f.display.image&&d.target&&a.tooltip(d.target.left,d.target.top,f.opts.tooltip,f.display.image,d.target.pub,d.e,d.target)}),f.canvas.on("mouse:out",function(d){f.display.image&&d.target&&a.tooltip(d.target.left,d.target.top,null,f.display.image,d.target.pub,d.e,d.target)}))}else f.canvas.on("mouse:down",function(d){if(this._selection=
this.selection)this.selection=a.specialKey(d.e)}),f.canvas.on("mouse:up",function(){this.selection=this._selection||this.selection});if(f.opts.ongroupcreate&&(f.opts.canvas.selection=!0,f.opts.selectable=!0,f.opts.ongroupcreate))f.canvas.on("selection:created",function(a){var d=[],b=[];f.display.image&&"group"===a.target.type&&(a.target.forEachObject(function(a){a.pub&&(b.push(a),d.push(a.pub))}),f.opts.ongroupcreate.call(this,f.display.image,d,a.e,b))});f.canvas.on("object:scaling",function(a){a.target.rescaleEvenly();
a.target.rescaleBorder()});f.canvas.on("object:selected",function(b){f.params.sel&&(b.target.params&&f.params.sel!==b.target)&&f.canvas.fire("before:selection:cleared",{target:f.params.sel});switch(b.target.type){case "polyline":case "polygon":a.Fabric.addPolygonAnchors(f,b.target),f.canvas.renderAll()}b.target.polyparams?f.params.sel=b.target.polyparams.polygon:b.target.params&&(f.params.sel=b.target);f.display.image&&(f.display.image.layer=d)});f.canvas.on("before:selection:cleared",function(d){f.params.sel=
null;f.display.image&&(f.display.image.layer=null);switch(d.target.type){case "polyline":case "polygon":a.Fabric.removePolygonAnchors(f,d.target),f.canvas.renderAll()}});if(f.divjq.closest(a.lightOpts[a.LIGHTWIN].drag).length)if(fabric.isTouchSupported)f.divjq.on("touchstart",function(){f.canvas.calcOffset()});else f.divjq.on("mouseenter",function(){f.canvas.calcOffset()});return f}};a.Fabric.showShapeLayer=function(d,b){var c=this,e=0,f,g,h,j;if(g=this.getShapeLayer(d)){j=g.canvas;h=this.display.layers[d];
...
"onshapelayershow",d):this.xeqPlugins("shape","onshapelayerhide",d);return this}};a.Fabric.displayShapeLayers=function(){var a;if(this!==this.display.image){if(this.display.image&&this.display.image.layers)for(a in this.display.image.layers)this.display.image.layers.hasOwnProperty(a)&&this.display.image.showShapeLayer(a,!1);if(this.layers)for(a in this.layers)this.layers.hasOwnProperty(a)&&this.showShapeLayer(a,!0)}};a.Fabric.getShapeLayer=function(a){var b,c;if(!a)return null;c=this.layers[a];if(!c){b=
this.display.layers[a];if(!b)return null;this.layers[a]={};c=this.layers[a];c.show=!0;c.nshape=0;c.dlayer=b;c.opts=c.dlayer.opts;c.canvas=c.dlayer.canvas;c.canvas.calcOffset()}return c};a.Fabric.activeShapeLayer=function(a){var b,c,e,f;if(a)if($.isArray(a)){b=0;for(c=this.zlayer-1;b<a.length;b++)f=this.layers[a[b]],"main"===f.dlayer.dtype&&(f.zindex=c--,f.show?e||(e=a[b]):f.zindex=-f.zindex,f.dlayer.divjq.css("z-index",f.zindex));a=e}else{f=this.layers[a];if(f.show){e=f.zindex;f.zindex=this.zlayer-
1;f.dlayer.divjq.css("z-index",f.zindex);for(b in this.layers)this.layers.hasOwnProperty(b)&&(c=this.layers[b],c!==f&&(c.zindex===f.zindex&&"main"===c.dlayer.dtype)&&(c.zindex=e,c.dlayer.divjq.css("z-index",c.zindex)));this.xeqPlugins("shape","onshapelayeractive",a)}a=this}else{for(b in this.layers)if(this.layers.hasOwnProperty(b)&&(c=this.layers[b],"main"===c.dlayer.dtype&&(void 0===f||c.zindex>f)))f=c.zindex,e=b;a=e}return a};a.Fabric._parseShapeOptions=function(d,b,c){var e,f,g,h,j,k,m,n={},q=
{};h="main"===this.display.layers[d].dtype?this.rgb.sect.zoom:1;j=h/("main"===this.display.layers[d].dtype?this.binning.bin||1:1);if(b.remove)return{remove:b.remove};q.tags=[];if(b.tags)if("string"===typeof b.tags){f=b.tags.toLowerCase().split(",");for(d=0;d<f.length;d++)q.tags[d]=f[d].trim()}else if($.isArray(b.tags))for(d=0;d<b.tags.length;d++)q.tags[d]=b.tags[d].trim();void 0!==b.angle&&(n.angle=-b.angle);void 0!==b.x&&void 0!==b.y&&(e=this.imageToDisplayPos(b),n.left=e.x,n.top=e.y);void 0!==b.left&&
(n.left=b.left);void 0!==b.top&&(n.top=b.top);void 0===n.left&&(n.left=c&&void 0!==c.left?c.left:this.display.canvasjq.attr("width")/2-1);void 0===n.top&&(n.top=c&&void 0!==c.top?c.top:this.display.canvasjq.attr("height")/2-1+1);b.dx&&(n.left+=b.dx);b.dy&&(n.top-=b.dy);switch(b.shape){case "annulus":q.radii=[];if(void 0!==b.radii)if("string"===typeof b.radii){q.radii=b.radii.replace(/ /g,"").split(",");for(e=d=0;d<q.radii.length;d++)""!==q.radii[d]&&(q.radii[e++]=parseInt(q.radii[d],10))}else q.radii=
b.radii;else{b.ireg&&a.SCALEIREG&&(b.iradius&&(b.iradius/=j),b.oradius&&(b.oradius/=j));h=(b.oradius-b.iradius)/b.nannuli;j=b.nannuli+1;for(d=0;d<j;d++)f=b.iradius+h*d,q.radii.push(f)}break;case "box":b.ireg&&a.SCALEIREG&&(b.width&&(b.width/=j),b.height&&(b.height/=j));break;case "circle":b.ireg&&a.SCALEIREG&&b.radius&&(b.radius/=j);break;case "ellipse":b.ireg&&a.SCALEIREG&&(b.r1&&(b.r1/=j),b.r2&&(b.r2/=j));break;case "point":switch(b.ptshape){case "box":b.width=2*b.ptsize;b.height=2*b.ptsize;break;
case "circle":b.radius=b.ptsize;break;case "ellipse":b.rx=b.ptsize,b.ry=b.ptsize/2}b.lockRotation=!0;b.lockScalingX=!0;b.lockScalingY=!0;b.lockUniScaling=!0;b.hasControls=!1;b.hasRotatingPoint=!1;b.hasBorders=!0;break;case "line":case "polygon":if(b.pts&&b.pts.length){if("string"===typeof b.pts){g=b.pts.replace(/ /g,"").split(",");f=g.length;if("string"===typeof g[0])for(d=0;d<f;d++)g[d]=parseFloat(g[d]);b.pts=[];for(e=d=0;d<f;d+=2,e++)b.pts[e]={x:g[d],y:g[d+1]}}f=b.pts.length;for(d=0;d<f;d++)b.pts[d]=
this.imageToDisplayPos(b.pts[d]);b.left&&b.top?g={x:b.left,y:b.top}:(g=a.centerPolygon(b.pts),n.left=g.x,n.top=g.y);b.points=[];for(d=0;d<f;d++)e={x:(b.pts[d].x-g.x)/h,y:(b.pts[d].y-g.y)/h},b.points.push(e)}else"polygon"===b.shape&&b.polypoints?b.points=b.polypoints:"line"===b.shape&&b.linepoints&&(b.points=b.linepoints);if(b.ireg&&a.SCALEIREG){f=b.points.length;for(d=0;d<f;d++)b.points[d].x/=j,b.points[d].y/=j}}for(k in b)if(b.hasOwnProperty(k))switch(k){case "tags":case "x":case "y":case "dx":case "dy":case "pts":case "left":case "top":case "angle":case "radii":case "ireg":break;

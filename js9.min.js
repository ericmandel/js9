...
alert("When using the file:// URI, Chrome must be run with the --allow-file-access-from-files switch to permit JS9 to access data."):alert("could not read off-screen image data [same-origin policy violation?]")}return this};a.Image.prototype.initLCS=function(a){var b=[[0,0,0],[0,0,0],[0,0,0]],d=function(a){var c,b,d=0,j=0,k=[[0,0,0],[0,0,0],[0,0,0]],m=a[0][0]*a[1][1];for(c=0;3>c;c++)for(b=0;2>b;b++)if(void 0===a[c][b]||isNaN(a[c][b]))return null;0<=m?d+=m:j+=m;m=-a[0][1]*a[1][0];0<=m?d+=m:j+=m;c=
...
a.Catalogs.opts={hasControls:!1,hasRotatingPoint:!1,hasBorders:!1,evented:!1,lockMovementX:!0,lockMovementY:!0,lockRotation:!0,lockScalingX:!0,lockScalingY:!0,lockUniScaling:!0,selectable:!1,canvas:{selection:!1},panzoom:!0,shape:"circle",strokeWidth:2,width:10,height:10,radius:5,eradius:{x:5,y:3},angle:0,tagcolors:{defcolor:"#00FF00"}};"function"!==typeof Object.create&&(Object.create=function(a){var b=function(){};b.prototype=a;return new b});a.jupyterFocus=function(a,b){var d;window.hasOwnProperty("Jupyter")&&
(d=a instanceof jQuery?a:$(a),d.find(b||"input, textarea").each(function(){Jupyter.keyboard_manager.register_events($(this))}))};a.getImageID=function(c,b){var d,e,f=0,g=1,h=a.images.length,j=/.*<([0-9][0-9]*)>$/;for(d=0;d<h;d++)e=a.images[d],e.display.id===b&&c===e.oid&&(0<=e.id.search(j)&&(e=e.id.replace(j,"$1"),g=Math.max(g,parseInt(e,10))),f++);return f?c+"<"+String(g+1)+">":c};var H=1;a.uniqueID=function(){return H++};a.waiting=function(c,b){var d;switch(c){case !0:window.hasOwnProperty("Spinner")&&
"spinner"===a.globalOpts.waitType?(b=b||$("body").get(0),a.spinner||(a.spinner={},d={color:a.globalOpts.spinColor,opacity:a.globalOpts.spinOpacity},a.spinner.spinner=new Spinner(d)),a.spinner.spinner.spin(b)):$("body").addClass("waiting");break;case !1:window.hasOwnProperty("Spinner")&&"spinner"===a.globalOpts.waitType?a.spinner&&a.spinner.spinner.stop():$("body").removeClass("waiting")}};a.msgHandler=function(c,b){var d,e,f,g=[],h=c.cmd,j=c.id;b&&(a.globalOpts.alerts=!1);if(a.publics[h]){$.isArray(c.args)||
(c.args=[c.args]);g=$.extend(!0,[],c.args);j&&g.push({display:j});try{f=a.publics[h].apply(null,g)}catch(k){f=sprintf("ERROR: %s",k.message)}b&&(a.globalOpts.alerts=!0,b(f));return f}if(!h||"#"===h)b&&b(""),b&&(a.globalOpts.alerts=!0);else{d=a.lookupCommand(h);e=a.lookupDisplay(j);if(d&&e)switch(d.getDisplayInfo(e),c.args?g=$.extend(!0,[],c.args):c.paramlist&&(g=c.paramlist.split(/ +/)),d.getWhich(g)){case "get":try{f=d.get(g)||""}catch(m){f="ERROR: "+m.message}break;case "set":try{f=d.set(g)||"OK"}catch(n){f=
"ERROR: "+n.message}break;default:f=sprintf("ERROR: unknown cmd type for '%s'",h)}else d||(f=sprintf("ERROR: unknown cmd '%s'",h)),e||(f=sprintf("ERROR: unknown display (%s)",j));b&&(a.globalOpts.alerts=!0,b(f));return f}};a.lightWin=function(c,b,d,e,f){var g;switch(a.LIGHTWIN){case "dhtml":g=dhtmlwindow.open(c,b,d,e,f),/iPad|iPhone|iPod/.test(navigator.platform)&&$("#"+c+" "+a.lightOpts.dhtml.drag).css("-webkit-overflow-scrolling","touch").css("overflow-y","scroll"),$("#"+c+" ."+a.lightOpts.dhtml.dragBar).doubletap(function(){g.close()},
...
m=0;a=document.createElement("canvas");f=a.getContext("2d");var n=e.height,p=e.width;a.width=p;a.height=n;f.drawImage(e,0,0);var r=f.getImageData(0,0,p,n).data,l=new Float32Array(n*p);for(f=0;f<n;f++)for(a=0;a<p;a++)k=0.299*r[m]+0.587*r[m+1]+0.114*r[m+2],l[(n-f)*p+a]=k,m+=4;a={head:{},name:c.name,filedata:l,naxis:2,axis:[0,p,n],bitpix:-32,data:l};a.dmin=Number.MAX_VALUE;a.dmax=Number.MIN_VALUE;for(m=0;m<n*p;m++)a.dmin=Math.min(a.dmin,a.data[m]),a.dmax=Math.max(a.dmax,a.data[m]);d(a,b)}};e.readAsDataURL(c)};

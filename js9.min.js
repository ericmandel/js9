...
!1,d):b.pub.lcs&&void 0!==b.pub.lcs[c]?m(b.pub.lcs[c],d)?!0:!1:m(b.pub[c],d)||m(b.params[c],d)||m(b[c],d)?!0:!1},p=function(b){return"true"===b?!0:"false"===b?!1:a.isNumber(b)?parseFloat(b):b},q=function(a){var b=String.fromCharCode(13,10);return(a||"").replace(/\\n/g,b)};this.lcs&&this.lcs.physical&&(e=Math.sqrt(Math.pow(this.lcs.physical.forward[0][0],2)+Math.pow(this.lcs.physical.forward[0][1],2)));if(b&&b.pub&&b.pub.wcsconfig&&b.pub.wcsconfig.wcssys)var r=b.pub.wcsconfig.wcssys;else r=this.params.wcssys,
b=f;var v=$(c).data("multi");f=b.pub.layer||"regions";for(c=0;c<h;c++){var u=d[c].name;var t=d[c].value;if("xpos"===u||"ypos"===u)switch(r){case "image":u="i"+u.charAt(0);break;case "physical":u="p"+u.charAt(0);break;default:u=this.raw.wcs&&0<this.raw.wcs?"xpos"===u?"ra":"dec":"xpos"===u?"ix":"iy"}switch(u){case "text":"text"===b.type&&n(b,u,t)&&(g[u]=q(p(t)));break;case "strokeDashes":t=t.trim().split(/\s+/);n(b,u,t)&&(g.strokeDashArray=0===t.length?[]:t.map(function(a){return parseInt(a,10)}));
...
c,"export"),a.globalOpts.xeqPlugins=w;1<t&&this.display.displayMessage("regions",l);return l};a.Regions.copyRegions=function(a,b){return this.copyShapes("regions",a,b)};a.Regions.parseRegions=function(c,b){var d=this,e,f,h=[],g=/^-?(annulus|box|circle|cross|ellipse|line|polygon|point|text)$/,k=/^(fk4|fk5|icrs|galactic|ecliptic|image|physical|linear)$/,l=/^(image|physical)$/,m=/[dr:]/,n=/\(\s*([^)]+?)\s*\)/,p=/(\{.*\})/,q=/\s*,\s*/,r=/#(?![a-zA-Z0-9]{6}['"])/,v=function(a){return"0"===a||"false"===
a.toLowerCase()?!1:!0},u=function(a){for(var b,c,d,e={},f=/([a-zA-Z][a-zA-Z0-9_]*)\s*=\s*(\d+(\s*\d+)*|[^ '"{]+|['"{][^'"}]*['"}])/g,g={color:function(a){return{color:a}},dash:function(a){if(a)return{strokeDashArray:[3,1]}},dashlist:function(a){if(a){var b=a.split(" ");for(a=0;a<b.length;a++)b[a]=parseFloat(b[a]);return{strokeDashArray:b}}},delete:function(a){return{removable:v(a)}},edit:function(a){return{selectable:v(a)}},fixed:function(a){return{zoomable:!v(a)}},font:function(a){var b={};a=a.split(" ");
...
return this.tmp.saveregionsFile=c};a.Regions.unremoveRegions=function(){var a=this.regstack.pop();return a?this.addShapes("regions",a):null};a.Regions.changeRegionTags=function(a,b,d){var c;a=a||"all";b=b||[];d=d||[];$.isArray(b)||(b=b.split(",").map(function(a){return a.trim()}));$.isArray(d)||(d=d.split(",").map(function(a){return a.trim()}));var f=this.getShapes("regions",a);for(a=0;a<f.length;a++){var h=f[a].tags;var g=[];for(c=0;c<b.length;c++)0>$.inArray(b[c],h)&&g.push(b[c]);for(c=0;c<h.length;c++)0>
...
l)}};a.Plot.annotate=function(c,b,d){var e;var f=[];var h=d.data,g=d.annotations.color||a.Plot.opts.annotateColor,k=function(a,b){if(!a.text)return null;var c=a.x||0;if("%Y"===a.y.toUpperCase())for(a=1;a<b.length-1;a++){if(b[a][0]>c){var d=Math.max(b[a-1][1],b[a][1],b[a+1][1]);break}}else d=a.y;return{x:c,y:d}};switch(a.globalOpts.plotLibrary){case "flot":var l=-25;c.find(".plotAnnotation").remove();for(e=0;e<d.annotations.data.length;e++){var m=d.annotations.data[e];var n=k(m,h);f=b.pointOffset({x:n.x,
...
b){Jupyter.keyboard_manager.register_events($(b))}))};a.getImageID=function(c,b,d){var e,f=0,h=1,g=a.images.length,k=/.*<([0-9][0-9]*)>$/,l=/<[0-9][0-9]*>/;c=c.replace(l,"");for(e=0;e<g;e++){var m=a.images[e];m.display.id===b&&m!==d&&c===m.id0.replace(l,"")&&(m.id&&0<=m.id.search(k)&&(m=m.id.replace(k,"$1"),h=Math.max(h,parseInt(m,10))),f++)}return f?c+"<"+String(h+1)+">":c};a.uniqueID=function(){var a=1;return function(){return a++}}();a.waiting=function(c,b){switch(c){case !0:if(window.hasOwnProperty("Spinner")&&

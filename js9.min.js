...
!1,this.context.webkitImageSmoothingEnabled=!1,this.context.msImageSmoothingEnabled=!1);this.tooltip=$("<div>").attr("id","tooltip_"+this.id).addClass("JS9Tooltip").appendTo(this.divjq);this.image=null;this.pluginInstances={};this.layers={};this.initMessages();this.blendMode=!1;this.mouseActions=a.globalOpts.mouseActions.slice(0);this.touchActions=a.globalOpts.touchActions.slice(0);this.mousetouchZoom=a.globalOpts.mousetouchZoom;this.divjq.on("mouseover",this,function(d){return a.mouseOverCB(d)});
this.divjq.on("mousedown touchstart",this,function(d){return a.mouseDownCB(d)});this.divjq.on("mousemove touchmove",this,function(d){return a.mouseMoveCB(d)});this.divjq.on("mouseup touchend",this,function(d){return a.mouseUpCB(d)});this.divjq.on("mouseout",this,function(d){return a.mouseOutCB(d)});this.divjq.on("keypress",this,function(d){return a.keyPressCB(d)});this.divjq.on("keydown",this,function(d){return a.keyDownCB(d)});this.divjq.on("wheel",this,function(d){return a.wheelCB(d)});this.divjq.on("dragenter",
this,function(d){return a.dragenterCB(this.id,d)});this.divjq.on("dragover",this,function(d){return a.dragoverCB(this.id,d)});this.divjq.on("dragexit",this,function(d){return a.dragexitCB(this.id,d)});this.divjq.on("drop",this,function(d){return a.dragdropCB(this.id,d,a.NewFITSImage)});this.divjq.on("contextmenu",this,function(){return!1});this.divjq.append('<div style="visibility:hidden; position:relative; top:-50;left:-50"> <input type="file" id="openLocalFile-'+this.id+'" multiple="true" onchange="javascript:for(var i=0; i<this.files.length; i++){JS9.Load(this.files[i], {display:\''+
...
12:8,hasControls:!0,hasRotatingPoint:!0,hasBorders:!0,transparentCorners:!1,centeredScaling:!0,selectable:!0,padding:0,canvas:{selection:!0,zindex:a.SHAPEZINDEX},fill:"transparent"};a.Fabric.rescaleStrokeWidth=function(d,b){var c=(this.scaleX+this.scaleY)/2;d=d||1;d*=c;!b&&this.params&&(b=this.params.sw1);b&&("group"===this.type?this.forEachObject(function(c){a.Fabric.rescaleStrokeWidth.call(c,d,b)}):this.setStrokeWidth(b/d))};a.Fabric.rescaleEvenly=function(){var a;if(this.params&&this.scaleX!==
this.scaleY)switch(this.params.shape){case "annulus":case "circle":a=this.params.lastscale||1,this.scaleX!==a?this.scaleY=this.scaleX:this.scaleY!==a&&(this.scaleX=this.scaleY),this.params.lastscale=this.scaleX}};fabric.Object.prototype.rescaleBorder=a.Fabric.rescaleStrokeWidth;fabric.Object.prototype.rescaleEvenly=a.Fabric.rescaleEvenly;a.Fabric.newShapeLayer=function(d,b,c){var e,f;if(this&&d){if(this.layers[d])return this.layers[d];this.layers[d]={};f=this.layers[d];f.params=[];f.params.sel=null;
f.params.sellayer=null;c?f.dtype="other":(f.dtype="main",c=this.divjq);e=c.attr("id")+"-"+d.replace(/\s+/,"_")+"-shapeLayer";f.display=this;f.opts=$.extend(!0,{},b);f.opts.canvas=f.opts.canvas||{};void 0===f.opts.canvas.selection&&(f.opts.canvas.selection=!0);f.opts.canvas.zindex=f.opts.canvas.zindex||a.SHAPEZINDEX;f.el=a.Fabric.elements;f.divjq=$("<div>").addClass("JS9Container").css("z-index",0).appendTo(c);f.canvasjq=$("<canvas>").addClass("JS9Layer").attr("id",e).attr("width",c.css("width")).attr("height",
c.css("height")).appendTo(f.divjq);a.bugs.webkit_resize&&"main"===f.dtype&&f.canvasjq.attr("width",this.width).attr("height",this.height);f.canvas=new fabric.Canvas(f.canvasjq[0]);f.canvas.renderOnAddRemove=!1;f.opts.movable&&(f.opts.hasControls=!0,f.opts.hasRotatingPoint=!0,f.opts.hasBorders=!0,f.opts.lockMovementX=!1,f.opts.lockMovementY=!1,f.opts.lockRotation=!1,f.opts.lockScalingX=!1,f.opts.lockScalingY=!1,f.opts.lockUniScaling=!1,f.opts.selectable=!0,f.opts.evented=!0,f.opts.usekeyboard=!0);
f.opts.selectable&&(f.opts.canvas.selection=!0);if(f.opts.onmousedown||f.opts.onmouseup||f.opts.onmousemove||f.opts.tooltip||f.opts.onmouseover||f.opts.onmouseout){f.opts.evented=!0;if(f.opts.onmousedown)f.canvas.on("mouse:down",function(d){if(f.display.image&&d.target)"main"===f.dtype&&(f.display.image.clickInRegion=!0),f.opts.onmousedown.call(this,f.display.image,d.target.pub,d.e,d.target);else if(this._selection=this.selection)this.selection=a.specialKey(d.e)});else f.canvas.on("mouse:down",function(d){if(this._selection=
this.selection)this.selection=a.specialKey(d.e)});if(f.opts.onmouseup)f.canvas.on("mouse:up",function(a){f.display.image&&a.target&&f.opts.onmouseup.call(this,f.display.image,a.target.pub,a.e,a.target);this.selection=this._selection||this.selection});else f.canvas.on("mouse:up",function(){this.selection=this._selection||this.selection});if(f.opts.onmousemove)f.canvas.on("mouse:move",function(a){f.display.image&&a.target&&f.opts.onmousemove.call(this,f.display.image,a.target.pub,a.e,a.target)});if(f.opts.onmouseover)f.canvas.on("mouse:over",
function(a){f.display.image&&a.target&&f.opts.onmouseover.call(this,f.display.image,a.target.pub,a.e,a.target)});if(f.opts.onmouseout)f.canvas.on("mouse:out",function(a){f.display.image&&a.target&&f.opts.onmouseout.call(this,f.display.image,a.target.pub,a.e,a.target)});f.opts.tooltip&&(f.canvas.on("mouse:over",function(d){f.display.image&&d.target&&a.tooltip(d.target.left,d.target.top,f.opts.tooltip,f.display.image,d.target.pub,d.e,d.target)}),f.canvas.on("mouse:out",function(d){f.display.image&&
d.target&&a.tooltip(d.target.left,d.target.top,null,f.display.image,d.target.pub,d.e,d.target)}))}else f.canvas.on("mouse:down",function(d){if(this._selection=this.selection)this.selection=a.specialKey(d.e)}),f.canvas.on("mouse:up",function(){this.selection=this._selection||this.selection});if(f.opts.ongroupcreate&&(f.opts.canvas.selection=!0,f.opts.selectable=!0,f.opts.ongroupcreate))f.canvas.on("selection:created",function(a){var d=[],b=[];f.display.image&&"group"===a.target.type&&(a.target.forEachObject(function(a){a.pub&&
(b.push(a),d.push(a.pub))}),f.opts.ongroupcreate.call(this,f.display.image,d,a.e,b))});f.canvas.on("object:scaling",function(a){a.target.rescaleEvenly();a.target.rescaleBorder()});f.canvas.on("object:selected",function(b){f.params.sel&&(b.target.params&&f.params.sel!==b.target)&&f.canvas.fire("before:selection:cleared",{target:f.params.sel});switch(b.target.type){case "polyline":case "polygon":a.Fabric.addPolygonAnchors(f,b.target),f.canvas.renderAll()}b.target.polyparams?f.params.sel=b.target.polyparams.polygon:
b.target.params&&(f.params.sel=b.target);f.display.image&&(f.display.image.layer=d)});f.canvas.on("before:selection:cleared",function(d){f.params.sel=null;f.display.image&&(f.display.image.layer=null);switch(d.target.type){case "polyline":case "polygon":a.Fabric.removePolygonAnchors(f,d.target),f.canvas.renderAll()}});if(f.divjq.closest(a.lightOpts[a.LIGHTWIN].drag).length)if(fabric.isTouchSupported)f.divjq.on("touchstart",function(){f.canvas.calcOffset()});else f.divjq.on("mouseenter",function(){f.canvas.calcOffset()});
return f}};a.Fabric.showShapeLayer=function(d,b){var c=this,e=0,f,g,h,j;if(g=this.getShapeLayer(d)){j=g.canvas;h=this.display.layers[d];if("show"===b||!0===b){"show"===b&&(g.show=!0);g.json&&g.show&&j.loadFromJSON(g.json,function(){var b,e,f,q;c.resize&&(j.getObjects().forEach(function(a){a.left+=c.resize.left;a.top+=c.resize.top;a.setCoords()}),j.calcOffset());j.renderAll();j.selection=g.opts.canvas.selection;e=h.opts.canvas.zindex;h.divjq.css("z-index",e);for(b in c.layers)if(c.layers.hasOwnProperty(b)&&

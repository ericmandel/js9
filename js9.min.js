...
d.errcode,e);else{if(d.errcode||d.stderr)b=d.stderr?d.stderr:sprintf("ERROR: while executing %s [%s]",e.name,d.errcode),a.error(b,a.analOpts.epattern);switch(e.rtype){case "text":case void 0:g.displayAnalysis("text",d.stdout);break;case "plot":g.displayAnalysis("plot",d.stdout);break;case "alert":d.stdout&&alert(d.stdout);break;case "fits":case "png":f=d.stdout.trim();"/"!==f.charAt(0)&&(f=a.InstallDir(f));a.Load(f,{display:g.display});break;case "none":break;default:a.error("unknown analysis result type: "+

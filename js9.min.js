...
"_"+e;"table"===this.imtab&&(l=this.raw.hdu.table,e=Math.floor(l.cx-(l.nx+1)/2+1),j=Math.floor(l.cx+l.nx/2),q=Math.floor(l.cy-(l.ny+1)/2+1),l=Math.floor(l.cy+l.ny/2),e=sprintf("[EVENTS][bin X=%s:%s,Y=%s:%s]",e,j,q,l),k+=e);window.setTimeout(function(){var d,e,h,j;try{d=m.lastIndexOf("."),0<=d&&(e=m.substring(0,d)+"_area"+m.substring(d)),j=b.cmdswitches||"",p=a.reproject(k,m,g,j),1<a.DEBUG&&a.log("reproject: %s %s %s -> %s",k,m,g,p),a.vunlink(e),a.vunlink(g),0>p.search(/\[struct stat="OK"/)&&(f=!0,

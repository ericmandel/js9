...
corsURL:"params/loadcors.html",proxyURL:"params/loadproxy.html",loadProxy:!1,archivesURL:"help/archives.html",imsectionURL:"params/imsection.html",postMessage:!1,waitType:"spinner",spinColor:"#FF0000",spinOpacity:0.35,resize:!0,resizeHandle:!0,mouseActions:["display value/position","change contrast/bias","pan the image"],touchActions:["display value/position","change contrast/bias","pan the image"],mousetouchZoom:!1,debug:0};a.imageOpts={contrast:1,bias:0.5,invert:!1,exp:1E3,colormap:"grey",scale:"linear",
scaleclipping:"dataminmax",zscalecontrast:0.25,zscalesamples:600,zscaleline:120,wcssys:"native",wcsunits:"sexagesimal",lcs:"physical",valpos:!0,opacity:1,sigma:"none",maskOpacity:0.4,alpha:255,alpha1:100,zoom:1,zooms:5,nancolor:"#000000",wcsalign:!0,listonchange:!1};a.regionOpts={};a.analOpts={epattern:/^(ERROR:[^\n]*)\n/,dpathURL:"params/datapath.html",prependJS9Dir:!0,dataDir:null};a.lightOpts={nclick:0,dhtml:{top:".dhtmlwindow",drag:".drag-contentarea",dragBar:"drag-handle",format:"width=%spx,height=%spx,center=1,resize=%s,scrolling=0",
textWin:"width=830px,height=400px,center=1,resize=1,scrolling=1",plotWin:"width=830px,height=420px,center=1,resize=1,scrolling=1",dpathWin:"width=830px,height=175px,center=1,resize=1,scrolling=1",paramWin:"width=830px,height=230px,center=1,resize=1,scrolling=1",imageWin:"width=512px,height=590px,center=1,resize=1,scrolling=1",lineWin:"width=400px,height=60px,center=1,resize=1,scrolling=1"}};a.textColorOpts={regions:"#00FF00",info:"#00FF00",inimage:"#000000"};a.plotOpts={zoomStack:!0,selection:{mode:"xy"},
series:{clickable:!0,hoverable:!0,lines:{show:!0},points:{show:!1}}};a.helpOpts={user:{type:"help",url:"user.html",title:"JS9 User Manual"},install:{type:"help",url:"install.html",title:"Installing JS9"},webpage:{type:"help",url:"webpage.html",title:"Adding JS9 To A Web Page"},yourdata:{type:"help",url:"yourdata.html",title:"Adding Data To A Web Page"},localtasks:{type:"help",url:"localtasks.html",title:"Local Analysis with JS9"},publicapi:{type:"help",url:"publicapi.html",title:"The JS9 Public API"},
helper:{type:"help",url:"helper.html",title:"Adding Server-side Analysis"},serverside:{type:"help",url:"serverside.html",title:"Server-side Analysis with JS9"},repfile:{type:"help",url:"repfile.html",title:"Dealing with Large Files"},regions:{type:"help",url:"regions.html",title:"JS9 Regions Format"},extmsg:{type:"help",url:"extmsg.html",title:"JS9 External Messaging"},python:{type:"help",url:"python.html",title:"JS9 with Python and Jupyter"},preferences:{type:"help",url:"preferences.html",title:"JS9 Site Preferences"},
...
g.id)}),this.png.image.src=this.rgbFile):(this.displayImage("all",h),k.call(this,d));break;case "string":this.source="fits2png";this.imtab="image";this.file=c;this.id0=this.file.split("/").reverse()[0];this.id=a.getImageID(this.id0,this.display.id);this.status.load="loading";$(this.png.image).on("load",function(){g.mkOffScreenCanvas();g.mkRawDataFromPNG();"zscale"===g.params.scaleclipping&&g.zscale(!0);g.mkSection();e=j(g,h);e.length&&g.mkSection.apply(g,e);g.displayImage("all",h);k.call(g,d);a.DEBUG&&
...
a.log("JS9 fitsFile: %s %s",d.file,d.fitsFile)}b.queried||(b.queryHelper("all"),b.queried=!0)}});return this};a.Image.prototype.queryHelper=function(c){var b=this;c=c||"all";if(a.helper.connected&&("all"===c||"getAnalysis"===c))this.analysisPackages||a.helper.send("getAnalysis",{fits:this.fitsFile},function(d){if(d)try{b.analysisPackages=JSON.parse(d)}catch(c){a.log("can't get analysis",c)}});return this};a.Image.prototype.expandMacro=function(c,b){var d,e,f=this;if(c)return d=c.replace(/\$([a-zA-Z0-9_()]+)/g,
function(d,c){var j,k;j=function(a,c){var d=a.params.wcssys;if(c)switch(c){case "wcs":if("physical"===d||"image"===d)a.params.wcssys=a.params.wcssys0;break;case "physical":case "image":a.params.wcssys=c}return d};var m=function(a,c){var d;"table"===a.imtab&&a.raw.hdu.table.filter?c+="["+a.raw.hdu.table.filter+"]":"image"===a.imtab&&(d=a.file.match(/\[.*\]/))&&(c=c.match(/\[.*\]/)?c.replace(/\[.*\]/,d):c+d);return c},n=c.split("(");n[1]&&(n[1]=n[1].replace(/\)$/,""));switch(n[0]){case "id":k=f.display.divjq.attr("id");
...
{};h.params.scalemin=Number.Nan;h.params.scalemax=Number.Nan;h.png={};h.png.image=new Image;$(h.png.image).on("load",m).on("error",q);h.png.image.src=a.InstallDir(f.url);break;case "regions":f.layer=this.display.newShapeLayer(c,a.Regions.opts);g=a.InstallDir(f.url);$.ajax({url:g,cache:!1,dataType:"json",success:n,error:q});break;case "text":g=a.InstallDir(f.url),$.ajax({url:g,cache:!1,dataType:"text",success:p,error:q})}}};a.Image.prototype.saveFITS=function(c){var b;window.hasOwnProperty("saveAs")?
...
d.rawid=d.rawid||"shift";d.x=c;d.y=b;this.rawDataLayer(d,function(a,c,d){var b,j,k,m,n=a.data.BYTES_PER_ELEMENT;void 0===c.xoff&&(c.xoff=0);void 0===c.yoff&&(c.yoff=0);c.xoff+=d.x;c.yoff+=d.y;if(!d.fill||"clear"===d.fill)if(0<c.bitpix?(m=d.blank||c.header.BLANK||0,c.header.BLANK=m):m=NaN,"function"===typeof c.data.fill)c.data.fill(m);else for(d=0;d<c.data.length;d++)c.data[d]=m;for(d=0;d<a.height;d++)if(k=d+c.yoff,!(0>k||k>=a.height)){b=0;j=b+c.xoff;m=a.width;0>j&&(b-=j,m+=j,j=0);j+m>a.width&&(m-=
j+m-a.width);if(0>=m)return!1;b=(d*a.width+b)*n;b=new Uint8Array(a.data.buffer,b,m*n);j=(k*a.width+j)*n;m=new Uint8Array(c.data.buffer,j,m*n);m.set(b)}return!0})};a.Image.prototype.reprojectData=function(c,b){var d=this,e={},f=!1,g,h,j,k,m,n,p,q,l;l=/NAXIS|NAXIS[1-4]|AMDX|AMDY|CD[1-2]_[1-2]|CDELT[1-4]|CNPIX[1-4]|CO1_[1-9][0-9]|CO2_[1-9][0-9]|CROTA[1-4]|CRPIX[1-4]|CRVAL[1-4]|CTYPE[1-4]|CUNIT[1-4]|DATE|DATE_OBS|DC-FLAG|DEC|DETSEC|DETSIZE|EPOCH|EQUINOX|EQUINOX[a-z]|IMAGEH|IMAGEW|LATPOLE|LONGPOLE|MJD-OBS|PC00[1-4]00[1-4]|PC[1-4]_[1-4]|PIXSCALE|PIXSCAL[1-2]|PLTDECH|PLTDECM|PLTDECS|PLTDECSN|PLTRAH|PLTRAM|PLTRAS|PPO|PROJP[1-9]|PROJR0|PV[1-3]_[1-3]|PV[1-4]_[1-4]|RA|RADECSYS|SECPIX|SECPIX|SECPIX[1-2]|UT|UTMID|VELOCITY|VSOURCE|WCSAXES|WCSDEP|WCSDIM|WCSNAME|XPIXSIZE|YPIXSIZE|ZSOURCE|LTM|LTV/;
...
this.divjq)};a.Magnifier.display=function(c,b){var d,e,f,g,h,j,k,m,n,p,q,l;c&&(c.display.pluginInstances.JS9Magnifier&&"active"===c.display.pluginInstances.JS9Magnifier.status)&&!c.clickState&&(c.magnifier||(c.magnifier={zoom:a.Magnifier.opts.zoom,posx:0,posy:0}),e=c.display.pluginInstances.JS9Magnifier,g=c.display.canvas,f=c.magnifier.zoom,k=Math.floor(e.width/f),m=Math.floor(e.height/f),b?(d=c.imageToDisplayPos(b),h=d.x-k/2,j=d.y-m/2,c.magnifier.posx=h,c.magnifier.posy=j):(h=c.magnifier.posx,j=
c.magnifier.posy),p=n=0,q=e.canvas.width,l=e.canvas.height,0>h&&(k+=h,n-=h*f,q+=h*f,h=0),d=h+k-g.width,0<d&&(k-=d,q=k*f),0>j&&(m+=j,p-=j*f,l+=j*f,j=0),d=j+m-g.height,0<d&&(m-=d,l=m*f),e.context.clear(),e.context.drawImage(g,h,j,k,m,n,p,q,l),c.magnifier.boxid||(c.magnifier.boxid=c.addShapes("magnifier","box"),$(e.canvas).css("background-color","black")),g=e.width/2,e=e.height/2,c.changeShapes("magnifier",c.magnifier.boxid,{left:g,top:e,width:f,height:f}))};a.Magnifier.zoom=function(c,b){var d,e;if(c&&

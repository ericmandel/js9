...
function(d,b,c){var e,f,g,h,l,j,m,n,q,k,r=[],p={};if((j=this.getShapeLayer(d))&&j.show){m=j.canvas;"main"===this.display.layers[d].dtype?(n=this.primary.sect.zoom,q=this.binning.bin||1):q=n=1;if("string"===typeof b)e=this.parseRegions(b),b="string"===typeof e?[{shape:b}]:e;else if(!$.isArray(b))if("object"===typeof b)b=[b];else return;if("string"===typeof c)try{l=JSON.parse(c)}catch(s){return a.error("can't parse shape opts: "+c,s),null}else l=c;m.size()||(c=this.display.layers[d],"regions"===d&&
...
d.dval).split(/ +/),e=parseFloat(d[0])-1,d=parseFloat(d[1])-1):"physical"===j?(d=this.logicalToImagePos({x:e.dval,y:d.dval}),e=d.x,d=d.y):(e=e.dval,d=d.dval);return[e,d]},x=function(a,b){var c=v(a);m&&(c.dtype&&"."!==c.dtype)&&(c.dval=Math.abs(c.dval/q["cdelt"+b]));return c.dval},y=function(a){a=v(a);m&&(a.dval+=q.crot,q.imflip&&(a.dval=-a.dval));return a.dval};if(!d.match(/(\C|\{|#)/))return d;try{q=JSON.parse(a.wcsinfo(this.wcs))}catch(A){q={cdelt1:1,cdelt2:1,crot:0,imflip:0}}l=this.getWCSSys();
...
typeof c?c:0<=c.search(a.analOpts.epattern)?{stderr:c}:{stdout:c};c.stderr&&a.error(c.stderr,a.analOpts.epattern);c.stdout&&(c=c.stdout.replace(/\n*$/,"").split("\n").pop(),d=c.split(".").pop().toLowerCase(),"png"===d?a.checkNew(new a.Image(c,b,e)):a.error("fits2png conversion failed: "+c))}):a.error("no helper available to convert this image: "+d):(a.waiting(!0),c=d.replace(/\[.*\]/,""),a.fetchURL(d,c,b,a.NewFitsImage)));else a.error("JS9.Load: no file specified for image load")});a.mkPublic("LoadRegions",

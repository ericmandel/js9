...
this.height+a.RESIZEFUDGE)),this.resizeSensor=new ResizeSensor(this.divjq,function(){var c=b.divjq.width(),e=b.divjq.height();a.bugs.webkit_resize&&(c-=a.RESIZEFUDGE,e-=a.RESIZEFUDGE);b.resizing=1;b.resize(c,e)}));this.context=this.canvas.getContext("2d");a.ANTIALIAS||(this.context.imageSmoothingEnabled=!1,this.context.mozImageSmoothingEnabled=!1,this.context.webkitImageSmoothingEnabled=!1);this.tooltip=$("<div>").attr("id","tooltip_"+this.id).addClass("JS9Tooltip").appendTo(this.divjq);this.image=
null;this.pluginInstances={};this.layers={};this.initMessages();this.blendMode=!1;this.divjq.on("mouseover",this,function(c){return a.mouseOverCB(c)});this.divjq.on("mousedown touchstart",this,function(c){return a.mouseDownCB(c)});this.divjq.on("mousemove touchmove",this,function(c){return a.mouseMoveCB(c)});this.divjq.on("mouseup touchend",this,function(c){return a.mouseUpCB(c)});this.divjq.on("mouseout",this,function(c){return a.mouseOutCB(c)});this.divjq.on("keypress",this,function(c){return a.keyPressCB(c)});
this.divjq.on("keydown",this,function(c){return a.keyDownCB(c)});this.divjq.on("dragenter",this,function(c){return a.dragenter(this.id,c.originalEvent)});this.divjq.on("dragover",this,function(c){return a.dragover(this.id,c.originalEvent)});this.divjq.on("dragexit",this,function(c){return a.dragexit(this.id,c.originalEvent)});this.divjq.on("drop",this,function(c){return a.dragdrop(this.id,c.originalEvent,a.NewFITSImage)});this.divjq.append('<div style="visibility:hidden; position:relative; top:-50;left:-50"> <input type="file" id="openLocalFile-'+
...
c.name,m.message,a.strace(m))}}else if("inactive"===h.status){if(h.winHandle&&(h.winHandle.show(),h.status="active",c.opts.plugindisplay))try{c.opts.plugindisplay.call(h,this.image)}catch(n){a.log("plugindisplayCB: %s [%s]\n%s",c.name,n.message,a.strace(n))}}else"active"===h.status&&h.winHandle&&(h.winHandle.hide(),h.status="inactive");break;case "new":a.error("external window support for plugins not yet implemented")}};a.Display.prototype.resize=function(c,b,d){var e,f,g,h,j=function(a){a.left+=

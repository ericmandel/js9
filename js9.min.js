...
d)};a.Image.prototype.displayAnalysis=function(d,b,c){var e,f,g,h=a.lightOpts[a.LIGHTWIN];c=c||"AnalysisResults: "+(this.fitsFile||this.id);e="Analysis_"+a.uniqueID();switch(d){case "text":f="<div class='JS9Analysis'></div>";b&&(f+="<pre class='JS9AnalysisText'>"+b+"</pre>");f=a.lightWin(e,"inline",f+"</div>",c,h.textWin);break;case "plot":try{g=JSON.parse(b)}catch(l){a.error("can't plot return data: "+b,l)}if(!g)return;f=sprintf("<div id='%s' class='JS9Analysis'><div id='%sPlot' class='JS9Plot' ></div></div>",

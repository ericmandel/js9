...
(g=this.display.rgb.gim),this.display.rgb.bim&&(k=this.display.rgb.bim));var r=this.display.context;var v=this.rgb;var w=v.sect;if(this.MakeRGBImage&&"function"===typeof this.MakeRGBImage&&this.MakeRGBImage()||this.MakePrimaryImage&&"function"===typeof this.MakePrimaryImage&&this.MakePrimaryImage())return this;if(this.rgbFile){var t=w.width/w.zoom;var u=w.height/w.zoom;var x=w.x0;var y=this.offscreen.canvas.height-1-(w.y0+u);y=this.offscreen.context.getImageData(x,y,t,u);if(1===w.zoom)v.img=y;else for(v.img=
r.createImageData(w.width,w.height),v=v.img,d=c=0;c<y.height;c++,d++){var B=c*y.width;var z=d*w.zoom;for(b=r=0;r<y.width;r++,b++){x=4*(B+r);var A=b*w.zoom;for(e=0;e<w.zoom;e++){var C=Math.floor(z+e);var E=C*w.width;for(C=0;C<w.zoom;C++){var D=Math.floor(A+C);D=4*(E+D);v.data[D]=y.data[x];v.data[D+1]=y.data[x+1];v.data[D+2]=y.data[x+2];v.data[D+3]=y.data[x+3]}}}}return this}v.img&&v.img.width===w.width&&v.img.height===w.height||(v.img=r.createImageData(w.width,w.height));v=v.img;var F=v.data.length-
...
J.rgb.img;a.isNull(this.mask.opacity)&&(this.mask.opacity=1)}}else if(a.notNull(this.params.flooropacity)&&!l){var R=255*this.params.flooropacity;var M=this.params.floorvalue;f=!0}var N=Math.max(1,Math.floor(1/w.zoom));var P=w.zoom*N;c=Math.floor(w.y1-1);for(d=0;c>=w.y0;c-=N,d++)for(B=c*this.raw.width,z=d*P,r=Math.floor(w.x0),b=0;r<w.x1;r+=N,b++){m?G=J.raw.data[B+r]>this.mask.value?K:I:n&&(G=255*J.raw.data[B+r]);if(l){if(y=h?h.colorData[B+r]:0,t=g?g.colorData[B+r]:0,u=k?k.colorData[B+r]:0,a.isNull(y)||
a.isNull(t)||a.isNull(u))return this.display.rgb.active=!1,a.error("RGB images are incompatible. Turning off RGB mode.","",!1),this.mkRGBImage(),this}else this.staticObj||(x=this.colorData[B+r]);var Q=G;f&&this.raw.data[B+r]<=M&&(Q=R);A=b*P;for(e=0;e<w.zoom;e++)for(C=Math.ceil(z+e),E=C*w.width,C=0;C<w.zoom;C++)if(D=Math.ceil(A+C),D=4*(E+D),D<=F)if(l)v.data[D]=h?h.psColors[y][0]:0,v.data[D+1]=g?g.psColors[t][1]:0,v.data[D+2]=k?k.psColors[u][2]:0,v.data[D+3]=G;else if(this.staticObj){var H=this.raw.data[B+
r];H=a.lookupStaticColor(this,H,q);v.data[D]=H.red;v.data[D+1]=H.green;v.data[D+2]=H.blue;v.data[D+3]=H.alpha}else if(void 0!==this.psColors[x])if(p&&L.data[D+3]){H=L.data[D+3]/255*this.mask.opacity;var O=1-H;v.data[D]=L.data[D]*H+this.psColors[x][0]*O;v.data[D+1]=L.data[D+1]*H+this.psColors[x][1]*O;v.data[D+2]=L.data[D+2]*H+this.psColors[x][2]*O;v.data[D+3]=255}else v.data[D]=this.psColors[x][0],v.data[D+1]=this.psColors[x][1],v.data[D+2]=this.psColors[x][2],v.data[D+3]=Q}return this};a.Image.prototype.blendImage=
...
JSON.parse(f[1])}catch(x){a.log("couldn't parse imsection as JSON: %s",b),g=null}g&&(c.extname=g.extname,c.extnum=g.extnum,c.hdus=g.hdus,c.binstr=g.binstr,c.parent=g)}f[2]&&(g=a.cleanPath(f[2]),c.parentFile=g);a.fetchURL(b,b,c,function(b){a.cleanupFITSFile(d.raw,!0);!1!==c.waiting&&a.waiting(!0,d.display);a.fits.handleFITSFile(b,c,p)})}});break;case "virtualFile":a.cleanupFITSFile(d.raw,!1);a.getFITSImage(r.fits,r,c,function(a){p(a,c)});break;default:a.error("image section cannot be extracted from this data file")}},
...
var u="awcs_"+a.uniqueID()+".txt";t=a.tanhdr(v,u,"");1<a.DEBUG&&a.log("tanhdr (reproj): %s %s -> %s",v,u,t);a.vunlink(v);0<=t.search(/\[struct stat="OK"/)&&(a.vunlink(p),p=u)}}catch(z){}if(n.hdu&&n.hdu.fits.vfile)c=n.hdu.fits.vfile,n.hdu.fits.extname?c+="["+n.hdu.fits.extname+"]":n.hdu.fits.extnum&&0<n.hdu.fits.extnum&&(c+="["+n.hdu.fits.extnum+"]");else{var x=this.toArray();c=this.id.replace(/\.png$/,"_png.fits");a.vfile(c,x)}u=this.id.replace(/\[.*\]/,"").replace(/\.png$/i,".fits").replace(/\.fz$/i,
"").replace(/\.gz$/i,"");u="reproj_"+a.uniqueID()+"_"+u;d=b.rawid||"reproject";for(v=0;v<this.raws.length&&(r=this.raws[v],r.id!==d||!a.cleanupFITSFile(r,!0));v++);n.hdu&&"table"===n.hdu.imtab&&n.hdu.table&&!c.match(/\[bin /)&&(c.match(/\[EVENTS\]/)||(c+="[EVENTS]"),r=n.hdu.table,n=Math.floor(r.xcen-r.xdim/2+1),v=Math.floor(r.xcen+r.xdim/2),d=Math.floor(r.ycen-r.ydim/2+1),r=Math.floor(r.ycen+r.ydim/2),c+="[bin X="+n+":"+v+",Y="+d+":"+r+"]");try{var y=u.lastIndexOf(".");0<=y&&(e=u.substring(0,y)+"_area"+
u.substring(y));var B=b.cmdswitches||"";B+=" -a 0 "+a.globalOpts.reprojSwitches;t=a.reproject(c,u,p,B);1<a.DEBUG&&a.log("reproject: %s %s %s [%s] -> %s",c,u,p,B,t);a.vunlink(e);a.vunlink(p);w&&a.vunlink(w);x&&a.vunlink(c);0>t.search(/\[struct stat="OK"/)&&(h=!0,(f=t.match(/msg="(.*)"/))&&f[1]?a.error(f[1]+" (from mProjectPP)"):a.error(t))}catch(z){if(h)return;a.vunlink(e);a.vunlink(p);t?a.error(t):a.error("WCS reproject failed",z)}return u}};a.Image.prototype.reprojectData=function(c){for(var b=[],
...
0<t.length&&(t.isactive="visible"===t.closest(".JS9PluginContainer").css("visibility"));u=$("#"+c+"Toolbar");0<u.length&&(u.isactive="visible"===u.closest(".JS9PluginContainer").css("visibility"));x=$("#"+c+"Statusbar");0<x.length&&(x.isactive="visible"===x.closest(".JS9PluginContainer").css("visibility"));y=$("#"+c+"Colorbar");0<y.length&&!x.length&&(y.isactive="visible"===y.closest(".JS9PluginContainer").css("visibility"));0<w.length&&(B=w.width(),z=w.height(),A=w.offset().top-$(window).scrollTop()-
5,C=w.offset().left-$(document).scrollLeft(),t.isactive&&(z+=t.height(),A-=t.height()),u.isactive&&(z+=u.height(),A-=u.height()),x.isactive?(z+=x.height(),A-=x.height()):y.isactive&&(z+=y.height(),A-=y.height(),A+=7))},J=function(a,b){if(a){if(0<w.length){var c="";t.isactive&&(c+=sprintf("<div class='JS9Menubar' id='%sMenubar' data-width=%s></div>",b,w.width()));u.isactive&&(c+=sprintf("<div class='JS9Toolbar' id='%sToolbar' data-width=%s></div>",b,w.width()));c+=sprintf("<div class='JS9' id='%s' data-width=%s data-height=%s></div>",
...
a.uniqueID();var f=function(a){return"mosaic_"+d+"_"+a};var k=f("in.lst");var w=f("in.tbl");var t=f("in.hdr");var u=f("bin.lst");var x=f("bin.tbl");var y=f("out.lst");var B=f("out.tbl");var z=f("out.hdr");f=c[0].id.replace(/\[.*\]/,"").replace(/\.fz$/i,"").replace(/\.gz$/i,"").replace(/\.fits$/i,"_mosaic.fits");var A=f.replace(/\.fits$/,"_area.fits");g=[k,w,t,u,x,y,B,z,A];A="|                                                    fname|\n|                                                     char|\n";
for(e=0;e<c.length;e++)A+=c[e].raw.hdu.fits.vfile+"\n";a.vfile(k,A);k=a.imgtbl(k,".",w,"-C");m("mImgtbl",k);a.vsize(w)||a.error("no image data found with which to construct a mosaic");k=a.makehdr(w,t,"");m("mMakeHdr",k);if("js9"===b.reduce){A=a.vread(t).split("\n");for(e=t=0;e<A.length;e++)switch(k=A[e].split("="),k[0].trim()){case "NAXIS1":t=Math.max(t,parseFloat(k[1].trim()));break;case "NAXIS2":t=Math.max(t,parseFloat(k[1].trim()))}h=Math.max(1,Math.floor(t/b.dim+.5));A="|                                                    fname|\n|                                                     char|\n";
w=a.vread(w);w=w.trim().split("\n");w.splice(0,3);for(e=0;e<w.length;e++)if(k=w[e].trim().split(/\s+/),t=k[k.length-2],k=k[k.length-1],t&&k){var C=k+"["+t+"]";var E=k.split("/").reverse()[0].replace(/\.(g|f)z$/,"");t="bin_"+t+"_"+E;g.push(t);k="0@0,0@0,"+h;p("bin %s [%s]",C,h);a.imsection(C,t,k,"");A+=t+"\n"}a.vfile(u,A);k=a.imgtbl(u,".",x,"-C");m("mImgtbl",k);a.vsize(x)||a.error("no image data found to construct a mosaic");k=a.makehdr(x,z,"");m("mMakeHdr",k);w=a.vread(x)}else k=a.shrinkhdr(b.dim,
t,z),m("mShrinkHdr",k),w=a.vread(w);w=w.trim().split("\n");w.splice(0,3);A="|                                                    fname|\n|                                                     char|\n";for(e=0;e<w.length;e++)k=w[e].trim().split(/\s+/),t=k[k.length-2],k=k[k.length-1],t&&k&&(u="-a 1","shrink"===b.reduce&&(u+=" -h "+t),u+=" "+a.globalOpts.reprojSwitches,E=k.split("/").reverse()[0].replace(/\.(g|f)z$/,""),x="reproj_"+t+"_"+E,A+=x+"\n",g.push(x),g.push(x.replace(/\.fits$/i,"_area.fits")),
p("reproject: %s [%s] -> %s",k,t,x),k=a.reproject(k,x,z,u),m("mProjectPP",k));a.vfile(y,A);k=a.imgtbl(y,".",B,"");m("mImgtbl",k);a.vsize(B)||a.error("no FITS files were added to output table for mosaic");p("create mosaic: %s",f);k=a.madd(B,z,f,"");m("mAdd",k);l();y=$.extend(!0,{},a.fits.options,b);y.image={xdim:0,ydim:0};y.file=f;a.fits.handleFITSFile(f,y,n)},a.SPINOUT);return this};a.Display.prototype.moveImageInStack=function(c,b){var d,e,f,h;for(e=d=0;d<a.images.length;d++)if(a.images[d].display.id===
...
c);if(d){switch(d.type){case "polyline":case "polygon":a.Fabric.addPolygonAnchors(b,d),b.canvas.renderAll()}d.polyparams?b.params.sel=d.polyparams.polygon:d.params&&(b.params.sel=d);b.display.image&&"activeSelection"!==d.type&&b.display.image.updateShapes(c,d,"select");f(1)}};if(this&&c){if(this.layers[c])return this.layers[c];this.layers[c]={};var k=this.layers[c];k.layerName=c;k.params=[];k.params.sel=null;k.params.sellayer=null;d?k.dtype="other":(k.dtype="main",d=this.divjq);var l=d.attr("id")+
"-"+c.replace(/\s+/,"_")+"-shapeLayer";k.display=this;k.opts=$.extend(!0,{},b);k.opts.canvas=k.opts.canvas||{};void 0===k.opts.canvas.selection&&(k.opts.canvas.selection=!0);k.el=a.Fabric.elements;k.divjq=$("<div>").addClass("JS9Container").css("z-index",0).appendTo(d);k.canvasjq=$("<canvas>").addClass("JS9Layer").attr("id",l).attr("width",d.css("width")).attr("height",d.css("height")).appendTo(k.divjq);a.bugs.webkit_resize&&"main"===k.dtype&&k.canvasjq.attr("width",this.width).attr("height",this.height);
k.canvas=new fabric.Canvas(k.canvasjq[0]);k.canvas.renderOnAddRemove=!1;k.canvas.preserveObjectStacking=!0;k.opts.movable?(k.opts.lockMovementX=!1,k.opts.lockMovementY=!1,k.opts.selectable=!0,k.opts.evented=!0):!1===k.opts.movable&&(k.opts.lockMovementX=!0,k.opts.lockMovementY=!0,k.opts.selectable=!1,k.opts.evented=!1);void 0===k.opts.changeable&&void 0!==k.opts.fixinplace&&(k.opts.changeable=!k.opts.fixinplace);k.opts.changeable?(k.opts.hasControls=!0,k.opts.hasRotatingPoint=!0,k.opts.hasBorders=
...
function(b){b=k.canvas.getActiveObject();if("activeSelection"===b.type){var d,e=k.canvas.getActiveObjects();for(d=0;d<e.length;d++){var h=e[d];if(h.params){if(h.params.parent&&h.params.parent.obj){var l=h.params.parent.obj;0>$.inArray(l,e)&&b.addWithUpdate(l)}if(h.params.children)for(l=0;l<h.params.children.length;l++){var m=h.params.children[l].obj;0>$.inArray(m,e)&&b.addWithUpdate(m)}switch(h.type){case "polyline":case "polygon":a.Fabric.removePolygonAnchors(k,h)}h&&k.display.image&&k.display.image.updateShapes(c,
h,"select")}}k.canvas.renderAll();f(1)}else g(k,b)}),k.canvas.on("before:selection:cleared",function(a){var b=k.canvas.getActiveObject();if("activeSelection"===b.type){var c=k.canvas.getActiveObjects();for(b=0;b<c.length;b++){var d=c[b];h(k,d)}f(a.e?-1:0)}else h(k,b)}));if(k.divjq.closest(a.lightOpts[a.LIGHTWIN].drag).length)if(fabric.isTouchSupported)k.divjq.on("touchstart",function(){k.canvas.calcOffset()});else k.divjq.on("mouseenter",function(){k.canvas.calcOffset()});return k}};a.Fabric.showShapeLayer=
...
(b.angle+=this.raw.wcsinfo.crot);else if(a.notNull(this.raw.header.LTM1_1)||a.notNull(this.raw.header.LTM1_2)){try{var l=Math.atan((this.raw.header.LTM1_2||0)/(this.raw.header.LTM1_1||0))}catch(w){l=0}l&&(l=180*-l/Math.PI,b.angle+=l)}a.notNull(this.params.transformAngle)&&(b.angle+=this.params.transformAngle);a.notNull(this.params.transformScale)&&(b.angle*=this.params.transformScale)}f.angle=-b.angle}void 0!==b.dx&&void 0!==b.dy&&(f.left=b.dx,f.top=b.dy);void 0!==b.x&&void 0!==b.y&&(l=this.imageToDisplayPos(b),
...
if(!this.layers||!c||!this.layers[c])return null;b=b||"all";"string"===typeof b&&/^[1-9]\d*$/.test(b)&&(b=parseInt(b,10));var f=this.layers[c].canvas;var h=1===fabric.major_version?f.getActiveGroup():(h=f.getActiveObject())&&"activeSelection"===h.type?h:null;c={group:null,canvas:f,layer:c};switch(typeof b){case "object":b.params&&(h&&h.contains(b)&&(c.group=h),d.call(this,b,c));break;case "number":f=f.getObjects();for(e=f.length;e--;){var g=f[e];g.params&&b===g.params.id&&(h&&h.contains(g)?c.group=
h:c.group=null,d.call(this,g,c))}break;case "string":if("selected"===b)if(1===fabric.major_version)if(f.getActiveObject())b=f.getActiveObject(),b.params&&(c.group=null,d.call(this,b,c));else{if(h)for(c.group=h,f=h.getObjects(),e=f.length;e--;)g=f[e],g.params&&!g.params.parent&&d.call(this,g,c)}else for(c.group=h,f=f.getActiveObjects(),e=f.length;e--;)g=f[e],g.params&&!g.params.parent&&d.call(this,g,c);else for(f=f.getObjects(),e=f.length;e--;)if(g=f[e],g.params&&(!g.params.parent||"child"===b||"All"===
b)&&("child"!==b||g.params.parent)){var k=g.stroke.toLowerCase();h&&h.contains(g)?c.group=h:c.group=null;if("all"===b.toLowerCase())d.call(this,g,c);else if(b.toLowerCase()===k||a.colorToHex(b).toLowerCase()===k)d.call(this,g,c);else if(b===g.params.shape)d.call(this,g,c);else if(b===g.params.file)d.call(this,g,c);else if("object"===typeof g.params.data&&b===g.params.data.syncid)d.call(this,g,c);else if("child"===b&&g.params.parent)d.call(this,g,c);else if("dcoords"===b&&g.params.preservedcoords)d.call(this,
g,c);else if("nodcoords"===b&&!g.params.preservedcoords)d.call(this,g,c);else if("parent"===b&&g.params.children&&g.params.children.length)d.call(this,g,c);else if(g.params.tags)for(k=0;k<g.params.tags.length;k++){var l=g.params.tags[k];b.match(/^\/.*\/$/)&&l.match(new RegExp(b.slice(1,-1)))?d.call(this,g,c):b===l&&d.call(this,g,c)}}}return this};a.Fabric.selectShapes=function(a,b,d){var c=[];if(d=this.getShapeLayer(a))return d=d.canvas,d.discardActiveObject(),this._selectShapes(a,b,function(a){c.push(a)}),
a=new fabric.ActiveSelection(c,{canvas:d}),d.setActiveObject(a),d.renderAll(),this};a.Fabric.updateShapes=function(a,b,d,e){var c=this;this._selectShapes(a,b,function(b,f){c._updateShape(a,b,f,d,e)});return this};a.Fabric._updateMultiDialogs=function(a){var b=this;$("form[class*='regionsConfigForm']").each(function(c,e){c=$(e).data("multi");var d=$(e).data("winid");e=$(e).data("im");c&&d&&e===b&&e.initRegionsForm(null,{winid:d,multi:c,setmode:a})})};a.Fabric._updateShape=function(c,b,d,e,f){var h,
...
"").replace(/\).*/,"").split(",");for(b=0;b<k.length;b++)k[b]=k[b].trim();h.wcsposstr=[k[0],k[1]];switch(d.shape){case "annulus":h.wcssizestr=[k[k.length-1]];break;case "box":case "cross":h.wcssizestr=[k[2],k[3]];break;case "circle":h.wcssizestr=[k[2]];break;case "ellipse":h.wcssizestr=[k[2],k[3]];break;case "line":case "polygon":for(h.wcspts=[],b=0;b<k.length;b+=2)l=a.strtoscaled(k[b]),a.isHMS(h.wcssys,l.dtype)&&(l.dval*=15),m=a.strtoscaled(k[b+1]),h.wcspts.push({ra:l.dval,dec:m.dval})}}};d=d||{};
f=f||{};e=e||"update";var p="main"!==this.display.layers[c].dtype||b.params.preservedcoords?1:this.rgb.sect.zoom;g.mode=e;g.id=b.params.id;g.shape=b.params.shape;g.layer=c;g.color=b.color||b.stroke;g.tags=b.params.tags;g.sticky=b.params.sticky;g.preservedcoords=b.params.preservedcoords;g.parent=b.params.parent?b.params.parent.obj.params.id:null;g.child=b.params.children&&b.params.children.length?b.params.children[0].id:null;var q=b.getCenterPoint();if(d.group){var r=d.group.getCenterPoint();q={x:q.x+
r.x,y:q.y+r.y};d.group.angle&&(q=a.rotatePoint(q,d.group.angle,r))}g.dx=q.x;g.dy=q.y;q=this.displayToImagePos(q);g.x=q.x;g.y=q.y;g.lcs=this.imageToLogicalPos(q);"export"!==e&&b.params.wcsconfig&&("image"===b.params.wcsconfig.wcssys?h="image":"physical"===b.params.wcsconfig.wcssys&&(h="physical"));if("image"===h||"image"===this.params.wcssys&&"physical"!==h){g.imsys="image";var v=g.x;var w=g.y;r=1}else g.imsys=g.lcs.sys,v=g.lcs.x,w=g.lcs.y,r=Math.sqrt(Math.pow(this.lcs.physical.reverse[0][0],2)+Math.pow(this.lcs.physical.reverse[0][1],
2));g.angle=-b.angle;d.group&&(g.angle-=d.group.angle);h=g.angle;if(!g.parent)switch(g.shape){case "box":case "cross":case "ellipse":case "text":if(a.notNull(this.params.transformScale)&&(g.angle/=this.params.transformScale),a.notNull(this.params.transformAngle)&&(g.angle-=this.params.transformAngle),this.raw.wcsinfo)this.raw.wcsinfo.crot&&(g.angle-=this.raw.wcsinfo.crot);else if(a.notNull(this.raw.header.LTM1_1)||a.notNull(this.raw.header.LTM1_2)){try{var t=Math.atan((this.raw.header.LTM1_2||0)/
(this.raw.header.LTM1_1||0))}catch(z){t=0}t&&(t=180*-t/Math.PI,g.angle-=t)}}for(;0>g.angle;)g.angle+=360;for(;360<=g.angle;)g.angle-=360;q=b.scaleX/p;t=b.scaleY/p;d.group&&(q*=d.group.scaleX,t*=d.group.scaleY);switch(g.shape){case "annulus":g.shape="annulus";g.radii=[];"image"!==g.imsys&&(g.lcs.radii=[]);g.imstr="annulus("+m(v)+","+m(w)+",";var u="annulus "+g.x+" "+g.y+" ";var x=b.getObjects();h=x.length;for(d=0;d<h;d++)p=x[d].radius*q,t=p*r,g.imstr+=m(t),u+=g.x+" "+g.y+" "+(g.x+p)+" "+g.y+" ",g.imstr=
d===h-1?g.imstr+")":g.imstr+",",g.radii.push(p),"image"!==g.imsys&&g.lcs.radii.push(t);break;case "box":case "cross":g.width=b.width*q;g.height=b.height*t;t=g.width*r;u=g.height*r;"image"!==g.imsys&&(g.lcs.width=t,g.lcs.height=u);g.imstr=g.shape+"("+m(v)+","+m(w)+","+m(t)+","+m(u)+","+g.angle.toFixed(4)+")";u=g.shape+" "+g.x+" "+g.y+" "+g.x+" "+g.y+" "+(g.x+g.width)+" "+g.y+" "+g.x+" "+g.y+" "+g.x+" "+(g.y+g.height)+" "+g.angle*Math.PI/180;break;case "circle":g.radius=b.radius*q;t=g.radius*r;"image"!==
g.imsys&&(g.lcs.radius=t);g.imstr="circle("+m(v)+","+m(w)+","+m(t)+")";u="circle "+g.x+" "+g.y+" "+g.x+" "+g.y+" "+(g.x+g.radius)+" "+g.y;break;case "ellipse":g.r1=b.width*q/2;g.r2=b.height*t/2;t=g.r1*r;u=g.r2*r;"image"!==g.imsys&&(g.lcs.r1=t,g.lcs.r2=u);g.imstr="ellipse("+m(v)+","+m(w)+","+m(t)+","+m(u)+","+g.angle.toFixed(4)+")";u="ellipse "+g.x+" "+g.y+" "+g.x+" "+g.y+" "+(g.x+g.r1)+" "+g.y+" "+g.x+" "+g.y+" "+g.x+" "+(g.y+g.r2)+" "+g.angle*Math.PI/180;break;case "point":g.width=b.width*q;g.height=
b.height*t;t=g.width*r;u=g.height*r;"image"!==g.imsys&&(g.lcs.width=t,g.lcs.height=u);g.imstr="point("+m(v)+","+m(w)+")";u="point "+g.x+" "+g.y;break;case "line":case "polygon":g.imstr=g.shape+"(";u=g.shape+" ";g.pts=[];"image"!==g.imsys&&(g.lcs.pts=[]);for(d=0;d<b.points.length;d++)0<d&&(g.imstr+=",",u+=" "),t=this.displayToImagePos({x:b.left+b.points[d].x*b.scaleX,y:b.top+b.points[d].y*b.scaleY}),t=a.rotatePoint(t,h,{x:g.x,y:g.y}),"image"===g.imsys?g.imstr+=m(t.x)+","+m(t.y):(r=this.imageToLogicalPos(t),
q=r.x,r=r.y,g.imstr+=m(q)+","+m(r),g.lcs.pts.push({x:q,y:r})),u+=t.x+" "+t.y,g.pts.push(t),"line"===g.shape&&(0===d?x=0:(q=g.pts[d-1],x+=Math.sqrt((t.x-q.x)*(t.x-q.x)+(t.y-q.y)*(t.y-q.y))));if("line"===g.shape&&a.notNull(x)){g.imstr+=') {"size":'+m(x)+',"units":"pixels"';if(2===g.pts.length){for(t=180*Math.atan2(g.pts[1].y-g.pts[0].y,g.pts[1].x-g.pts[0].x)/Math.PI;0>t;)t+=360;var y=',"posang":'+t.toFixed(4)+',"posunits":"degrees"';g.imstr+=y}g.imstr+="}"}else g.imstr+=")";g.angle=0;break;case "text":g.imstr=
"text("+m(v)+","+m(w)+',"'+b.text+'",'+g.angle.toFixed(4)+")",u="text "+g.x+" "+g.y+' "'+b.text+'" '+g.angle*Math.PI/180,g.text=b.text}this.raw.wcs&&0<this.raw.wcs&&(n(this.raw.wcs,k,g,u,y,f,g),b.params.wcsconfig&&b.params.wcsconfig.wcssys&&(m=a.globalOpts.xeqPlugins,a.globalOpts.xeqPlugins=!1,x=this.getWCSSys(),a.notWCS(b.params.wcsconfig.wcssys)||(this.setWCSSys(b.params.wcsconfig.wcssys,!1),n(this.raw.wcs,k,g,u,y,f,b.params.wcsconfig)),g.wcsconfig=$.extend(!0,{},b.params.wcsconfig),this.setWCSSys(x,
!1),a.globalOpts.xeqPlugins=m));g.data=b.params.data;b.set("pub",g);b.params.winid&&($(b.params.winid).is(":visible")?this.initRegionsForm(b):b.params.winid=null);if(f.nocb)return g;if(!b.params.parent&&!e.match(l)){if(this.params.xeqonchange&&k.show&&k.opts.onchange)try{this.params.xeqonchange=!1,a.xeqByName(k.opts.onchange,window,this,g)}catch(z){a.log("error in onchange: %s [%s]\n%s",this.id,z.message,a.strace(z))}finally{this.params.xeqonchange=!0}this.xeqPlugins("shape","on"+c+"change",g)}if("regions"===
c&&a.globalOpts.regToClipboard){switch(e){case "update":d=g.parent||g.id;break;default:d=null}if(a.notNull(d)){try{var B=this.listRegions(d,{mode:1,includedcoords:!0})}catch(z){B=null}B&&(a.clipboard=B)}}"regions"===c&&"wcs"===e&&this._updateMultiDialogs(!0);return g};a.Fabric.removeShapes=function(c,b,d){var e=this,f,h={mode:1,includedcoords:!0},g=[];if(f=this.getShapeLayer(c)){var k=f.canvas;d=d||{};"regions"===c&&a.globalOpts.unremoveReg&&(this.regstack.push(this.listRegions(b,h,c)),this.regstack.length>
a.globalOpts.unremoveReg&&(this.regstack=this.regstack.slice(0,a.globalOpts.unremoveReg)));this._selectShapes(c,b,function(a,b){if(!(!1===a.params.removable&&!d.overrideRemovable||a.params.sticky&&!1===d.sticky)){e._updateShape(c,a,b,"remove");a.params.winid&&(a.params.winid.close(),a.params.winid=null);if(a.params.parent){var f=a.params.parent.obj;for(b=f.params.children.length-1;0<=b;b--)if(a===f.params.children[b].obj){f.params.children.splice(b,1);break}}for(b=0;b<a.params.children.length;b++)f=
a.params.children[b].obj,g.push(f);g.push(a)}});for(b=0;b<g.length;b++)k.remove(g[b]);1===fabric.major_version?k.getActiveGroup()&&k.discardActiveGroup():k.discardActiveObject();k.renderAll();!k.getObjects().length&&a.globalOpts.resetEmptyShapeId&&(f.nshape=0);return this}};a.Fabric.getShapes=function(c,b,d){var e={},f=[];d=d||{};if("string"===typeof d)try{d=JSON.parse(d)}catch(k){a.error("can't parse getShapes opts: "+d,k)}if("regions"===c&&("text"===d.format||"csv"===d.format)){d.mode=d.mode||1;
b=this.listRegions(b,d);if("csv"===d.format){var h=b.split(";");c=0;for(b="";c<h.length;c++)if(h[c])if(h[c].toLowerCase().match(a.WCSEXP))d.includewcs&&(b+=h[c].trim()+"\n");else{var g=h[c].replace(/\(/,",").replace(/\).*/,"").trim();b+=g+"\n"}}return b}this._selectShapes(c,b,function(a){e=a.pub||{};d.includeObj&&(e.obj=a);f.push(e)});return f};a.Fabric.changeShapes=function(c,b,d){var e=this,f,h,g,k,l,m,n,p,q,r,v,w=[],t=[];if((l=this.getShapeLayer(c))&&d){if("string"===typeof d)try{d=JSON.parse(d)}catch(y){a.error("can't parse shape opts: "+
d,y)}if(this.display.image!==this)this.delayedShapes=this.delayedShapes||[],this.delayedShapes.push({layer:c,shape:b,mode:"change",opts:d});else{var u=l.canvas;var x=u.getActiveObject();this._selectShapes(c,b,function(b,B){p="main"!==e.display.layers[c].dtype||b.params.preservedcoords?1:e.rgb.sect.zoom;d.radii&&(b.params.radii=[]);d.tags&&(b.params.tags=[]);k=$.extend(!0,{},b.params,d);g=e._parseShapeOptions(c,k,b);if(g.remove){if(!0===g.remove||"true"===g.remove)g.remove="all";if(!1!==g.remove&&
"false"!==g.remove){e.removeShapes(c,g.remove||"all");return}}if(g.opts.send)switch(g.opts.send){case "front":u.sendToFront(x);u.discardActiveObject();break;case "back":u.sendToBack(x),u.discardActiveObject()}q=e._exportShapeOptions(d).filter(function(a){return!b.params.exports.hasOwnProperty(a)});g.params.exports=b.params.exports.concat(q);g.opts.stroke&&(g.opts.color=g.opts.stroke,g.opts.stroke=a.colorToHex(g.opts.stroke));switch(b.params.shape){case "text":g.opts.stroke&&(g.opts.fill=g.opts.stroke);
g.opts.strokeWidth=0;break;case "line":case "polygon":g.opts.points&&g.opts.points.length&&(b.angle=0)}b.set(g.opts);b.params=$.extend(!1,{},b.params,g.params);d.strokeWidth&&(b.params.sw1=d.strokeWidth);switch(b.params.shape){case "annulus":if(d.radii&&d.radii.length){b.forEachObject(function(a){w.push(a)});m=w.length;for(f=0;f<m;f++)b.remove(w[f]),u.remove(w[f]);m=b.params.radii.length;n=0;r=$.extend(!0,{},d);r.stroke=r.stroke||b.get("stroke");for(f=0;f<m;f++)r.radius=b.params.radii[f],h=new fabric.Circle(r),
n=Math.max(n,b.params.radii[f]),b.add(h);b.scaleX=p;b.scaleY=p;b.width=2*n;b.height=2*n;x===b&&u.setActiveObject(b)}break;case "box":d.width&&(b.scaleX=p);d.height&&(b.scaleY=p);break;case "circle":d.radius&&(b.scaleX=p,b.scaleY=p);break;case "cross":r=$.extend(!0,{},d);r.stroke=r.stroke||b.get("stroke");r.width&&(b.scaleX=p,t[0]=[{x:-r.width/2,y:0},{x:r.width/2,y:0}],delete r.width);r.height&&(b.scaleY=p,t[1]=[{x:0,y:-r.height/2},{x:0,y:r.height/2}],delete r.height);r.angle&&delete r.angle;b.forEachObject(function(a,
...
"moving");a.Fabric.updateChildren(l.dlayer,b,"scaling");a.Fabric.updateChildren(l.dlayer,b,"rotating");a.Fabric.updateChildren(l.dlayer,b,"deltas");b.setCoords();e._handleChildText(c,b,d);e._updateShape(c,b,B,"update");if(d.onchangeshapes&&b.pub)try{a.xeqByName(d.onchangeshapes,e,e,b.pub)}catch(z){a.error("in onchangeshapes callback",z,!1)}});(void 0===d.redraw||d.redraw)&&u.renderAll();return this}}};a.Fabric.refreshShapes=function(c){var b;if(c){var d=this.getWCSSys();if("image"===d){var e=a.globalOpts.xeqPlugins;
a.globalOpts.xeqPlugins=!1;this.raw.wcs&&0<this.raw.wcs?this.setWCSSys("native",!1):this.setWCSSys("physical",!1)}if(b=this.listRegions("all",{mode:1,sticky:!1,savewcsconfig:!0,saveid:!0},c))this.removeShapes(c,"all",{overrideRemovable:!0,sticky:!1}),this.addShapes(c,b,{restoreid:!0});"image"===d&&(this.setWCSSys(d,!1),this.updateShapes(c,"all","refresh"),a.globalOpts.xeqPlugins=e);return this}};a.Fabric.copyShapes=function(c,b,d){var e,f=[];if("object"===typeof b)f.push(b);else if("all"===b)for(b=
...
l.x;var w=g-l.y;v=v*r+w*k;r=l.x+r*v;l=l.y+k*v;r<m?r=m:r>p&&(r=p);l<n?l=n:l>q&&(l=q);m=(r-d)*(r-d)+(l-g)*(l-g);if(m<e){e=m;var t=r;var u=l;var x=h===f.length?0:h}}c=this.getShapeLayer(c);a.Fabric.removePolygonAnchors(c.dlayer,b);f.splice(x+1,0,{x:t,y:u});if(1===fabric.major_version)c.canvas.setActiveObject(b);else{switch(b.type){case "polyline":case "polygon":a.Fabric.addPolygonAnchors(c.dlayer,b),c.dlayer.canvas.renderAll()}b.polyparams?c.dlayer.params.sel=b.polyparams.polygon:b.params&&(c.dlayer.params.sel=
b)}}};a.Fabric._removePolygonPoint=function(c,b){if(b&&b.polyparams){var d=b.polyparams.polygon;var e=d.points;if(!("polygon"===d.type&&3>=e.length||"polyline"===d.type&&2>=e.length)){var f=b.polyparams.point;delete b.polyparams.point;c=this.getShapeLayer(c);a.Fabric.removePolygonAnchors(c.dlayer,d);e.splice(f,1);a.resetPolygonCenter(d);c.canvas.setActiveObject(d)}}};a.Fabric.addPolygonAnchors=function(c,b){var d,e={},f=c.canvas,h=function(b){b=b.target;var d=b.polyparams.polygon,g=b.polyparams.point,
h=d.get("points"),k=c.display.image;void 0!==g&&!1!==d.params.changeable&&(d.angle?e=a.rotatePoint({x:b.left,y:b.top},-d.angle,{x:d.left,y:d.top}):(e.x=b.left,e.y=b.top),h[g].x=(e.x-d.left)/d.scaleX,h[g].y=(e.y-d.top)/d.scaleY,a.resetPolygonCenter(d),k._updateShape(d.params.layerName,d,null,"update"),k&&(k.params.listonchange||d.params.listonchange)&&k.listRegions(d,{mode:2}),f.renderAll())},g=function(b){var c,d={};for(c=0;c<b.params.anchors.length;c++)d.x=b.left+b.points[c].x*b.scaleX,d.y=b.top+
b.points[c].y*b.scaleY,b.angle&&(d=a.rotatePoint(d,b.angle,{x:b.left,y:b.top})),b.params.anchors[c].set({left:d.x,top:d.y,angle:b.angle}),b.params.anchors[c].setCoords();b._calcDimensions(!0);f.renderAll()};if(!b.params.anchors&&!1!==b.params.changeable){b.params.anchors=[];for(d=0;d<b.points.length;d++){e.x=b.left+b.points[d].x*b.scaleX;e.y=b.top+b.points[d].y*b.scaleY;b.angle&&(e=a.rotatePoint(e,b.angle,b.getCenterPoint()));var k=new fabric.Rect({left:e.x,top:e.y,hasControls:!1,hasRotatingPoint:!1,
hasBorders:!1,selectable:!0,fill:b.get("stroke"),hoverCursor:"pointer",width:a.Fabric.opts.cornerSize+2,height:a.Fabric.opts.cornerSize+2,padding:2});k.on("moving",h);k.polyparams={};k.polyparams.polygon=b;k.polyparams.point=d;b.params.anchors[d]=k;f.add(k)}b.on("moving",function(){g(b)});b.on("rotating",function(){g(b)});b.on("scaling",function(){g(b)});b.setCoords()}};a.Fabric.removePolygonAnchors=function(a,b){var c=a.canvas;if(b&&b.params&&b.params.anchors&&!1!==b.params.changeable){for(a=0;a<
b.params.anchors.length;a++)c.remove(b.params.anchors[a]);delete b.params.anchors}};a.Fabric.updateChildren=function(c,b,d){var e,f;if("regions"===c.layerName)if("objects"===d){var h={};c.canvas.getObjects().forEach(function(a){a.params&&(a.params.parent||a.params.children.length)&&(h[a.params.id]=a)});c.canvas.getObjects().forEach(function(a){if(a.params)for(a.params.parent&&(a.params.parent.obj=h[a.params.parent.id]),e=0;e<a.params.children.length;e++)a.params.children[e].obj=h[a.params.children[e].id]})}else if(b&&
b.params)if("deltas"===d){if(b.params.parent){var g=b.params.parent;g.dleft=g.obj.left-b.left;g.dtop=g.obj.top-b.top;g.moved=!0}}else for(e=0;e<b.params.children.length;e++){var k=b.params.children[e].obj;g=k.params.parent;switch(d){case "moving":k.left=b.left-g.dleft;k.top=b.top-g.dtop;break;case "rotating":for(f=b.angle;0>f;)f+=360;for(;360<=f;)f-=360;var l=f-g.lastangle;var m=a.rotatePoint({x:k.left,y:k.top},l,{x:b.left,y:b.top});k.left=m.x;k.top=m.y;g.dleft=b.left-k.left;g.dtop=b.top-k.top;for(k.angle+=
l;0>k.angle;)k.angle+=360;for(;360<=k.angle;)k.angle-=360;g.lastangle=f;break;case "scaling":g.dleft*=b.scaleX/g.lastscalex,g.dtop=b.scaleY/g.lastscaley*(g.dtop-g.textheight)+g.textheight,g.lastscalex=b.scaleX,g.lastscaley=b.scaleY,g.moved=!0,k.left=b.left-g.dleft,k.top=b.top-g.dtop}k.setCoords();c.display.image&&c.display.image.updateShapes(c.layerName,k,"updatechild")}};a.resetPolygonCenter=function(c){var b={};if(1===fabric.major_version){c._calcDimensions();var d=(c.minX+c.width/2)*c.scaleX;var e=
...
c.height/2});c.setCoords()};a.Fabric.print=function(c){var b,d,e,f=0,h=sprintf("width=%s,height=%s,menubar=1,toolbar=1,status=0,scrollbars=1,resizable=1",this.display.canvasjq.attr("width"),this.display.canvasjq.attr("height"));c=c||{};if("string"===typeof c)try{c=JSON.parse(c)}catch(m){a.error("can't parse print opts: "+c,m)}var g=this.display.canvas.toDataURL("image/png");var k="<html><body style='padding: 0px; margin: 0px' onload='window.print(); return false'>";var l=sprintf("<div style='position:absolute; left:%spx; top:%spx'>",
0,f);k+=l+"<img src='"+g+"'></div>";for(b in this.layers)this.layers.hasOwnProperty(b)&&(g=this.layers[b],"main"===g.dlayer.dtype&&g.show&&(k+=""+l+g.dlayer.canvas.toSVG()+"</div>"));(void 0===c.colorbar||c.colorbar)&&(c=this.display.pluginInstances.JS9Colorbar)&&c.isActive()&&(f+=2,g=c.colorbarjq[0].toDataURL("image/png"),f+=this.display.height,l=sprintf("<div style='position:absolute; left:%spx; top:%spx'>",0,f),k+=l+"<img src='"+g+"'></div>",c.textjq&&c.textjq[0]&&(g=c.textjq[0].toDataURL("image/png"),
f+=c.colorbarjq.height()+1,l=sprintf("<div style='position:absolute; left:%spx; top:%spx'>",0,f),k+=l+"<img style='width:"+this.display.width+"px;'src='"+g+"'></div>"));k+="</body></html>";window.isElectron&&(e="data:text/html,<html><body><script>window.addEventListener('message', (ev) => {document.documentElement.innerHTML=ev.data; window.setTimeout(() => {window.print()}, 250);},false)\x3c/script><p>waiting for image ...</body></html>");(d=window.open(e,this.id,h))?d.document?(d.document.open(),
...
a.Image.prototype._exportShapeOptions=a.Fabric._exportShapeOptions;a.Image.prototype._handleChildText=a.Fabric._handleChildText;a.Image.prototype._addPolygonPoint=a.Fabric._addPolygonPoint;a.Image.prototype._removePolygonPoint=a.Fabric._removePolygonPoint;a.Image.prototype._updateMultiDialogs=a.Fabric._updateMultiDialogs;a.Image.prototype.addShapes=a.Fabric.addShapes;a.Image.prototype.updateShapes=a.Fabric.updateShapes;a.Image.prototype.getShapes=a.Fabric.getShapes;a.Image.prototype.changeShapes=
a.Fabric.changeShapes;a.Image.prototype.removeShapes=a.Fabric.removeShapes;a.Image.prototype.refreshShapes=a.Fabric.refreshShapes;a.Image.prototype.copyShapes=a.Fabric.copyShapes;a.Image.prototype.selectShapes=a.Fabric.selectShapes;a.Image.prototype.getShapeLayer=a.Fabric.getShapeLayer;a.Image.prototype.showShapeLayer=a.Fabric.showShapeLayer;a.Image.prototype.activeShapeLayer=a.Fabric.activeShapeLayer;a.Image.prototype.displayShapeLayers=a.Fabric.displayShapeLayers;a.Image.prototype.toggleShapeLayers=
a.Fabric.toggleShapeLayers;a.Image.prototype.print=a.Fabric.print;for(c in a.Fabric.opts)a.Fabric.opts.hasOwnProperty(c)&&(fabric.Object.prototype[c]=a.Fabric.opts[c])};a.Fabric.initGraphics();a.MouseTouch={};a.MouseTouch.CLASS="JS9";a.MouseTouch.NAME="MouseTouch";a.MouseTouch.WIDTH=512;a.MouseTouch.HEIGHT=220;a.MouseTouch.BASE=a.MouseTouch.CLASS+a.MouseTouch.NAME;a.MouseTouch.mouseText=[];a.MouseTouch.mouseText[0]="Move mouse, no buttons pressed:";a.MouseTouch.mouseText[1]="Move mouse, primary button pressed:";
...
b){b=sprintf(a.MouseTouch.textHTML,b);return $("<div>").addClass(a.MouseTouch.BASE+"Text").html(b).appendTo(c)};a.MouseTouch.addAction=function(c,b,d){b=a.MouseTouch.actionid(b,d);d=sprintf(a.MouseTouch.actionHTML,d);return $("<div>").addClass(a.MouseTouch.BASE+"Action").attr("id",b).html(d).appendTo(c)};a.MouseTouch.isPinch=function(c,b){var d,e,f=a.globalOpts.pinchWait,h=a.globalOpts.pinchThresh;if(!c)return-1;b=c.display;if(!b.mousetouchZoom||2!==c.pos.touches.length)return-1;switch(b.ispinch){case -1:case 1:return b.ispinch}c=
...
n=!1,p=this.raw.wcsinfo||{cdelt1:1,cdelt2:1};var q={type:"multi",pub:{shape:"multi",wcsconfig:{}},params:{}};var r=function(a){if(void 0!==a)return"number"===typeof a&&0!==a%1&&(a=Math.round(1E4*(a+1E-5))/1E4),String(a)},v=function(a){return(a||"").replace(/\n/g,"\\n")};if(c&&c.pub&&c.pub.wcsconfig&&c.pub.wcsconfig.wcssys)var w=c.pub.wcsconfig.wcssys;else w=this.params.wcssys,c=q;b=b||{};if(c.params.winid)var t=c.params.winid;else b.winid&&(t=b.winid);if(t){var u="#"+$(t).attr("id")+" .regionsConfigForm ";
if($(u).length){n=$(u).data("multi")?!0:b.multi;$(u+"."+c.pub.shape).each(function(a,b){$(b).removeClass("nodisplay")});$(u+".val").each(function(b,m){h="";f=$(m).attr("name");switch(f){case "x":case "y":c.pub.lcs&&void 0!==c.pub.lcs[f]?h=r(c.pub.lcs[f]):void 0!==c.pub[f]&&(h=r(c.pub[f]));break;case "radii":c.pub.radii&&(h=a.notWCS(w)||!c.pub.wcsconfig.wcsstr?c.pub.imstr.replace(/^annulus\(/,"").replace(/\)$/,"").split(",").slice(2).join(","):c.pub.wcsconfig.wcsstr.replace(/^annulus\(/,"").replace(/\)$/,
"").split(",").slice(2).join(","));break;case "pts":c.pub.pts?c.pub.pts.forEach(function(a){h&&(h+=", ");h+=a.x.toFixed(2)+", "+a.y.toFixed(2)}):c.pub.imstr&&(h=c.pub.imstr.replace(/^.*\(/,"").replace(/\)$/,""));break;case "linelength":if(c.pub.pts&&2===c.pub.pts.length){k=c.pub.pts[0];l=c.pub.pts[1];h=r(Math.sqrt((l.x-k.x)*(l.x-k.x)+(l.y-k.y)*(l.y-k.y)));switch(w){case "image":case "physical":break;default:h*=Math.abs(p.cdelt1),h*=Math.abs(p.cdelt2)}h=r(h);d.tmp.linelength=h}break;case "lineangle":if(c.pub.pts&&
2===c.pub.pts.length){k=c.pub.pts[0];l=c.pub.pts[1];for(h=180*Math.atan2(l.y-k.y,l.x-k.x)/Math.PI;0>h;)h+=360;h=r(h);d.tmp.lineangle=h}break;case "fontFamily":c.getFontFamily&&(h=c.getFontFamily());break;case "fontSize":c.getFontSize&&(h=c.getFontSize());break;case "fontStyle":c.getFontStyle&&(h=c.getFontStyle());break;case "fontWeight":c.getFontWeight&&(h=c.getFontWeight());break;case "strokeWidth":c.params.sw1&&(h=c.params.sw1);break;case "colorPicker":void 0!==c.stroke&&(h=a.colorToHex(c.stroke));
break;case "strokeDashes":c.strokeDashArray&&(h=c.strokeDashArray.join(" "),h.match(/NaN/)&&(h=""));break;case "regstr":h=a.notWCS(w)||!c.pub.wcsconfig.wcsstr?c.pub.imsys+";"+c.pub.imstr:c.pub.wcsconfig.wcssys+";"+c.pub.wcsconfig.wcsstr;break;case "xpos":switch(w){case "image":c.pub.preservedcoords&&void 0!==c.pub.dx?h=sprintf("d%.1f",c.pub.dx):void 0!==c.pub.x&&(h=sprintf("%.1f",c.pub.x));break;case "physical":c.pub.lcs?h=sprintf("%.1f",c.pub.lcs.x):void 0!==c.pub.x&&(h=sprintf("%.1f",c.pub.x));
break;default:void 0!==c.pub.wcsconfig.ra?h=sprintf("%.6f",c.pub.wcsconfig.ra):void 0!==c.pub.x&&(h=sprintf("%.1f",c.pub.x))}break;case "ypos":switch(w){case "image":c.pub.preservedcoords&&void 0!==c.pub.dy?h=sprintf("d%.1f",c.pub.dy):void 0!==c.pub.y&&(h=sprintf("%.1f",c.pub.y));break;case "physical":c.pub.lcs?h=sprintf("%.1f",c.pub.lcs.y):void 0!==c.pub.y&&(h=sprintf("%.1f",c.pub.y));break;default:void 0!==c.pub.wcsconfig.dec?h=sprintf("%.6f",c.pub.wcsconfig.dec):void 0!==c.pub.y&&(h=sprintf("%.1f",
c.pub.y))}break;case "radius":case "oradius":case "length":case "width":case "r1":switch(w){case "image":void 0!==c.pub[f]&&(h=r(c.pub[f]));break;case "physical":c.pub.lcs&&void 0!==c.pub.lcs[f]&&(h=r(c.pub.lcs[f]));break;default:void 0!==c.pub.wcsconfig.wcssizestr?h=r(c.pub.wcsconfig.wcssizestr[0]):void 0!==c.pub[f]&&(h=r(c.pub[f]))}break;case "height":case "r2":switch(w){case "image":void 0!==c.pub[f]&&(h=r(c.pub[f]));break;case "physical":c.pub.lcs&&void 0!==c.pub.lcs[f]&&(h=r(c.pub.lcs[f]));break;
default:void 0!==c.pub.wcsconfig.wcssizestr?h=r(c.pub.wcsconfig.wcssizestr[1]):void 0!==c.pub[f]&&(h=r(c.pub[f]))}break;case "wcssys":case "savewcs":x=$(u).find("[name='"+f+"']");if(!x.find("option").length)for(e=0;e<a.wcssyss.length;e++)x.append("<option>"+a.wcssyss[e]+"</option>");g="savewcs"===f?a.globalOpts.regSaveWCS||w:w;x.find("option").each(function(a,b){g===b.value&&(h=b.value)});break;case "wcsunits":c.pub.wcsunits&&(h=c.pub.wcsunits);break;case "childtext":c.params.children&&0<c.params.children.length&&
(h=v(c.params.children[0].obj.text));break;case "text":void 0!==c.pub[f]&&(h=v(r(c.pub[f])));break;case "id":n?h="selected":void 0!==c.pub.id&&(h=String(c.pub.id),$(m).css("width",h.length+"ch"));break;case "tags":void 0!==c.pub[f]&&(h=r(c.pub[f]));break;case "savefile":h=$(u).data("savefile")||d.tmp.saveregionsFile||"js9.reg";window.isElectron&&window.currentDir&&!h.match(/.*\//)&&(h=window.currentDir+"/"+h);break;case "selectfilter":h=$(u).data("selectfilter");break;default:void 0!==c.pub[f]&&(h=
r(c.pub[f]))}$(m).val(h)});(n||!this.raw.wcs||0>this.raw.wcs)&&$(u).find("[name='wcssys']").hide();"text"!==c.type&&c.params.children&&($(u+".childtext").removeClass("nodisplay"),$(u+"[name='childtext']").prop("readonly",0<c.params.children.length));if(b.firsttime){window.isElectron&&$(u).find(".rsavebrowse, .rconfigbrowse").removeClass("nodisplay");n&&($(u).find("label[for='savecur']").text("sel"),$(u).find("input[id='savecur']").data("tooltip","save selected regions"));if(a.favorites.wcs&&a.favorites.wcs.length){var x=
$(u).find(".rwcsbuttons").removeClass("nodisplay");q=x.find(".rwcsbuttoncontainer");if(q.length&&!q.find(".rwcsbutton").length){for(e=0;e<a.favorites.wcs.length;e++){var y=a.favorites.wcs[e];var B="string"===typeof y?y.split(":"):y;y=B[0];B=B[1]||y;if("save"===b.type){var z="rsavecol_R"+(e+2);var A="rsaveradio"}else z="rconfigcol_R"+(e+2),A="rconfigradio";q.append("<span class='rconfigcol_R rwcsbutton "+z+"'>\n                                <input type='radio'\n                                       id='rwcsbutton_"+
y+"'\n                                       name='rwcsbutton'\n                                       class='rwcsradio "+A+"}'\n                                       value='"+y+"'\n                                       data-tooltip='save using "+y+' wcs\'\n                                       onclick=\'\n                                           $(this).closest("form")\n                                           .find("[name=savewcs]")\n                                           .val("'+y+'")\n                                           .trigger("change");\'>\n                                <label for=\'rwcsbutton_'+
y+"'>"+B+"</label>\n                                </span>")}$(u).find(".rwcsbuttons").find("[value='"+w+"']").prop("checked",!0)}}a.globalOpts.internalColorPicker&&$.fn.spectrum.inputTypeColorSupport()||(x=$(u).find("input[name='colorPicker']"),x.spectrum({showButtons:!1,showInput:!1,preferredFormat:"hex6"}),x.on("move.spectrum",function(a,b){$(u).find("input[name='color']").val(b.toHexString())}))}void 0===c.params.listonchange&&(c.params.listonchange=!1);c.params.listonchange?$(u+"[name='listonchange']").prop("checked",
!0):$(u+"[name='listonchange']").prop("checked",!1);void 0===c.params.changeable&&void 0!==c.params.fixinplace&&(c.params.changeable=!c.params.fixinplace);void 0===c.params.changeable&&(c.params.changeable=!0);c.params.changeable?$(u+"[name='changeable']").prop("checked",!0):$(u+"[name='changeable']").prop("checked",!1);c.params.sticky?$(u+"[name='sticky']").prop("checked",!0):$(u+"[name='sticky']").prop("checked",!1);$(u+"[id='includejson']").prop("checked",a.globalOpts.regIncludeJSON);$(u+"[id='includecomments']").prop("checked",
a.globalOpts.regIncludeComments);$(u+"[id='savedcoords']").prop("checked",a.globalOpts.regSaveDCoords);$(u+"[id='includewcs']").prop("checked",a.globalOpts.csvIncludeWCS);$(u).find("input[name='saveformat']").prop("checked",!1);$(u).find("input[value='"+a.globalOpts.regSaveFormat+"']").prop("checked",!0);$(u).find("input[name='rwcsbutton']").prop("checked",!1);$(u).find("input[value='"+(a.globalOpts.regSaveWCS||w)+"']").prop("checked",!0);y="save"===b.type?"save"+a.globalOpts.regSaveWhich1:"save"+
a.globalOpts.regSaveWhich2;$(u+"[id='"+y+"']").prop("checked",!0);"save"===b.type&&$(u).find("input[name='savefile']").trigger("change");$(u).find("input[name='strokeMenu']").prop("selectedIndex",0);$(u).find("input[name='dashesMenu']").prop("selectedIndex",0);if(n)$(u).find(".regid").hide(),$(u).find(".edit").hide(),$(u).find(".childtext").hide(),$(u).find(".multi").removeClass("nodisplay"),0>=b.setmode?($(u).find("[name='multitext']").val(""),0>b.setmode&&($(u).data("selectfilter",""),$(u).find("[name='selectfilter']").val(""))):
(y=this.listRegions("selected",{mode:1,includejson:!1,includecomments:!1}).replace(/ *; */g,"\n"),$(u).find("[name='multitext']").val(y));else switch($(u).find("input:text[readonly]").css("border-color","A5A5A5").css("background","#E9E9E9"),c.pub.shape){case "box":case "cross":case "ellipse":$(u+".angle").removeClass("nodisplay");break;case "text":$(u+".textangle").removeClass("nodisplay");break;case "line":c.pub.pts&&2===c.pub.pts.length?($(u+".linelength").removeClass("nodisplay"),$(u+".lineangle").removeClass("nodisplay")):
($(u+".linelength").addClass("nodisplay"),$(u+".lineangle").addClass("nodisplay"))}$(u+".xtrareg").addClass("nodisplay");$(u+".xtracsv").addClass("nodisplay");$(u+".xtrasvg").addClass("nodisplay");$(u+".xtra"+a.globalOpts.regSaveFormat).removeClass("nodisplay");$(u).data("im",this);$(u).data("shape",c);$(u).data("winid",t);$(u).data("multi",n);a.BROWSER[3]?(t="touchstart",q="touchend"):(t="mouseover",q="mouseout");if("save"===b.type)$(u).on(t,function(){$(u).find("input[name='savefile']").focus()});
$(u).data("tooltipInit")||($(u).data("tooltipInit",!0),$(".rconfigcol_R, .rsavecol_R").on(t,function(b){var c=b.currentTarget;b=$(c).find("input, textarea, span").data("tooltip");c=$(c).closest(a.lightOpts[a.LIGHTWIN].top).find(a.lightOpts[a.LIGHTWIN].dragBar);b&&c.length&&(m=$(c)[0].childNodes[0].nodeValue.replace(/:.*/,""),$(c)[0].childNodes[0].nodeValue=m+": "+b)}),$(".rconfigcol_R, .rsavecol_R").on(q,function(b){b=$(b.currentTarget).closest(a.lightOpts[a.LIGHTWIN].top).find(a.lightOpts[a.LIGHTWIN].dragBar);
...
c)return b.params.children&&0<b.params.children.length?!1:""!==d;if("strokeDashes"===c)return b.strokeDashArray?JSON.stringify(b.strokeDashArray)!==JSON.stringify(d):""!==d;if("tags"!==c&&""===d)return!1;if("misc"===c&&""!==d)return!0;if("angle"===c)return b.angle!==-parseFloat(d);if("ix"===c)return b.pub.preservedcoords&&"d"===d.charAt(0).toLowerCase()?m(b.pub.dx,a.saostrtod(d.substring(1))):m(b.pub.x,a.saostrtod(d));if("iy"===c)return b.pub.preservedcoords&&"d"===d.charAt(0).toLowerCase()?m(b.pub.dy,
a.saostrtod(d.substring(1))):m(b.pub.y,a.saostrtod(d));if("px"===c&&b.pub.lcs)return m(b.pub.lcs.x,a.saostrtod(d));if("py"===c&&b.pub.lcs)return m(b.pub.lcs.y,a.saostrtod(d));if("ra"===c)return b.pub.wcsconfig.wcsposstr?m(a.saostrtod(b.pub.wcsconfig.wcsposstr[0]),a.saostrtod(d)):b.pub.wcsposstr?m(a.saostrtod(b.pub.wcsposstr[0]),a.saostrtod(d)):!1;if("dec"===c){if(b.pub.wcsconfig.wcsposstr)return m(a.saostrtod(b.pub.wcsconfig.wcsposstr[1]),a.saostrtod(d));if(b.pub.wcsposstr)return m(a.saostrtod(b.pub.wcsposstr[1]),
a.saostrtod(d))}return"sticky"===c?m(b.pub.sticky||!1,d):b.pub.lcs&&void 0!==b.pub.lcs[c]?m(b.pub.lcs[c],d)?!0:!1:m(b.pub[c],d)||m(b.params[c],d)||m(b[c],d)?!0:!1},p=function(b){return"true"===b?!0:"false"===b?!1:a.isNumber(b)?parseFloat(b):b},q=function(a){var b=String.fromCharCode(13,10);return(a||"").replace(/\\n/g,b)};this.lcs&&this.lcs.physical&&(e=Math.sqrt(Math.pow(this.lcs.physical.forward[0][0],2)+Math.pow(this.lcs.physical.forward[0][1],2)));if(b&&b.pub&&b.pub.wcsconfig&&b.pub.wcsconfig.wcssys)var r=
b.pub.wcsconfig.wcssys;else r=this.params.wcssys,b=f;var v=$(c).data("multi");var w=b.pub.layer||"regions";for(f=0;f<h;f++){var t=d[f].name;var u=d[f].value;if("xpos"===t||"ypos"===t)switch(r){case "image":t="i"+t.charAt(0);break;case "physical":t="p"+t.charAt(0);break;default:t=this.raw.wcs&&0<this.raw.wcs?"xpos"===t?"ra":"dec":"xpos"===t?"ix":"iy"}switch(t){case "text":"text"===b.type&&n(b,t,u)&&(g[t]=q(p(u)));break;case "strokeDashes":u=u.trim().split(/\s+/);n(b,t,u)&&(g.strokeDashArray=0===u.length?
[]:u.map(function(a){return parseInt(a,10)}));break;case "childtext":"text"!==b.type&&n(b,t,u)&&(g.text=q(p(u)));break;case "ix":n(b,t,u)&&(b.pub.preservedcoords&&"d"===u.charAt(0).toLowerCase()?(g.dx=p(u.substring(1)),void 0===g.dy&&void 0!==b.pub.dy&&(g.dy=b.pub.dy)):(g.x=p(u),void 0===g.y&&void 0!==b.pub.y&&(g.y=b.pub.y)));break;case "iy":n(b,t,u)&&(b.pub.preservedcoords&&"d"===u.charAt(0).toLowerCase()?(g.dy=p(u.substring(1)),void 0===g.dx&&void 0!==b.pub.dx&&(g.dx=b.pub.dx)):(g.y=p(u),void 0===
g.x&&void 0!==b.pub.x&&(g.x=b.pub.x)));break;case "px":n(b,t,u)&&(g.px=p(u),void 0===g.py&&b.pub.lcs&&(g.py=b.pub.lcs.y));break;case "py":n(b,t,u)&&(g.py=p(u),void 0===g.px&&b.pub.lcs&&(g.px=b.pub.lcs.x));break;case "ra":n(b,t,u)&&(g.ra=u,void 0===g.dec&&(b.pub.wcsconfig.wcsposstr?g.dec=b.pub.wcsconfig.wcsposstr[1]:b.pub.wcsposstr&&(g.dec=b.pub.wcsposstr[1])));break;case "dec":n(b,t,u)&&(g.dec=u,void 0===g.ra&&(b.pub.wcsconfig.wcsposstr?g.ra=b.pub.wcsconfig.wcsposstr[0]:b.pub.wcsposstr&&(g.ra=b.pub.wcsposstr[0])),
void 0===g.wcssys&&(g.wcssys=r));break;case "wcssys":break;case "radius":case "length":case "width":case "r1":switch(r){case "image":n(b,t,u)&&(g[t]=p(u));break;case "physical":n(b,t,u)&&(g[t]=p(u)*e);break;default:u=a.strtoscaled(u),u="."===u.dtype&&"galactic"!==r&&"ecliptic"!==r?u.dval:Math.abs(u.dval/k.cdelt1),t=t.replace("wcs",""),n(b,t,u)&&(g[t]=p(u))}break;case "height":case "r2":switch(r){case "image":n(b,t,u)&&(g[t]=p(u));break;case "physical":n(b,t,u)&&(g[t]=p(u)*e);break;default:u=a.strtoscaled(u),
u="."===u.dtype&&"galactic"!==r&&"ecliptic"!==r?u.dval:Math.abs(u.dval/k.cdelt2),t=t.replace("wcs",""),n(b,t,u)&&(g[t]=p(u))}break;case "radii":n(b,t,u)&&(g[t]=u);break;case "linelength":if(b.pub.pts&&2===b.pub.pts.length&&a.isNumber(u)&&u!==this.tmp.linelength){u=parseFloat(u);switch(r){case "image":case "physical":break;default:void 0!==k.cdelt1?u/=Math.abs(k.cdelt1):void 0!==k.cdelt2&&(u/=Math.abs(k.cdelt2))}if(g.pts){t=g.pts[0];var x=g.pts[1]}else t=b.pub.pts[0],x=b.pub.pts[1];if(0<=$.inArray("line",
a.Regions.opts.noCenteredScaling)){var y=Math.sqrt((t.x-x.x)*(t.x-x.x)+(t.y-x.y)*(t.y-x.y));var B=t.x-u*(t.x-x.x)/y;u=t.y-u*(t.y-x.y)/y;g.pts=[t,{x:B,y:u}]}else B={x:(t.x+x.x)/2,y:(t.y+x.y)/2},y=parseFloat(this.tmp.lineangle)||0,t.x=B.x-u/2,t.y=B.y,x.x=B.x+u/2,x.y=B.y,g.pts=[a.rotatePoint(t,y,B),a.rotatePoint(x,y,B)]}break;case "lineangle":b.pub.pts&&2===b.pub.pts.length&&a.isNumber(u)&&u!==this.tmp.lineangle&&(y=parseFloat(u)-parseFloat(this.tmp.lineangle)||0,g.pts?(t=g.pts[0],x=g.pts[1]):(t=b.pub.pts[0],
x=b.pub.pts[1]),0<=$.inArray("line",a.Regions.opts.noCenteredScaling)?g.pts=[t,a.rotatePoint(x,y,t)]:(B={x:(t.x+x.x)/2,y:(t.y+x.y)/2},g.pts=[a.rotatePoint(t,y,B),a.rotatePoint(x,y,B)]));break;case "remove":n(b,t,u)&&(v?g[t]="selected":void 0!==b.pub.id&&(g[t]=b.pub.id));break;case "misc":if(u.trim())try{var z=JSON.parse(u);$.extend(g,z)}catch(A){a.error("invalid json: "+u)}break;case "selectfilter":u&&($(c).data("selectfilter",u),this.selectShapes(w,u));break;default:n(b,t,u)&&(g[t]=p(u))}}v?this.changeShapes(w,
"selected",g):this.changeShapes(w,b,g);this.initRegionsForm(b,{multi:v})};a.Regions.pasteFromClipboard=function(c){var b,d=0,e=0;if(this){(b=a.CopyFromClipboard().trim())||a.error(a.CLIPBOARDERROR);if(b.match(/(annulus|box|circle|cross|ellipse|line|polygon|point|text) *\(/)){var f=a.globalOpts.regToClipboard;a.globalOpts.regToClipboard=!1;var h=this.addShapes("regions",b,{rtn:"objs"});if(c){var g=h.length;for(c=0;c<g;c++)d+=h[c].pub.x,e+=h[c].pub.y;d/=g;e/=g;for(c=0;c<g;c++){var k=h[c].pub.x-d+this.ipos.x;
var l=h[c].pub.y-e+this.ipos.y;this.changeShapes("regions",h[c].pub.id,{x:k,y:l})}}a.globalOpts.regToClipboard=f}else a.error(a.CLIPBOARDERROR2);return b}};a.Regions.listRegions=function(c,b,d){var e=this,f,h,g,k,l="";var m="none";var n=!1,p=[];var q={};var r=[],v=[],w=function(c,d){var f,g={},h=c.params,k=h.children,l=h.exports;for(f=0;f<l.length;f++){var m=l[f];if(("text"!==m||"text"===c.type)&&"textOpts"!==m){if("strokeDashArray"===m&&c.strokeDashArray){var n=c.strokeDashArray.join("");if(""===
n||n.match(/NaN/))continue}"id"===m&&b.file||"wcsconfig"===m&&b.file||"data"===m&&"object"===typeof h.data&&!1===h.data.doexport&&b.file||("strokeWidth"===m&&h.sw1?g[m]=h.sw1:void 0!==c[m]?g[m]=c[m]:void 0!==h[m]?g[m]=h[m]:d&&void 0!==d[m]&&(g[m]=d[m]))}}if(0<k.length&&k[0].obj.text){n=k[0].obj;g.text=n.text;g.textOpts=w(n);if(c.angle!==n.angle){if(g.textOpts.angle=-n.angle,"circle"===c.params.shape||"annulus"===c.params.shape)n.params.hasTextOpts=!0}else 0===n.angle||"circle"!==c.params.shape&&"annulus"!==
c.params.shape||(g.textOpts.angle=-n.angle,n.params.hasTextOpts=!0);if(n.params.parent.moved||n.params.hasTextOpts)n.pub.ra&&n.pub.dec?(x&&y&&x!==y?(n=e.wcs2wcs(x,y,n.pub.ra,n.pub.dec),n=n.trim().split(/\s+/),d=a.saostrtod(n[0]),a.isHMS(y)&&(d*=15),n=a.saostrtod(n[1])):(d=n.pub.ra,n=n.pub.dec),g.textOpts.ra=d,g.textOpts.dec=n):n.pub.lcs?(g.textOpts.px=n.pub.lcs.x,g.textOpts.py=n.pub.lcs.y):(g.textOpts.x=n.pub.x,g.textOpts.y=n.pub.y);g.textOpts.color===c.stroke&&delete g.textOpts.color;g.textOpts.text&&
delete g.textOpts.text;Object.keys(g.textOpts).length||delete g.textOpts}return g};b=b||{};if("string"===typeof b)try{b=JSON.parse(b)}catch(E){a.error("can't parse listRegions opts: "+b,E)}var t=b.mode;a.isNull(t)&&(t=3);a.isNull(d)&&(d="regions");if(b.wcssys||b.wcsunits){var u=a.globalOpts.xeqPlugins;a.globalOpts.xeqPlugins=!1;if(b.wcssys){var x=this.getWCSSys();this.setWCSSys(b.wcssys,!1);var y=this.getWCSSys()}if(b.wcsunits){var B=this.getWCSUnits();this.setWCSUnits(b.wcsunits,!1)}this.updateShapes(d,
c,"export")}a.isNull(b.includedcoords)&&(b.includedcoords=a.globalOpts.regListDCoords);p=this.getShapes(d,c,{includeObj:!0});var z=p.length;if(t)for(f=0;f<z;f++){var A=p[f];if((h=A.tags.join(","))&&"source,include"!==h&&"include,source"!==h){n=!0;break}}for(C in a.Regions.opts.tagcolors)a.Regions.opts.tagcolors.hasOwnProperty(C)&&v.push(a.Regions.opts.tagcolors[C]);for(f=0;f<z;f++)if(A=p[f],q=A.obj,r=[],!A.sticky||!1!==b.sticky)if(!A.preservedcoords||b.includedcoords)if($.isArray(q.params.preservedcoords)){for(h=
...
(l=l.replace(/ *#[^;]*/g,""));if(x||B)x&&this.setWCSSys(x,!1),B&&this.setWCSUnits(B,!1),this.updateShapes(d,c,"export"),a.globalOpts.xeqPlugins=u;1<t&&this.display.displayMessage("regions",l);return l};a.Regions.copyRegions=function(a,b){return this.copyShapes("regions",a,b)};a.Regions.parseRegions=function(c,b){var d=this,e,f,h=[],g=/^-?(annulus|box|circle|cross|ellipse|line|polygon|point|text)$/,k=/^(fk4|fk5|icrs|galactic|ecliptic|image|physical|linear)$/,l=/^(image|physical)$/,m=/[dr:]/,n=/\(\s*([^)]+?)\s*\)/,
p=/(\{.*\})/,q=/\s*,\s*/,r=/#(?![a-zA-Z0-9]{6}['"])/,v=function(a){return"0"===a||"false"===a.toLowerCase()?!1:!0},w=function(a){for(var b,c,d,e={},f=/([a-zA-Z][a-zA-Z0-9_]*)\s*=\s*(\d+(\s*\d+)*|[^ '"{]+|['"{][^'"}]*['"}])/g,g={color:function(a){return{color:a}},dash:function(a){if(a)return{strokeDashArray:[3,1]}},dashlist:function(a){if(a){var b=a.split(" ");for(a=0;a<b.length;a++)b[a]=parseFloat(b[a]);return{strokeDashArray:b}}},delete:function(a){return{removable:v(a)}},edit:function(a){return{selectable:v(a)}},
...
b.includes("{")?d.cmd=b.split("{")[0].trim().toLowerCase():b.includes("#")?d.cmd=b.split("#")[0].trim().toLowerCase():d.cmd=b.trim().toLowerCase();d.cmd&&(d.isregion=0<=d.cmd.search(g));var e=b.trim().split(r);if((c=p.exec(e[0]))&&c[0])try{d.opts=JSON.parse(c[0].trim())}catch(M){a.error("can't parse opts: "+c[0],M)}d.comment=e[1];if(d.comment){var f=w(d.comment.trim());void 0!==f._comment&&(d.comment=f._comment,delete f._comment)}f&&(d.opts=$.extend({},f,d.opts));(c=n.exec(b))&&c[0].match(p)?d.args=
[]:c&&c[1]&&(d.args=c[1].split(q));d.isregion&&d.cmd.startsWith("-")&&(d.cmd=d.cmd.slice(1),d.comment?d.comment.match(/exclude/)||(d.comment+=",exclude"):d.comment="exclude");return d},u=function(b,c){var e={};if("d"===b.charAt(0).toLowerCase()&&"d"===c.charAt(0).toLowerCase())return e.dx=parseFloat(b.substring(1)),e.dy=parseFloat(c.substring(1)),e;b=a.strtoscaled(b);c=a.strtoscaled(c);!b.dtype.match(m)&&!c.dtype.match(m)||z||y.match(l)||(C=!0,B=y);if(z||C)return a.isHMS(B,b.dtype)&&(b.dval*=15),
"r"===b.dtype&&(b.dval=180*b.dval/Math.PI),"r"===c.dtype&&(c.dval=180*c.dval/Math.PI),c=a.wcs2pix(d.raw.wcs,b.dval,c.dval).split(/ +/),e.x=parseFloat(c[0]),e.y=parseFloat(c[1]),e;if("physical"===B)return c=d.logicalToImagePos({x:b.dval,y:c.dval}),e.x=c.x,e.y=c.y,e;e.x=b.dval;e.y=c.dval;return e},x=function(b,c){b=a.strtoscaled(b);var e=d.raw.wcsinfo||{cdelt1:1,cdelt2:1};!b.dtype.match(m)||z||y.match(l)||(C=!0,B=y);z||C?("r"===b.dtype&&(b.dval=180*b.dval/Math.PI),b.dval=Math.abs(b.dval/e["cdelt"+c])):
...
{wcssys:B};2<=D&&"line"!==F.shape&&"polygon"!==F.shape&&$.extend(F,u(E.args[0],E.args[1]));F.textOpts&&void 0!==F.textOpts.ra&&void 0!==F.textOpts.dec&&(F.textOpts._wcssys=B);switch(E.cmd){case "annulus":if(0<D)for(F.radii=[],e=2;e<D;e++)F.radii.push(x(E.args[e],1));break;case "box":case "cross":3<=D&&(F.width=x(E.args[2],1));4<=D&&(F.height=x(E.args[3],2));5<=D&&(F.angle=a.strtoscaled(E.args[4]).dval);break;case "circle":3<=D&&(F.radius=x(E.args[2],1));break;case "ellipse":3<=D&&(F.r1=x(E.args[2],
1));4<=D&&(F.r2=x(E.args[3],2));5<=D&&(F.angle=a.strtoscaled(E.args[4]).dval);break;case "line":case "polygon":if(0<D)for(F.pts=[],f=e=0;e<D;e+=2,f++){var G=u(E.args[e],E.args[e+1]);a.notNull(G.dx)&&a.notNull(G.dy)?F.pts[f]={dx:G.dx,dy:G.dy}:F.pts[f]={x:G.x,y:G.y}}break;case "text":3<=D&&(F.text=E.args[2].replace(/^['"]/,"").replace(/["']$/,"")),4<=D&&(F.angle=a.strtoscaled(E.args[3]).dval)}E.comment&&(F.tags=E.comment);h.push(F)}else E.cmd.match(k)?(e=a.globalOpts.xeqPlugins,a.globalOpts.xeqPlugins=
!1,this.setWCSSys(E.cmd,!1),a.globalOpts.xeqPlugins=e,B=this.getWCSSys(),z=a.isWCS(B)):"remove"!==E.cmd&&"delete"!==E.cmd||h.push({remove:!0})}e=a.globalOpts.xeqPlugins;a.globalOpts.xeqPlugins=!1;this.setWCSSys(y,!1);this.setWCSUnits(b);a.globalOpts.xeqPlugins=e;return h};a.Regions.saveRegions=function(c,b,d){var e,f,h;c&&(f=c.match(/\.([^.]*)$/))&&f[1]&&f[1].match(/^(reg|svg|csv)$/)&&(e=f[1]);if("object"===typeof d){var g=d;d=null}else if(d&&"string"===typeof d){try{g=JSON.parse(d)}catch(p){g=null}g&&
...
a.saveAs(b,c):a.error("no saveAs() available to save region file");return this.tmp.saveregionsFile=c};a.Regions.unremoveRegions=function(){var a=this.regstack.pop();return a?this.addShapes("regions",a):null};a.Regions.changeRegionTags=function(a,b,d){var c;a=a||"all";b=b||[];d=d||[];$.isArray(b)||(b=b.split(",").map(function(a){return a.trim()}));$.isArray(d)||(d=d.split(",").map(function(a){return a.trim()}));var f=this.getShapes("regions",a);for(a=0;a<f.length;a++){var h=f[a].tags;var g=[];for(c=
...
y&&c.append($("<option>",{value:a.displays[b].id,text:a.displays[b].id}))}),B=a.lightWin(f,h,g,"Closing a light window",n.lcloseWin),$(B).data("dispid",y),$(B).data("winid",A),!1}};d=p.argv[0];var t=p.argv[1];var u=p.argv[2];b=p.argv[3];p=p.argv[4];if("object"===typeof t)t=$.extend(!0,{},t);else if(t&&"string"===typeof t)try{t=JSON.parse(t)}catch(E){t={}}else t={};u=u||"light";var x=(u||"")+"win";switch(u){case "light":if(t.id){var y=t.id;delete t.id}else y=x+a.uniqueID();var B="d"+y;if(b)p=p||n.imageWin;
else{var z=r(y,t,n.imageWin);b=z.html;p=p||z.winopts}z=sprintf("JS9 Display"+a.IDFMT,y);var A=a.lightWin(B,"inline",b,z,p);var C=v(d,t);A.onclose=function(){return w(C)};return y;case "new":t.id?(y=t.id,delete t.id):y=x+a.uniqueID();p=p||"width="+a.globalOpts.newWindowWidth+", height="+a.globalOpts.newWindowHeight;v=document.getElementsByTagName("head")[0].innerHTML;v=v.replace(/src=['"].*astroemw?\.js['"]/,"");v.match(/src=["'].*js9\.js/)||v.match(/src=["'].*js9\.min\.js/)||(v+='<script type="text/javascript" src="js9.min.js">\x3c/script>');
r=b||r(y,t);b="<html><head>"+v+"</head><body";d&&(b+=sprintf(" onload='JS9.Preload(\"%s\",%s);'",d,JSON.stringify(t)));b+=">"+r+"</body></html>\n";window.isElectron&&(z="data:text/html,<html><body><script>window.addEventListener('message', (ev) => {document.documentElement.innerHTML=ev.data\x3c/script><p></body></html>");if(z=window.open(z,y,p))return z.document?(z.document.open(),z.document.write(b),z.document.close()):z.postMessage?a.error("LoadWindow('new') is disabled on the Desktop for security reasons"):

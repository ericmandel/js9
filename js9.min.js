...
(r[e]+=1));for(b=0;b<f;b++)n+=r[b];e=n/f;for(b=k=0;b<f&&k<f;b++)for(this.hist[b]=k/f,d+=r[b];d>=e&&k<f;)d-=e,k++;for(d=(f-1)/f;b<f;)this.hist[b++]=d}for(b=0;b<q;b++)d=this.hist[b*f/q],d=Math.floor(d*h),this.psColors[b]=this.colorCells[d];for(b=0;b<t;b++){h=b/t;for(e=0;e<f-1&&!(this.hist[e]>h);e++);d=e/f;this.psInverse[b]=d*c+g}break;default:a.error("unknown scale '"+this.params.scale+"'")}return this};a.Image.prototype.mkRGBImage=function(){var d,c,b,e,f,g,h,k,l,n,m,r,q,t,u,v,p,w,x,A,z,C,D=null,y=
null,B=null,F=!1;if(!this.rgb)return this;!a.globalOpts.rgb.active||this!==a.globalOpts.rgb.rim&&this!==a.globalOpts.rgb.gim&&this!==a.globalOpts.rgb.bim||(F=!0,a.globalOpts.rgb.rim&&(D=a.globalOpts.rgb.rim),a.globalOpts.rgb.gim&&(y=a.globalOpts.rgb.gim),a.globalOpts.rgb.bim&&(B=a.globalOpts.rgb.bim));h=this.display.context;d=this.rgb;c=d.sect;if(this.MakeRGBImage&&"function"===typeof this.MakeRGBImage&&this.MakeRGBImage()||this.MakePrimaryImage&&"function"===typeof this.MakePrimaryImage&&this.MakePrimaryImage())return this;
if(this.rgbFile){f=c.width/c.zoom;g=c.height/c.zoom;b=c.x0;e=this.offscreen.canvas.height-1-(c.y0+g);e=this.offscreen.context.getImageData(b,e,f,g);if(1===c.zoom)d.img=e;else for(d.img=h.createImageData(c.width,c.height),d=d.img,r=n=0;n<e.height;n++,r++)for(w=n*e.width,t=r*c.zoom,m=h=0;h<e.width;h++,m++)for(b=4*(w+h),q=m*c.zoom,u=0;u<c.zoom;u++)for(v=Math.floor(t+u),x=v*c.width,v=0;v<c.zoom;v++)p=Math.floor(q+v),p=4*(x+p),d.data[p]=e.data[b],d.data[p+1]=e.data[b+1],d.data[p+2]=e.data[b+2],d.data[p+
...
a.error("no virtual file available for mosaic: "+e.id);a.waiting(!0,h);window.setTimeout(function(){var e,h,t,u,v,p,w,x,A,z,C,D=a.uniqueID();C=function(a){return"mosaic_"+D+"_"+a};h=C("in.lst");v=C("in.tbl");u=C("in.hdr");t=C("bin.lst");w=C("bin.tbl");x=C("out.lst");A=C("out.tbl");z=C("out.hdr");C=d[0].id.replace(/\[.*\]/,"").replace(/\.fz$/i,"").replace(/\.gz$/i,"").replace(/\.fits$/i,"_mosaic.fits");e=C.replace(/\.fits$/,"_area.fits");g=[h,v,u,t,w,x,A,z,e];e=sprintf("%s\n%s\n","|                                                    fname|",
"|                                                     char|");for(b=0;b<d.length;b++)e+=d[b].raw.hdu.fits.vfile+"\n";a.vfile(h,e);h=a.imgtbl(h,".",v,"-C");l("mImgtbl",h);a.vsize(v)||a.error("no image data found with which to construct a mosaic");h=a.makehdr(v,u,"");l("mMakeHdr",h);if("js9"===c.reduce){e=a.vread(u).split("\n");for(b=u=0;b<e.length;b++)switch(h=e[b].split("="),h[0].trim()){case "NAXIS1":u=Math.max(u,parseFloat(h[1].trim()));break;case "NAXIS2":u=Math.max(u,parseFloat(h[1].trim()))}f=
Math.max(1,Math.floor(u/c.dim+.5));e=sprintf("%s\n%s\n","|                                                    fname|","|                                                     char|");v=a.vread(v);v=v.trim().split("\n");v.splice(0,3);for(b=0;b<v.length;b++)h=v[b].trim().split(/\s+/),u=h[h.length-2],h=h[h.length-1],u&&h&&(p=sprintf("%s[%s]",h,u),u=sprintf("bin_%s_%s",u,h.split("/").reverse()[0].replace(/\.(g|f)z$/,"")),g.push(u),h="0@0,0@0,"+f,m("bin %s [%s]",p,f),a.imsection(p,u,h,""),e+=u+"\n");a.vfile(t,
e);h=a.imgtbl(t,".",w,"-C");l("mImgtbl",h);a.vsize(w)||a.error("no image data found to construct a mosaic");h=a.makehdr(w,z,"");l("mMakeHdr",h);v=a.vread(w)}else h=a.shrinkhdr(c.dim,u,z),l("mShrinkHdr",h),v=a.vread(v);v=v.trim().split("\n");v.splice(0,3);e=sprintf("%s\n%s\n","|                                                    fname|","|                                                     char|");for(b=0;b<v.length;b++)h=v[b].trim().split(/\s+/),u=h[h.length-2],h=h[h.length-1],u&&h&&(t="-a 1","shrink"===
c.reduce&&(t+=" -h "+u),t+=" "+a.globalOpts.reprojSwitches,w=sprintf("reproj_%s_%s",u,h.split("/").reverse()[0].replace(/\.(g|f)z$/,"")),e+=w+"\n",g.push(w),g.push(w.replace(/\.fits$/i,"_area.fits")),m("reproject: %s [%s] -> %s",h,u,w),h=a.reproject(h,w,z,t),l("mProjectPP",h));a.vfile(x,e);h=a.imgtbl(x,".",A,"");l("mImgtbl",h);a.vsize(A)||a.error("no FITS files were added to output table for mosaic");m("create mosaic: %s",C);h=a.madd(A,z,C,"");l("mAdd",h);k();x=$.extend(!0,{},a.fits.options,c);x.image=
{xdim:0,ydim:0};x.file=C;a.fits.handleFITSFile(C,x,n)},a.SPINOUT);return this};a.Command=function(d){for(var c in d)d.hasOwnProperty(c)&&(this[c]=d[c]);d.name||a.error("command has no name");d.get||d.set||a.error("command requires get and/or set routine");a.commands.push(this);1<a.DEBUG&&a.log("JS9 command:  %s",this.name)};a.Command.prototype.getDisplayInfo=function(a){a&&a.id&&(this.display=a,this.image=a.image);return this};a.Command.prototype.getWhich=function(a){return this.get&&!this.set?"get":
this.set&&!this.get?"set":this.which?this.which(a):0===a.length?"get":"set"};a.Helper=function(){"file:"===a.globalOpts.helperProtocol&&(a.globalOpts.helperProtocol="http:");document.domain&&"localhost"!==document.domain||(a.globalOpts.htimeout=a.globalOpts.lhtimeout);a.globalOpts.helperProtocol+="//";this.helper=this.connected=!1;this.type=a.allinone&&!a.globalOpts.allinoneHelper?"none":a.globalOpts.helperType||"sock.io";this.pageid=null;this.connect()};a.Helper.prototype.connectinfo=function(){var d;
return null===a.helper.connected?"notConfigured":a.helper.connected?(d=sprintf("connected %s %s",a.helper.type,a.helper.url),a.helper.pageid&&(d+="<p>"+a.helper.pageid),d):"notConnected "+a.helper.type};a.Helper.prototype.connect=function(d){var c=a.globalOpts.ehretries,b=a.globalOpts.ehtimeout,e=this,f=function(b,c,d){e.connected=!1;e.helper=!1;e.ready=!0;$(document).trigger("JS9:helperReady",{type:"socket.io",status:"error"});c=c||"timeout";d&&"timeout"!==d||(d="or connection refused");d===c&&(c=
...
this.url=document.domain?location.origin?location.origin:a.globalOpts.helperProtocol+document.domain:a.globalOpts.helperProtocol+"localhost";this.baseurl=this.url;switch(this.type){case "none":this.connected=null;this.ready=!0;$(document).trigger("JS9:helperReady",{type:"none",status:"OK"});break;case "get":case "post":a.globalOpts.helperCGI||a.error("cgi script name missing for helper");this.url+="/"+a.globalOpts.helperCGI;this.helper=this.connected=!0;a.DEBUG&&a.log("JS9 helper: connect: "+this.type);
this.ready=!0;$(document).trigger("JS9:helperReady",{type:"get",status:"OK"});break;case "sock.io":case "nodejs":a.globalOpts.helperPort||a.error("port missing for helper");this.url=this.url.replace(/:[0-9][0-9]*$/,"")+":"+a.globalOpts.helperPort;this.sockurl=window.multiElectron?this.url+"/socket.io/socket.io.slim.js":this.url+"/socket.io/socket.io.js";window.isElectron?(this.aliveurl=this.url+"/alive",h(this.aliveurl,this.sockurl,c)):g(this.sockurl);break;default:a.error("unknown helper type: "+
this.type)}};a.Helper.prototype.send=function(d,c,b){if(!this.connected)return null;if(c&&"object"===typeof c){try{c.cookie=document.cookie}catch(e){delete c.cookie}a.globalOpts.dataPath&&!c.dataPath&&(c.dataPath=a.globalOpts.dataPath+":.")}else c={dataPath:"."};a.TOROOT&&(c.dataPath+=":"+a.TOROOT);switch(this.type){case "get":case "post":c.key=d;a.helper.pageid&&(c.pageid=a.helper.pageid);a.DEBUG&&a.log("JS9 cgi helper [%s, %s]: %s",this.type,JSON.stringify(c),this.url);$.ajax({url:this.url,type:this.type.toUpperCase(),
data:c,dataType:"text",success:function(c){"string"===typeof c&&0<=c.search(a.analOpts.epattern)&&a.log(c);b&&b(c)},error:function(b,c,d){a.DEBUG&&a.log("JS9 helper: "+this.type+" failure: "+c+" "+d)}});break;case "sock.io":case "nodejs":a.helper.socket.emit(d,c,b)}return this};a.WebWorker=function(d){this.worker=new Worker(d);this.worker.onmessage=a.WebWorker.prototype.msgHandler.bind(this);this.handlers=[]};a.WebWorker.prototype.msgHandler=function(d){var c,b=d.data;switch(b.cmd){case "progress":a.progress(b.result.value,
b.result.max);break;case "initsocketio":this.sockinit=!0;for(d=0;d<this.handlers.length;d++)if(c=this.handlers[d],c.id===b.id){c.func(b.result);this.handlers.splice(d,1);break}break;case "uploadFITS":for(d=0;d<this.handlers.length;d++)if(c=this.handlers[d],c.id===b.id){c.func(b.result);this.handlers.splice(d,1);break}break;case "connect_error":case "connect_timeout":1<a.DEBUG&&a.log("JS9 worker socketio: "+b.cmd);break;case "disconnect":a.waiting(!1);b.result?a.worker.postMessage("initsocketio",[a.helper.url,
a.helper.pageid],function(){a.error(b.result)}):1<a.DEBUG&&a.log("JS9 worker socketio: disconnect");break;case "error":a.error(b.result||"in web worker")}};a.WebWorker.prototype.postMessage=function(d,c,b,e){var f=d+a.uniqueID(),g={id:f,cmd:d,args:c};b&&(c=c||[],this.handlers.push({id:f,cmd:d,args:c,func:b}));e?this.worker.postMessage(g,e):this.worker.postMessage(g)};a.WebWorker.prototype.socketio=function(d){var c=a.helper;a.worker.sockinit?d&&d():a.worker.postMessage("initsocketio",[c.url,c.pageid],
function(b){"OK"===b?d&&d():a.error("can't init  socket.io for JS9 worker: "+b)})};a.WebWorker.prototype.terminate=function(){this.worker.terminate()};fabric.major_version=parseFloat(fabric.version.split(".")[0]);fabric.minor_version=parseFloat(fabric.version.split(".")[1]);a.Fabric={};a.Fabric.elements="cornerSize cornerColor cornerStyle borderColor transparentCorners selectionLineWidth centeredScaling hasControls hasRotatingPoint lockMovementX lockMovementY lockRotation lockScalingX lockScalingY lockUniScaling selectable hasBorders params pub".split(" ");
...
height:this.rgb.img.height,color:"black",strokeWidth:1,tags:"SVGBorder"})),f=this.layers[b].dlayer.canvas.toSVG(),a.globalOpts.svgBorder&&this.removeShapes(b,l)}catch(n){a.error("can't convert layer to SVG: "+b)}break;default:try{e=this.listRegions(c,{mode:1},b),f=sprintf("%s\n%s\n","# Region file format: JS9 version 1.0",e.replace(/; */g,"\n"))}catch(n){a.error("can't convert layer to region: "+b)}}c=new Blob([f],{type:"text/plain;charset=utf-8"});d||(d=b&&"regions"!==b?"js9_"+b+"."+g:"js9."+g);
window.hasOwnProperty("saveAs")?saveAs(c,d):a.error("no saveAs function available to save region file");return d};a.Regions.editRegionTags=function(a,c,b){var d,f,g,h,k=[];g=this.getShapes("regions",a);if(g.length){for(d=0;d<g.length;d++)for(h=g[d].tags,k.push(c),f=0;f<h.length;f++)h[f]!==b&&k.push(h[f]);this.changeShapes("regions",a,{tags:k})}};a.Regions.toggleRegionTags=function(a,c,b){var d,f,g,h,k;g=this.getShapes("regions",a);if(g.length){for(d=0;d<g.length;d++)for(h=g[d].tags,k="",f=0;f<h.length;f++)if(h[f]===
...
c;c++)for(b=0;2>b;b++)if(void 0===a[c][b]||Number.isNaN(a[c][b]))return null;0<=g?d+=g:f+=g;g=-a[0][1]*a[1][0];0<=g?d+=g:f+=g;c=d+f;if(0===c||1E-15>Math.abs(c/(d-f)))return null;c=1/c;h[0][0]=a[1][1]*c;h[1][0]=-a[1][0]*c;h[0][1]=-a[0][1]*c;h[1][1]=a[0][0]*c;h[2][0]=-(a[2][0]*h[0][0]+a[2][1]*h[1][0]);h[2][1]=-(a[2][0]*h[0][1]+a[2][1]*h[1][1]);return h};a.log=function(a){for(var c=[],b=0;b<arguments.length;++b)c[b-0]=arguments[b];if(void 0!==window.console&&void 0!==window.console.log)try{console.log.apply(console,
[].concat($jscomp.arrayFromIterable(c)))}catch(e){b=Function.prototype.bind.call(console.log,console),b.apply(console,c)}};a.isNumber=function(a){a=Number.parseFloat(a);return!Number.isNaN(a)&&Number.isFinite(a)};a.notNull=function(a){return void 0!==a&&null!==a};a.isNull=function(a){return void 0===a||null===a};a.cardpars=function(d){var c=d.slice(0,8).trim();if("HISTORY"===c||"COMMENT"===c)return[c,d.slice(9).trim()];if("="===d[8])return d=d.slice(10).replace(/'/g," ").replace(/ \/.*/,"").trim(),
"T"===d?d=!0:"F"===d?d=!1:a.isNumber(d)&&(d=parseFloat(d)),[c,d]};a.raw2FITS=function(d,c){var b,e,f,g,h,k=!1,l="",n={};f=function(a,b,c,d){var e,f=a;"XTENSION"!==b||c?(e=new RegExp(b+" *= *(-?[-+]?[0-9]*.?[0-9]*([eE][-+]?[0-9]+)?) *"),a?(a=a.replace(e,"$1"),a=parseFloat(a)):a=void 0,a!==c&&(f=sprintf("%-8s= %20s / %-47s",b,c,d||""))):f=sprintf("%s  = %20s / %-47s","SIMPLE","T","file does conform to FITS standard");n[b]=!0;return f};if(!d)return l;c=c||{};"boolean"===typeof c&&(c={addcr:c});if(d.card||
d.cardstr)for(h=d.header||{},e=d.card?d.card.length:d.ncard,b=0;b<e;b++){if(g=d.card?d.card[b].slice(0,80):d.cardstr.slice(80*b,80*(b+1)),!c.notab||!g.match(/^TAB(TYP|MIN|MAX|DIM)[1,2]/)){if(g.match(/^XTENSION/)&&0===b&&c.simple)l+=f(g,"XTENSION");else if(g.match(/^BITPIX /)&&d.bitpix)l+=f(g,"BITPIX",d.bitpix,"bits/pixel");else if(g.match(/^NAXIS1 /)&&d.width)l+=f(g,"NAXIS1",d.width,"x image dim");else if(g.match(/^NAXIS2 /)&&d.height)l+=f(g,"NAXIS2",d.height,"y image dim");else if(g.match(/^CRPIX1 /)&&
...
g.errLog=function(b,c){a.log("error in %s: %s [%s]\n%s",b,this.name,c.message,a.strace(c))};if(d)for(h=d instanceof jQuery?d:"object"===typeof d?$(d):$("#"+d),f=0;f<c.instances.length;f++){if(h.is(c.instances[f].odivjq))return c.instances[f]}else h=$("div");d?b?(g.id=h.attr("id")||c.name,g.winType="light",g.winHandle=b,g.odivjq=h,g.divjq=h,g.outerdivjq=g.divjq.closest(a.lightOpts[a.LIGHTWIN].top)):(g.id=h.attr("id")||c.name,g.winType="div",0<=$.inArray(g.name,a.globalOpts.hiddenPluginDivs)&&(l="hidden"),
h.wrap(sprintf("<div class='JS9PluginContainer' style='visibility: %s'>",l)),g.odivjq=h,g.divjq=h,g.divjq.addClass(c.xclass+"Plugin").addClass("JS9Plugin"),g.odivjq.attr("id")||g.odivjq.attr("id",g.id),g.outerdivjq=g.divjq.closest(".JS9PluginContainer"),!1!==h.data("toolbarseparate")&&(c.opts.toolbarSeparate||h.data("toolbarseparate"))&&(b="<div class='"+a.lightOpts[a.LIGHTWIN].dragBar+"'>",$(b).insertBefore(g.divjq))):(g.id=c.name,g.winType="virtual");g.plugin=c;g.el=d;g.status="active";c.instances.push(g);
if("virtual"===g.winType)for(f=0;f<a.displays.length;f++)a.displays[f].pluginInstances[c.name]||(g.div=null,g.display=a.displays[f],c.func.apply(g,e),a.displays[f].pluginInstances[c.name]=g);else{g.div=g.divjq[0];g.outerdiv=g.outerdivjq[0];!c.opts.winDims||g.divjq.width()&&g.divjq.height()||(g.divjq.css("width",c.opts.winDims[0]),g.divjq.css("height",c.opts.winDims[1]));d=g.divjq.data("js9id")||g.id;"*"===d?c.opts.dynamicSelect?(g.display=a.displays[0],g.isDynamic=!0,a.Dysel.addPlugins(c.name),k=
"*"):a.error(c.name+" is not dynamically selectable"):(g.display=a.lookupDisplay(d),k=g.display.id);if(h=h.data("toolbarhtml")||c.opts.toolbarHTML)h=a.Image.prototype.expandMacro.call(null,h,[{name:"title",value:c.opts.winTitle||""}]),d=g.divjq.closest(a.lightOpts[a.LIGHTWIN].drag),0===d.length&&(d=g.divjq),$("<div class='JS9PluginToolbar-"+g.winType+"'>").css("z-index",a.BTNZINDEX).html(h).data("displayid",k).insertAfter(d);g.display.pluginInstances[c.name]=g;c.func.apply(g,e);if("*"===k)for(f=0;f<
a.displays.length;f++)a.displays[f].pluginInstances[c.name]?g.display=a.displays[f]:a.displays[f].pluginInstances[c.name]=g}return g};a.instantiatePlugins=function(){var d,c=function(b){var c,d,g;$("div."+b.name).each(function(){a.instantiatePlugin($(this),b,null,b.opts.divArgs)});b.opts.menuItem||!b.opts.winDims||b.opts.winDims[0]||b.opts.winDims[1]||a.instantiatePlugin(null,b,null,b.opts.divArgs);for(c=0;c<b.instances.length;c++)if(g=b.instances[c],g.isDynamic)for(d=0;d<a.displays.length;d++)a.displays[d].pluginInstances[b.name]||
(a.displays[d].pluginInstances[b.name]=g)};for(d=0;d<a.plugins.length;d++)c(a.plugins[d])};a.initEmscripten=function(){var d;if(!window.hasOwnProperty("Astroem"))if(d={responseType:"arraybuffer",allowCache:!0},a.globalOpts.useWasm&&"object"===typeof WebAssembly&&("file:"!==location.protocol||a.globalOpts.allowFileWasm))a.globalOpts.astroemWasm=a.InstallDir(Module.wasmBinaryFile||"astroemw.wasm"),a.fetchURL(a.globalOpts.astroemWasm,null,d,function(c){Module.wasmBinary=c;a.globalOpts.astroemURL=a.InstallDir("astroemw.js");
...
0,.3098]])))};a.initCommands=function(){a.hasOwnProperty("Command")&&(a.checkNew(new a.Command({name:"analysis",alias:"run",help:"list/run analysis for current image",get:function(){var a,c,b,e,f,g="",h=this.image;if(h&&h.analysisPackages)for(c=0;c<h.analysisPackages.length;c++){f=h.analysisPackages[c];for(a=0;a<f.length;a++)e=f[a],g&&(g+=", "),b=e.xclass?e.xclass+":"+e.name:e.name,g+=sprintf("%s (%s)",e.title,b);c<h.analysisPackages.length-1&&(g+="\n")}return g},set:function(d){var c,b=this.image;
b&&((c=b.lookupAnalysis(d[0]))?c.purl?(d=b.displayAnalysis("params",a.InstallDir(c.purl),{title:c.title+": "+b.fitsFile}),$(d).data("dispid",b.display.id).data("aname",c.name)):b.runAnalysis(c.name):a.error("unknown analysis command '"+d[0]+"'"))}})),a.checkNew(new a.Command({name:"colormap",alias:"cmap",help:"set/get colormap for current image",get:function(){var a;if(a=this.image)return a=a.getColormap(),sprintf("%s %s %s",a.colormap,a.contrast,a.bias)},set:function(a){var c=this.image;c&&c.setColormap.apply(c,
[].concat($jscomp.arrayFromIterable(a)))}})),a.checkNew(new a.Command({name:"colormaps",alias:"cmaps",help:"get list of available colormaps",get:function(){var d,c="";for(d=0;d<a.colormaps.length;d++)c&&(c+=", "),c+=a.colormaps[d].name;return c}})),a.checkNew(new a.Command({name:"grid",help:"set/get coordinate grid for current image",get:function(){var a,c=this.image;c&&(a=c.displayCoordGrid());return a?"true":"false"},set:function(a){var c,b=this.image;b&&(c=a[0].match(/true/i)?!0:!1,b.displayCoordGrid(c,
a[1]))}})),a.checkNew(new a.Command({name:"help",help:"get list of available commmands",get:function(){var d,c,b="<table>";for(d=0;d<a.commands.length;d++)c=a.commands[d],b+="<tr><td>"+c.name+"</td><td>"+c.help,c.alias&&(b+=" ("+c.alias,c.alias2&&(b+=", "+c.alias2),b+=")"),b+="</td></tr>";return b+"</table>"}})),a.checkNew(new a.Command({name:"helper",help:"get/set helper connection",get:function(){return a.helper.connectinfo()},set:function(d){a.helper.connect(d[0].trim())}})),a.checkNew(new a.Command({name:"image",
help:"get name of currently loaded image or display specified image",get:function(){var a=this.image;if(a)return a.file},set:function(d){var c,b;for(c=0;c<a.images.length;c++)if(b=a.images[c],0<=b.file.search(d[0])&&b.display===this.display){b.displayImage("display");break}}})),a.checkNew(new a.Command({name:"images",help:"get list of currently loaded images",get:function(){var d,c,b="";for(d=0;d<a.images.length;d++)c=a.images[d],c.display===this.display&&(b&&(b+=", "),b+=c.file);return b}})),a.checkNew(new a.Command({name:"load",
help:"load image(s)",set:function(d){var c,b,e,f=d.length;for(c=0;c<f;c++){e=null;b=c+1;if(b<f&&0===d[b].indexOf("{"))try{e=JSON.parse(d[b])}catch(g){e=null}e?(a.Load(d[c],e,{display:this.display.id}),c++):a.Load(d[c],{display:this.display.id})}}})),a.checkNew(new a.Command({name:"pan",help:"set/get pan location for current image",get:function(){var a;if(a=this.image)return a=a.getPan(),sprintf("%s %s",a.x,a.y)},set:function(a){var c=this.image;c&&c.setPan.apply(c,[].concat($jscomp.arrayFromIterable(a)))}})),
a.checkNew(new a.Command({name:"pix2wcs",help:"get image pixel value for specified wcs position",set:function(d){var c=this.image;if(c)return d=a.Pix2WCS(parseFloat(d[0]),parseFloat(d[1]),{display:c}),d.str}})),a.checkNew(new a.Command({name:"print",help:"print image window",get:function(){var a=this.image;a&&a.print()}})),a.checkNew(new a.Command({name:"refresh",help:"refresh current image using specified file (def: use last file)",set:function(d){var c,b,e,f=d.length;c=this.image;if(0===f)e={refresh:c},
a.Load(c.file,e,{display:this.display.id});else for(c=0;c<f;c++){e=null;b=c+1;if(b<f&&0===d[b].indexOf("{"))try{e=JSON.parse(d[b])}catch(g){e=null}e?(e.refresh=!0,a.Load(d[c],e,{display:this.display.id}),c++):(e={refresh:!0},a.Load(d[c],e,{display:this.display.id}))}}})),a.checkNew(new a.Command({name:"regcnts",help:"counts in regions for current image",get:function(){var a=this.image;a&&a.countsInRegions("$sregions","$bregions",{lightwin:!0})},set:function(a){var c=this.image;if(c)return c.countsInRegions.apply(c,
[].concat($jscomp.arrayFromIterable(a)))}})),a.checkNew(new a.Command({name:"regions",alias:"reg",alias2:"region",help:"add region to current image or list all regions",get:function(){var a=this.image;if(a)return a.listRegions("all",{mode:0})||""},set:function(a){var c=this.image;c&&("delete"===a[0]||"remove"===a[0]?(a=a.slice(1).join(" "),c.removeShapes("regions",a)):(a=a.join(" "),c.addShapes("regions",a)))}})),a.checkNew(new a.Command({name:"resize",help:"get/set display size for current image",
get:function(){var a;if(a=this.image)return a=a.display,sprintf("%s %s",a.width,a.height)},set:function(a){var c,b;(c=this.image)&&a.length&&(c=c.display,b=parseInt(a[0],10),a=1<a.length?parseInt(a[1],10):b,c.resize(b,a))}})),a.checkNew(new a.Command({name:"scale",help:"set/get scaling for current image",get:function(){var a;if(a=this.image)return a=a.getScale(),sprintf("%s %s %s",a.scale,a.scalemin,a.scalemax)},set:function(a){var c=this.image;c&&c.setScale.apply(c,[].concat($jscomp.arrayFromIterable(a)))}})),
a.checkNew(new a.Command({name:"scales",help:"get list of available scales",get:function(){return a.scales.join(", ")}})),a.checkNew(new a.Command({name:"section",help:"display section of current image",set:function(d){var c,b=this.image;if(1===d.length&&"full"===d[0])b.displaySection("full");else{d=d.join(" ");try{c=JSON.parse(d)}catch(e){a.error("invalid JSON section")}b.displaySection(c)}}})),a.checkNew(new a.Command({name:"status",help:"get status for specified (or current) image",get:function(d){var c,
b,e,f="";for(c=0;c<a.images.length;c++)if(b=a.images[c],0<=b.file.search(d[0])){e=b;break}e?c=1:(c=0,e=this.image);if(e){if(c>d.length)return e.status.load;for(;c<d.length;c++)switch(b=d[c].toLowerCase().trim(),b){case "load":f&&(f+="\n"),f+=e.status.load}}return f}})),a.checkNew(new a.Command({name:"url",help:"display a url",set:function(d){a.DisplayHelp(d[0])}})),a.checkNew(new a.Command({name:"wcssys",help:"set/get wcs system for current image",get:function(){var a=this.image;if(a)return a.getWCSSys()},
set:function(a){var c=this.image;c&&c.setWCSSys(a[0])}})),a.checkNew(new a.Command({name:"wcsu",help:"set/get wcs units used for current image",get:function(){var a=this.image;if(a)return a.getWCSUnits()},set:function(a){var c=this.image;c&&c.setWCSUnits(a[0])}})),a.checkNew(new a.Command({name:"wcssystems",help:"get list of available wcs systems",get:function(){return a.wcssyss.join(", ")}})),a.checkNew(new a.Command({name:"wcsunits",help:"get list of available wcs units",get:function(){return a.wcsunitss.join(", ")}})),
a.checkNew(new a.Command({name:"wcs2pix",help:"get wcs position for specified image pixel",set:function(d){var c=this.image;if(c)return d=a.WCS2Pix(parseFloat(d[0]),parseFloat(d[1]),{display:c}),d.str}})),a.checkNew(new a.Command({name:"zoom",help:"set/get zoom for current image",get:function(){var a=this.image;if(a)return a.getZoom()},set:function(a){var c=this.image;c&&c.setZoom(a[0])}})))};a.initAnalysis=function(){$(document).on("keyup keypress",".js9AnalysisForm",function(a){var c=$(this);if(13===
(a.which||a.keyCode))return a.preventDefault(),(a=$(this).data("enterfunc"))&&c.find("[name='"+a+"']").click(),!1})};a.init=function(){var d;window.HTMLCanvasElement&&JSON||a.error("your browser does not support JS9 (no HTML5 canvas and/or JSON). Please try a modern version of Firefox, Chrome, Safari, Opera, or Edge.");window.hasOwnProperty("JS9Prefs")&&"object"===typeof JS9Prefs&&JS9Prefs.globalOpts&&JS9Prefs.globalOpts.installDir&&(a.INSTALLDIR=JS9Prefs.globalOpts.installDir);if(!a.INSTALLDIR){try{$('link[href$="js9.css"]').each(function(){var c=
$(this).attr("href");c&&"js9.css"===c.split("/").reverse()[0]&&(a.INSTALLDIR=c.replace(/js9\.css$/,""))})}catch(c){a.INSTALLDIR=""}a.INSTALLDIR&&(a.INSTALLDIR=a.cleanPath(a.INSTALLDIR))}a.INSTALLDIR&&"/"!==a.INSTALLDIR.slice(-1)&&(a.INSTALLDIR+="/");a.TOROOT=a.INSTALLDIR.replace(/([^/.])+/g,"..");window.hasOwnProperty("JS9Inline")&&"object"===typeof JS9Inline&&(a.inline=$.extend(!0,{},JS9Inline));"dhtml"===a.LIGHTWIN&&($("<div>").attr("id","dhtmlwindowholder").appendTo($(document.body)).append("<span style='display:none'>.</span>"),
...
0]=arguments[b];c=a.parsePublicArgs(c);return a.lookupImage(c.argv[0],c.display)});a.mkPublic("LookupDisplay",function(d){for(var c=[],b=0;b<arguments.length;++b)c[b-0]=arguments[b];c=a.parsePublicArgs(c);return a.lookupDisplay(c.argv[0]||c.display,c.argv[1])});a.mkPublic("RenameDisplay",function(d){for(var c=[],b=0;b<arguments.length;++b)c[b-0]=arguments[b];var e,b=a.parsePublicArgs(c);switch(b.argv.length){case 0:return;case 1:c=b.display;b=b.argv[0];break;default:c=b.argv[0],b=b.argv[1]}(e=a.lookupDisplay(c))&&
e.id&&(c=e.id,e.oid||(e.oid=c),e.id=b,a.helper.send("renameDisplay",{odisplay:c,ndisplay:e.id}))});a.mkPublic("CloseDisplay",function(d){for(var c=[],b=0;b<arguments.length;++b)c[b-0]=arguments[b];var e,c=a.parsePublicArgs(c);e=a.lookupDisplay(c.argv[0]||c.display);for(c=a.images.length-1;0<=c;c--)b=a.images[c],b.display===e&&b.closeImage()});a.mkPublic("AddColormap",function(d){for(var c=[],b=0;b<arguments.length;++b)c[b-0]=arguments[b];var e,f,g,h,k=a.parsePublicArgs(c),l=function(b,c){var d,e;
...
var e,f,g,h,k,l,n,m,r,q,t,u,v,p,w,x,A,z,C=a.lightOpts[a.LIGHTWIN];z=a.parsePublicArgs(c);var D=function(b){b=$.inArray(b,a.displays);0<=b&&a.displays.splice(b,1)};l=function(b,c,d){var f,g;c=c||{};c.clone&&(g=a.lookupDisplay(c.clone,!1));d&&(v=d.match(/(.*width=)([0-9]+)(px,height=)([0-9]+)(px.*)/,"$1@@H@@$3"),p=parseInt(v[2],10),w=parseInt(v[4],10));f="<hr class='hline0'>";!g||0<$("#"+c.clone+"Menubar").length&&!g.pluginInstances.JS9Menubar.isDynamic?f+="<div class='JS9Menubar' id='"+b+"Menubar'></div>":
d&&(w-=40);f+="<div class='JS9' id='"+b+"'></div>";!g||0<$("#"+c.clone+"Colorbar").length&&!g.pluginInstances.JS9Colorbar.isDynamic?(e=g&&g.pluginInstances.JS9Colorbar?"data-showTicks='"+g.pluginInstances.JS9Colorbar.showTicks+"'":"",f+=sprintf("<div style='margin-top: 2px;'><div class='JS9Colorbar' id='%sColorbar' %s></div></div>",b,e),g&&g.pluginInstances.JS9Colorbar&&!g.pluginInstances.JS9Colorbar.showTicks&&(w-=15)):d&&(w-=44);return d?(a.Dysel.retrievePlugins().length&&(p+=2,w+=2),{html:f,winopts:v[1]+
String(p)+v[3]+String(w)+v[5]}):f};k=function(b,c){var d;d=new a.Display(f);d.winid=m;a.helper.send("addDisplay",{display:f});a.instantiatePlugins();c.display=f;b&&a.Load(b,c);return d};var y=function(b){var c,d;d=0;var e=[];for(c=0;c<a.images.length;c++)d=a.images[c],d.display.id===f&&e.push(d);if(!e.length)return D(b),!0;if("move"===a.globalOpts.lightWinClose){if(a.isNull(a.globalOpts.lightWinMoveTo)||a.globalOpts.lightWinMoveTo===f)d=0;else for(d=c=0;c<a.displays.length;c++)if(a.displays[c].id===
...
{onceOnly:!0},function(){var b,c;c=$("#lightWinCloseForm").find("#lightWinCloseSel");for(b=0;b<a.displays.length;b++)a.displays[b].id!==f&&c.append($("<option>",{value:a.displays[b].id,text:a.displays[b].id}))}),h=a.lightWin(r,q,t,"Closing a light window",C.lcloseWin),$(h).data("dispid",f),$(h).data("winid",m),!1}},b=z.argv[0];x=z.argv[1];A=z.argv[2];c=z.argv[3];z=z.argv[4];if("object"===typeof x)x=$.extend(!0,{},x);else if("string"===typeof x)try{x=JSON.parse(x)}catch(B){x={}}else x={};A=A||"light";
u=(A||"")+"win";switch(A){case "light":return x.id?(f=x.id,delete x.id):f=u+a.uniqueID(),h="d"+f,c?z=z||C.imageWin:(n=l(f,x,C.imageWin),c=n.html,z=z||n.winopts),n=sprintf("JS9 Display"+a.IDFMT,f),m=a.lightWin(h,"inline",c,n,z),g=k(b,x),m.onclose=function(){return y(g)},f;case "new":x.id?(f=x.id,delete x.id):f=u+a.uniqueID();z=z||sprintf("width=%s, height=%s",a.globalOpts.newWindowWidth,a.globalOpts.newWindowHeight);k=document.getElementsByTagName("head")[0].innerHTML;k=k.replace(/src=['"].*astroemw?\.js['"]/,
"");k.match(/src=["'].*js9\.js/)||k.match(/src=["'].*js9\.min\.js/)||(k+=sprintf('<%s type="text/javascript" src="js9.min.js"></%s>',"script","script"));l=c||l(f,x);c="<html><head>"+k+"</head><body";b&&(c+=sprintf(" onload='JS9.Preload(\"%s\",%s);'",b,JSON.stringify(x)));c+=">"+l+"</body></html>\n";window.isElectron&&(n="data:text/html,<html><body><script>window.addEventListener('message', function(ev){document.documentElement.innerHTML=ev.data\x3c/script><p></body></html>");if(n=window.open(n,f,
z))return n.document?(n.document.open(),n.document.write(c),n.document.close()):n.postMessage?a.error("LoadWindow('new') is disabled on the Desktop for security reasons"):a.error("no method available for drawing image into window"),f;a.error("could not create window (check your pop-up blockers)")}});a.mkPublic("LoadProxy",function(d){for(var c=[],b=0;b<arguments.length;++b)c[b-0]=arguments[b];var e,f,g,h,k=a.parsePublicArgs(c);g=k.argv[0];h=k.argv[1];a.globalOpts.loadProxy||a.error("proxy load not available for this server");

...
a.spinner&&a.spinner.spinner.stop():$("body").removeClass("waiting")}};a.msgHandler=function(d,b){var c,e,f,g=[],h=d.cmd,j=d.id;b&&(a.globalOpts.alerts=!1);if(a.publics[h]){$.isArray(d.args)||(d.args=[d.args]);g=$.extend(!0,[],d.args);j&&g.push({display:j});if(b&&"RunAnalysis"===h){1===g.length&&g.push(null);g.push(b);a.publics[h].apply(null,g);a.globalOpts.alerts=!0;return}try{f=a.publics[h].apply(null,g)}catch(k){f=sprintf("ERROR: %s",k.message)}b&&(a.globalOpts.alerts=!0,b(f));return f}if(!h||
"#"===h)b&&b(""),b&&(a.globalOpts.alerts=!0);else{c=a.lookupCommand(h);e=a.lookupDisplay(j);if(c&&e)switch(c.getDisplayInfo(e),d.args?g=$.extend(!0,[],d.args):d.paramlist&&(g=d.paramlist.split(/ +/)),c.getWhich(g)){case "get":try{f=c.get(g)||""}catch(n){f="ERROR: "+n.message}break;case "set":try{f=c.set(g)||"OK"}catch(p){f="ERROR: "+p.message}break;default:f=sprintf("ERROR: unknown cmd type for '%s'",h)}else c||(f=sprintf("ERROR: unknown cmd '%s'",h)),e||(f=sprintf("ERROR: unknown display (%s)",j));
...
for(e=0;e<d.length;e++)if(-1!==d[e].type.indexOf("image/"))switch(d[e].type){case "image/fits":f(e);break;default:a.handleImageFile(d[e],b,c)}else f(e)};a.fetchURL=function(d,b,c,e){var f=new XMLHttpRequest,g;b||(b=d,d=/([^\\\/]+)$/.exec(b)[1]);g=$.extend(!0,{},c,a.fits.options);f.open("GET",b,!0);f.responseType=c.responseType?c.responseType:"blob";a.globalOpts.xtimeout&&(f.timeout=a.globalOpts.xtimeout);f.onload=function(){var h;4===this.readyState&&(200===this.status||0===this.status?"blob"===f.responseType?

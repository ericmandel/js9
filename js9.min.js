...
g[b-1][1]:(b=(g[d][1]-g[d-1][1])/(g[d][0]-g[d-1][0]))?b*(c-g[d-1][0])+g[d-1][1]:g[d][1];e[f]=255*d}break;case "lut":f=this.colors.length;c=Math.floor(c*f/d);0>c?(e[0]=255*this.colors[0][0],e[1]=255*this.colors[0][1],e[2]=255*this.colors[0][2]):c<f?(e[0]=255*this.colors[c][0],e[1]=255*this.colors[c][1],e[2]=255*this.colors[c][2]):(e[0]=255*this.colors[f-1][0],e[1]=255*this.colors[f-1][1],e[2]=255*this.colors[f-1][2]);break;default:a.error("unknown colormap type")}return e};a.Display=function(c){var b=
this;this.divjq=c instanceof jQuery?c:"object"===typeof c?$(c):$("#"+c);this.divjq.attr("id")||this.divjq.attr("id",a.DEFID);this.id=this.divjq.attr("id");this.divjq.addClass("JS9");this.width=this.divjq.attr("data-width");this.width||(this.width=a.WIDTH);this.divjq.css("width",this.width);this.width=parseInt(this.divjq.css("width"),10);this.height=this.divjq.attr("data-height");this.height||(this.height=a.HEIGHT);this.divjq.css("height",this.height);this.height=parseInt(this.divjq.css("height"),
...
!1);this.tooltip=$("<div>").attr("id","tooltip_"+this.id).addClass("JS9Tooltip").appendTo(this.divjq);this.image=null;this.pluginInstances={};this.layers={};this.initMessages();this.blendMode=!1;this.divjq.on("mouseover",this,function(c){return a.mouseOverCB(c)});this.divjq.on("mousedown touchstart",this,function(c){return a.mouseDownCB(c)});this.divjq.on("mousemove touchmove",this,function(c){return a.mouseMoveCB(c)});this.divjq.on("mouseup touchend",this,function(c){return a.mouseUpCB(c)});this.divjq.on("mouseout",
this,function(c){return a.mouseOutCB(c)});this.divjq.on("keypress",this,function(c){return a.keyPressCB(c)});this.divjq.on("keydown",this,function(c){return a.keyDownCB(c)});this.divjq.on("dragenter",this,function(c){return a.dragenter(this.id,c.originalEvent)});this.divjq.on("dragover",this,function(c){return a.dragover(this.id,c.originalEvent)});this.divjq.on("dragexit",this,function(c){return a.dragexit(this.id,c.originalEvent)});this.divjq.on("drop",this,function(c){return a.dragdrop(this.id,
c.originalEvent,a.NewFITSImage)});this.divjq.append('<div style="visibility:hidden; position:relative; top:-50;left:-50"> <input type="file" id="openLocalFile-'+this.id+'" multiple="true" onchange="javascript:for(var i=0; i<this.files.length; i++){JS9.Load(this.files[i], {display:\''+this.id+"'}); }\"> </div>");this.divjq.append('<div style="visibility:hidden; position:relative; top:-50;left:-50"> <input type="file" id="refreshLocalFile-'+this.id+'" multiple="true" onchange="javascript:for(var i=0; i<this.files.length; i++){JS9.RefreshImage(this.files[i], {display:\''+
this.id+"'}); }\"> </div>");this.divjq.append('<div style="visibility:hidden; position:relative; top:-50;left:-50"> <input type="file" id="openLocalRegions-'+this.id+'" multiple="true" onchange="javascript:for(var i=0; i<this.files.length; i++){JS9.LoadRegions(this.files[i], {display:\''+this.id+"'}); }\"> </div>");a.displays.push(this);a.DEBUG&&a.log("JS9 display:  %s (%d,%d)",this.id,this.x,this.y)};a.Display.prototype.initMessages=function(){this.messageContainer=$("<div>").addClass("JS9Container").css("z-index",

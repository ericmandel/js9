...
JSON.parse(d)}catch(l){a.error("can't parse showShapeLayer opts: "+d,l)}var h=f.canvas;var k=this.display.layers[c];if(a.isNull(b))return f.show;if(!0===b||"true"===b){if(!d.local&&(f.show=!0,this!==this.display.image))return;f.show&&(h.selection=f.opts.canvas.selection);f.json&&f.show&&h.loadFromJSON(f.json,function(){var b,d;a.Fabric.updateChildren(f.dlayer,null,"objects");e.resize&&(h.getObjects().forEach(function(b){b.left+=e.resize.left;b.top+=e.resize.top;b.setCoords()}),h.calcOffset());f.zindex=
...
e[f].params.id;for(c=g.length-1;0<=c;c--){var k=g[c];if(h===k){1E-6===e[f].pub.radius&&(e[f].pub.radius=0);b.radii.push(e[f].pub.radius);g.splice(c,1);break}}if(!g.length)break}b.radii.sort(function(b,a){return b-a})}this.changeShapes(a,this.editAnnulus.annulus,b);this.removeShapes(a,this.editAnnulus.ids);delete this.editAnnulus}};a.Fabric.updateChildren=function(c,b,d){var e,f,g,h={};if("regions"===c.layerName)if("objects"===d){b=c.canvas.getObjects().reverse();d=b.length;do{for(f=0;d--;)c=b[d],
"group"!==c.type||c.params||(b.splice.apply(b,[d,1].concat($jscomp.arrayFromIterable(c.getObjects()))),f++);d=b.length}while(f);b.forEach(function(b){b.params&&(b.params.parent||b.params.children.length)&&(h[b.params.id]=b)});b.forEach(function(b){if(b.params)for(b.params.parent&&(b.params.parent.obj=h[b.params.parent.id]),e=0;e<b.params.children.length;e++)b.params.children[e].obj=h[b.params.children[e].id]})}else if(b&&b.params)if("deltas"===d)b.params.parent&&(f=b.params.parent,f.dleft=f.obj.left-
b.left,f.dtop=f.obj.top-b.top,f.moved=!0);else for(e=0;e<b.params.children.length;e++){var k=b.params.children[e].obj;f=k.params.parent;switch(d){case "moving":k.left=b.left-f.dleft;k.top=b.top-f.dtop;break;case "rotating":for(g=b.angle;0>g;)g+=360;for(;360<=g;)g-=360;var m=g-f.lastangle;var l=a.rotatePoint({x:k.left,y:k.top},m,{x:b.left,y:b.top});k.left=l.x;k.top=l.y;f.dleft=b.left-k.left;f.dtop=b.top-k.top;for(k.angle+=m;0>k.angle;)k.angle+=360;for(;360<=k.angle;)k.angle-=360;f.lastangle=g;break;
case "scaling":f.dleft*=b.scaleX/f.lastscalex,f.dtop=b.scaleY/f.lastscaley*(f.dtop-f.textheight)+f.textheight,f.lastscalex=b.scaleX,f.lastscaley=b.scaleY,f.moved=!0,k.left=b.left-f.dleft,k.top=b.top-f.dtop}k.setCoords();c.display.image&&c.display.image.updateShapes(c.layerName,k,"updatechild")}};a.resetPolygonCenter=function(c){var b={};var d=c._calcDimensions();var e=(d.left+d.width/2)*c.scaleX;d=(d.top+d.height/2)*c.scaleY;c.angle?b=a.rotatePoint({x:c.left+e,y:c.top+d},c.angle,{x:c.left,y:c.top}):
(b.x=c.left+e,b.y=c.top+d);e/=c.scaleX;var f=d/c.scaleY;for(d=0;d<c.points.length;d++)c.points[d].x-=e,c.points[d].y-=f;c.left=b.x;c.top=b.y;b=c._calcDimensions();c.width=b.width;c.height=b.height;c.pathOffset={x:b.left+c.width/2,y:b.top+c.height/2};c.setCoords()};a.Fabric.saveSelection=function(a){var b,c=[];if(b=this.getShapeLayer(a)){var e=b.canvas.getActiveObjects();for(a=0;a<e.length;a++){var f=e[a];f.params&&!1!==f.params.changeable&&c.push(f.params.id)}c.length&&(b.savesel=c)}};a.Fabric.restoreSelection=
...
function(a,b,d){a.display.blendMode&&a.displayImage("rgb")};a.MouseTouch.Actions["wheel zoom"]=function(c,b){var d=a.globalOpts.panzoomRefreshLimit,e=0;b=b.originalEvent.deltaY*Math.sign(a.DIRZOOM);if(c&&a.globalOpts.mousetouchZoom&&(c.tmp.wheelzooms=c.tmp.wheelzooms||0,0===c.tmp.wheelzooms++%a.MODZOOM)){var f=c.getZoom();b=0>b?Math.min(a.MAXZOOM,f+a.ADDZOOM):Math.max(a.MINZOOM,f-a.ADDZOOM);if(a.globalOpts.mousetouchLimit){var g=Math.min(c.display.width/c.raw.width,c.display.height/c.raw.height);
if(g>b&&f>b)return}b=Math.round(100*(b+1E-5))/100;f=$jscomp.makeIterator(Object.keys(c.layers));for(g=f.next();!g.done;g=f.next())g=g.value,c.layers[g].show&&c.layers[g].opts.panzoom&&c.layers[g].canvas.size()>d&&(c.tmp.panzoomRefresh=c.tmp.panzoomRefresh||{},c.tmp.panzoomRefresh[g]={},e++);c.tmp.panzoomTimeout&&(clearTimeout(c.tmp.panzoomTimeout),delete c.tmp.panzoomTimeout);if(e||c.tmp.panzoomRefresh)c.tmp.panzoomTimeout=setTimeout(function(){c.refreshLayers(c.tmp.panzoomRefresh);delete c.tmp.panzoomRefresh},
a.TIMEOUT);c.setZoom(b)}};a.MouseTouch.Actions["pan the image"]=function(c,b,d){var e=a.globalOpts.panMouseThreshold;b=a.globalOpts.panzoomRefreshLimit;if(c){d=c.rgb.sect;var f=(c.pos0.x-c.pos.x)/d.zoom;var g=(c.pos0.y-c.pos.y)/d.zoom;if(Math.abs(f)>=e||Math.abs(g)>=e){"x"===c.params.flip?f=-f:"y"===c.params.flip?g=-g:"xy"===c.params.flip&&(f=-f,g=-g);90===c.params.rot90?(e=f,f=-g,g=e):180===c.params.rot90?(f=-f,g=-g):-90===c.params.rot90&&(e=f,f=g,g=-e);f={x:d.xcen+f,y:d.ycen-g};c.params.rotate&&
...
b.winid=this.displayAnalysis("regions",g,{title:f,winformat:h});c&&c.params&&(c.params.winid=b.winid)}};a.Regions.initConfigForm=function(c,b){var d=this,e,f,g,h,k,m,l,p=!1,r=this.raw.wcsinfo||{cdelt1:1,cdelt2:1};var n={type:"multi",pub:{shape:"multi",wcsconfig:{}},params:{}};var u=function(b){if(void 0!==b)return"number"===typeof b&&0!==b%1&&(b=Math.round(1E4*(b+1E-5))/1E4),String(b)},x=function(b){var a=String.fromCharCode(13,10);return"string"===typeof b?b.replace(/\\n/g,a):b};if(c&&c.pub)var t=
c.pub.wcsconfig&&c.pub.wcsconfig.wcssys?c.pub.wcsconfig.wcssys:this.params.wcssys;else t=this.params.wcssys,c=n;var v=!1===c.params.changeable;b=b||{};if(c.params.winid)var w=c.params.winid;else b.winid&&(w=b.winid);if(w){var q="#"+$(w).attr("id")+" .regionsConfigForm ";if($(q).length){p=$(q).data("multi")?!0:b.multi;$(q+"."+c.pub.shape).each(function(b,a){$(a).removeClass("nodisplay")});$(q+".val").each(function(b,l){g="";f=$(l).attr("name");switch(f){case "x":case "y":c.pub.lcs&&void 0!==c.pub.lcs[f]?
g=u(c.pub.lcs[f]):void 0!==c.pub[f]&&(g=u(c.pub[f]));break;case "radii":c.pub.radii&&(g=!a.notWCS(t)&&c.pub.wcsconfig&&c.pub.wcsconfig.wcsstr?c.pub.wcsconfig.wcsstr.replace(/^annulus\(/,"").replace(/\)$/,"").split(",").slice(2).join(","):c.pub.imstr.replace(/^annulus\(/,"").replace(/\)$/,"").split(",").slice(2).join(","));break;case "pts":c.pub.pts?c.pub.pts.forEach(function(b){g&&(g+=", ");g+=b.x.toFixed(2)+", "+b.y.toFixed(2)}):c.pub.imstr&&(g=c.pub.imstr.replace(/^.*\(/,"").replace(/\)$/,""));
break;case "linelength":if(c.pub.pts&&2===c.pub.pts.length){k=c.pub.pts[0];m=c.pub.pts[1];g=u(Math.sqrt((m.x-k.x)*(m.x-k.x)+(m.y-k.y)*(m.y-k.y)));switch(t){case "image":case "physical":break;default:g*=Math.abs(r.cdelt1),g*=Math.abs(r.cdelt2)}g=u(g);d.tmp.linelength=g}break;case "lineangle":if(c.pub.pts&&2===c.pub.pts.length){k=c.pub.pts[0];m=c.pub.pts[1];for(g=180*Math.atan2(m.y-k.y,m.x-k.x)/Math.PI;0>g;)g+=360;g=u(g);d.tmp.lineangle=g}break;case "fontFamily":c.getFontFamily&&(g=c.getFontFamily());
break;case "fontSize":c.getFontSize&&(g=c.getFontSize());break;case "fontStyle":c.getFontStyle&&(g=c.getFontStyle());break;case "fontWeight":c.getFontWeight&&(g=c.getFontWeight());break;case "colorPicker":g=void 0!==c.pub.color?a.colorToHex(c.pub.color):$(q).data("colorpicker")||a.globalOpts.defcolor;break;case "color":p||(void 0!==c.pub.color?g=u(c.pub.color):$(q).data("colorpicker")&&(g=$(q).data("colorpicker")));break;case "strokeWidth":g=c.params.sw1?c.params.sw1:$(q).data("strokewidth")||"";
break;case "strokeDashes":c.strokeDashArray?(g=c.strokeDashArray.join(" "),g.match(/NaN/)&&(g="")):g=$(q).data("strokedashes")||"";break;case "regstr":g=!a.notWCS(t)&&c.pub.wcsconfig&&c.pub.wcsconfig.wcsstr?c.pub.wcsconfig.wcssys+";"+c.pub.wcsconfig.wcsstr:c.pub.imsys+";"+c.pub.imstr;break;case "xpos":switch(t){case "image":c.pub.preservedcoords&&void 0!==c.pub.dx?g=sprintf("d%.1f",c.pub.dx):void 0!==c.pub.x&&(g=sprintf("%.1f",c.pub.x));break;case "physical":c.pub.lcs?g=sprintf("%.1f",c.pub.lcs.x):
void 0!==c.pub.x&&(g=sprintf("%.1f",c.pub.x));break;default:c.pub.wcsconfig&&a.notNull(c.pub.wcsconfig.ra)?g=sprintf("%.6f",c.pub.wcsconfig.ra):void 0!==c.pub.x&&(g=sprintf("%.1f",c.pub.x))}$(q+"[name='"+f+"']").prop("readonly",v);break;case "ypos":switch(t){case "image":c.pub.preservedcoords&&void 0!==c.pub.dy?g=sprintf("d%.1f",c.pub.dy):void 0!==c.pub.y&&(g=sprintf("%.1f",c.pub.y));break;case "physical":c.pub.lcs?g=sprintf("%.1f",c.pub.lcs.y):void 0!==c.pub.y&&(g=sprintf("%.1f",c.pub.y));break;
default:c.pub.wcsconfig&&a.notNull(c.pub.wcsconfig.dec)?g=sprintf("%.6f",c.pub.wcsconfig.dec):void 0!==c.pub.y&&(g=sprintf("%.1f",c.pub.y))}$(q+"[name='"+f+"']").prop("readonly",v);break;case "radius":case "oradius":case "length":case "width":case "r1":switch(t){case "image":void 0!==c.pub[f]&&(g=u(c.pub[f]));break;case "physical":c.pub.lcs&&void 0!==c.pub.lcs[f]&&(g=u(c.pub.lcs[f]));break;default:c.pub.wcsconfig&&a.notNull(c.pub.wcsconfig.wcssizestr)?g=u(c.pub.wcsconfig.wcssizestr[0]):void 0!==c.pub[f]&&
(g=u(c.pub[f]))}$(q+"[name='"+f+"']").prop("readonly",v);break;case "height":case "r2":switch(t){case "image":void 0!==c.pub[f]&&(g=u(c.pub[f]));break;case "physical":c.pub.lcs&&void 0!==c.pub.lcs[f]&&(g=u(c.pub.lcs[f]));break;default:c.pub.wcsconfig&&a.notNull(c.pub.wcsconfig.wcssizestr)?g=u(c.pub.wcsconfig.wcssizestr[1]):void 0!==c.pub[f]&&(g=u(c.pub[f]))}$(q+"[name='"+f+"']").prop("readonly",v);break;case "wcssys":case "savewcs":z=$(q).find("[name='"+f+"']");if(!z.find("option").length)for(e=0;e<
a.wcssyss.length;e++)z.append("<option>"+a.wcssyss[e]+"</option>");h="savewcs"===f?a.globalOpts.regSaveWCS||t:t;z.find("option").each(function(b,a){h===a.value&&(g=a.value)});break;case "wcsunits":c.pub.wcsunits&&(g=c.pub.wcsunits);break;case "childtext":c.params.children&&0<c.params.children.length&&(g=x(c.params.children[0].obj.text));break;case "text":void 0!==c.pub[f]&&(g=x(u(c.pub[f])));break;case "id":p?g="selected":void 0!==c.pub.id&&(g=String(c.pub.id),$(l).css("width",g.length+"ch"));break;
case "tags":void 0!==c.pub[f]&&(g=u(c.pub[f]));break;case "savefile":g=$(q).data("savefile")||d.tmp.saveregionsFile||"js9.reg";window.electron&&!g.match(/.*\//)&&(g=window.electron.currentDir+"/"+g);break;case "selectfilter":g=$(q).data("selectfilter");break;case "selectshape":case "selectcolor":case "selecttag":case "selectwcs":case "selectgroup":a.Regions.regionsConfigSetSelectMenu(d,$(q),f);break;default:void 0!==c.pub[f]&&(g=u(c.pub[f]))}$(l).val(g)});(p||!this.raw.wcs||0>this.raw.wcs)&&$(q).find("[name='wcssys']").hide();
...
e[0])}a.isNull(e[1])?(e[1]=1,e[2]=Infinity):e[1]=""===e[1]?-Infinity:parseFloat(e[1]);a.isNull(e[2])?e[2]=e[1]:e[2]=""===e[2]?Infinity:parseFloat(e[2]);var g={active:!0,red:f._r,green:f._g,blue:f._b,alpha:255*f._a,min:e[1],max:e[2]};"string"===typeof e[0]&&(g.name=e[0])}else"object"===typeof e&&(g=e);d.push(g)}d.sort(function(b,a){return b.min-a.min});return d};a.lookupStaticColor=function(a,b,d){var c={red:0,green:0,blue:0,alpha:0};if(a&&a.staticObj){c=a.params.nocolor||c;if(b<a.staticObj.colors[0].min)return c;

...
(this.raw.header.LTM1_1||0))}catch(A){t=0}t&&(t=180*-t/Math.PI,g.angle-=t)}}for(;0>g.angle;)g.angle+=360;for(;360<=g.angle;)g.angle-=360;q=b.scaleX/p;t=b.scaleY/p;d.group&&(q*=d.group.scaleX,t*=d.group.scaleY);switch(g.shape){case "annulus":g.shape="annulus";g.radii=[];"image"!==g.imsys&&(g.lcs.radii=[]);g.imstr="annulus("+m(v)+","+m(w)+",";var x="annulus "+g.x+" "+g.y+" ";r=b.getObjects();var y=r.length;for(d=0;d<y;d++)h=r[d].radius*q,t=h*u,g.imstr+=m(t),x+=g.x+" "+g.y+" "+(g.x+h)+" "+g.y+" ",g.imstr=
d===y-1?g.imstr+")":g.imstr+",",g.radii.push(h),"image"!==g.imsys&&g.lcs.radii.push(t);break;case "box":case "cross":g.width=b.width*q;g.height=b.height*t;t=g.width*u;x=g.height*u;"image"!==g.imsys&&(g.lcs.width=t,g.lcs.height=x);g.imstr=g.shape+"("+m(v)+","+m(w)+","+m(t)+","+m(x)+","+g.angle.toFixed(4)+")";x=g.shape+" "+g.x+" "+g.y+" "+g.x+" "+g.y+" "+(g.x+g.width)+" "+g.y+" "+g.x+" "+g.y+" "+g.x+" "+(g.y+g.height)+" "+g.angle*Math.PI/180;break;case "circle":g.radius=b.radius*q;t=g.radius*u;"image"!==
g.imsys&&(g.lcs.radius=t);g.imstr="circle("+m(v)+","+m(w)+","+m(t)+")";x="circle "+g.x+" "+g.y+" "+g.x+" "+g.y+" "+(g.x+g.radius)+" "+g.y;break;case "ellipse":g.r1=b.width*q/2;g.r2=b.height*t/2;t=g.r1*u;x=g.r2*u;"image"!==g.imsys&&(g.lcs.r1=t,g.lcs.r2=x);g.imstr="ellipse("+m(v)+","+m(w)+","+m(t)+","+m(x)+","+g.angle.toFixed(4)+")";x="ellipse "+g.x+" "+g.y+" "+g.x+" "+g.y+" "+(g.x+g.r1)+" "+g.y+" "+g.x+" "+g.y+" "+g.x+" "+(g.y+g.r2)+" "+g.angle*Math.PI/180;break;case "point":g.width=b.width*q;g.height=
b.height*t;t=g.width*u;x=g.height*u;"image"!==g.imsys&&(g.lcs.width=t,g.lcs.height=x);g.imstr="point("+m(v)+","+m(w)+")";x="point "+g.x+" "+g.y;break;case "line":case "polygon":g.imstr=g.shape+"(";x=g.shape+" ";g.pts=[];"image"!==g.imsys&&(g.lcs.pts=[]);for(d=0;d<b.points.length;d++)0<d&&(g.imstr+=",",x+=" "),t=this.displayToImagePos({x:r.x+b.left+b.points[d].x*b.scaleX,y:r.y+b.top+b.points[d].y*b.scaleY}),t=a.rotatePoint(t,h,{x:g.x,y:g.y}),"image"===g.imsys?g.imstr+=m(t.x)+","+m(t.y):(u=this.imageToLogicalPos(t),
q=u.x,u=u.y,g.imstr+=m(q)+","+m(u),g.lcs.pts.push({x:q,y:u})),x+=t.x+" "+t.y,g.pts.push(t),"line"===g.shape&&(0===d?y=0:(q=g.pts[d-1],y+=Math.sqrt((t.x-q.x)*(t.x-q.x)+(t.y-q.y)*(t.y-q.y))));if("line"===g.shape&&a.notNull(y)){g.imstr+=') {"size":'+m(y)+',"units":"pixels"';if(2===g.pts.length){for(t=180*Math.atan2(g.pts[1].y-g.pts[0].y,g.pts[1].x-g.pts[0].x)/Math.PI;0>t;)t+=360;var B=',"posang":'+t.toFixed(4)+',"posunits":"degrees"';g.imstr+=B}g.imstr+="}"}else g.imstr+=")";g.angle=0;break;case "text":g.imstr=
"text("+m(v)+","+m(w)+',"'+b.text+'",'+g.angle.toFixed(4)+")",x="text "+g.x+" "+g.y+' "'+b.text+'" '+g.angle*Math.PI/180,g.text=b.text}this.raw.wcs&&0<this.raw.wcs&&(n(this.raw.wcs,k,g,x,B,f,g),b.params.wcsconfig&&b.params.wcsconfig.wcssys&&(m=a.globalOpts.xeqPlugins,a.globalOpts.xeqPlugins=!1,r=this.getWCSSys(),a.notWCS(b.params.wcsconfig.wcssys)||(this.setWCSSys(b.params.wcsconfig.wcssys,!1),n(this.raw.wcs,k,g,x,B,f,b.params.wcsconfig)),g.wcsconfig=$.extend(!0,{},b.params.wcsconfig),this.setWCSSys(r,
!1),a.globalOpts.xeqPlugins=m));g.data=b.params.data;b.set("pub",g);b.params.winid&&($(b.params.winid).is(":visible")?this.initRegionsForm(b):b.params.winid=null);if(f.nocb)return g;if(!b.params.parent&&!e.match(l)){if(this.params.xeqonchange&&k.show&&k.opts.onchange)try{this.params.xeqonchange=!1,a.xeqByName(k.opts.onchange,window,this,g)}catch(A){a.log("error in onchange: %s [%s]\n%s",this.id,A.message,a.strace(A))}finally{this.params.xeqonchange=!0}this.xeqPlugins("shape","on"+c+"change",g)}if("regions"===
c&&a.globalOpts.regToClipboard){switch(e){case "update":d=g.parent||g.id;break;default:d=null}if(a.notNull(d)){try{var z=this.listRegions(d,{mode:1,includedcoords:!0})}catch(A){z=null}z&&(a.clipboard=z)}}"regions"===c&&"wcs"===e&&this._updateMultiDialogs(!0);return g};a.Fabric.removeShapes=function(c,b,d){var e=this,f,h={mode:1,includedcoords:!0},g=[];if(f=this.getShapeLayer(c)){var k=f.canvas;d=d||{};"regions"===c&&a.globalOpts.unremoveReg&&(this.regstack.push(this.listRegions(b,h,c)),this.regstack.length>

...
e.keys.length;a++)g.keys[e.keys[a]]=this.expandMacro("$"+e.keys[a],c);g.id=this.expandMacro("$id");g.image=this.file;g.fits=this.fitsFile;g.rtype=e.rtype;switch(b.helper.type){case "nodejs":case "socket.io":c=e.xclass?e.xclass+":"+e.name:e.name;break;default:c="runAnalysis"}b.waiting(!0,this.display);b.helper.send(c,g,function(a){var c,k;b.waiting(!1);c="object"===typeof a?a:0<=a.search(b.analOpts.epattern)?{stderr:a}:{stdout:a};c.errcode=c.errcode||0;if(d)d(c.stdout,c.stderr,c.errcode,e);else{if(c.stderr)if(a=
c.stderr,0<=a.search(/WARNING:/i)&&0>a.search(/ERROR:/i))b.log(a);else{h(a,b.analOpts.epattern);return}else if(c.errcode)if(a=sprintf("ERROR: running %s [%s]",e.name,c.errcode),c.stdout)b.log(a);else{h(a,b.analOpts.epattern);return}switch(e.rtype){case "text":case void 0:f.displayAnalysis("text",c.stdout,{divid:b.globalOpts.analysisDiv});break;case "plot":f.displayAnalysis("plot",c.stdout,{divid:b.globalOpts.analysisDiv});break;case "alert":c.stdout&&alert(c.stdout);break;case "fits":case "png":(k=
c.stdout.split(/\s+/))&&k[0]&&(a=b.cleanPath(k[0]),"/"!==a.charAt(0)&&(a=b.InstallDir(a)),c={proxyFile:a},k[1]&&(k=b.cleanPath(k[1]),c.parentFile=k),c.fits2fits=!1,b.Load(a,c,{display:f.display}));break;case "regions":(k=c.stdout.split(/\s+/))&&k[0]&&(a=b.cleanPath(k[0]),g={responseType:"text"},b.fetchURL(null,a,g,function(a,b){f.addShapes("regions",a,b)}));break;case "catalog":(k=c.stdout.split(/\s+/))&&k[0]&&(a=b.cleanPath(k[0]),g={responseType:"text"},b.fetchURL(null,a,g,function(a,b){f.loadCatalog(null,
...
g=p(f,h),g=c.pointOffset({x:g.x,y:g.y}),0>g.left||g.left>a.width()||(f=sprintf("<div class='plotAnnotation' style='position: absolute; left: %spx; top:%spx; color: %s; font-size: small'>%s</div>",g.left,g.top+-25,k,"&darr;"+f.text),a.append(f))},y=function(a,b,c,d){var e;switch(c){case "x":e={xaxis:{type:d,autorange:!0}};break;case "y":e={yaxis:{type:d,autorange:!0}}}Plotly.restyle(a.attr("id"),e)},v=function(a){var c,d,e,f=a.data,g=a.annotations.color||b.plotOpts.annotateColor,h=[];for(c=0;c<a.annotations.data.length;c++)if(d=

...
c.pathOffset={x:b.left+c.width/2,y:b.top+c.height/2});c.setCoords()};a.Fabric.print=function(c){var b,d=0,e=sprintf("width=%s,height=%s,menubar=1,toolbar=1,status=0,scrollbars=1,resizable=1",this.display.canvasjq.attr("width"),this.display.canvasjq.attr("height"));c=c||{};if("string"===typeof c)try{c=JSON.parse(c)}catch(k){a.error("can't parse print opts: "+c,k)}var f=this.display.canvas.toDataURL("image/png");var h="<html><body style='padding: 0px; margin: 0px' onload='window.print(); return false'>";
var g=sprintf("<div style='position:absolute; left:%spx; top:%spx'>",0,d);h+=g+"<img src='"+f+"'></div>";for(b in this.layers)this.layers.hasOwnProperty(b)&&(f=this.layers[b],"main"===f.dlayer.dtype&&f.show&&(h+=""+g+f.dlayer.canvas.toSVG()+"</div>"));(void 0===c.colorbar||c.colorbar)&&(c=this.display.pluginInstances.JS9Colorbar)&&c.isActive()&&(d+=2,f=c.colorbarjq[0].toDataURL("image/png"),d+=this.display.height,g=sprintf("<div style='position:absolute; left:%spx; top:%spx'>",0,d),h+=g+"<img src='"+
f+"'></div>",c.textjq&&c.textjq[0]&&(f=c.textjq[0].toDataURL("image/png"),d+=c.colorbarjq.height()+1,g=sprintf("<div style='position:absolute; left:%spx; top:%spx'>",0,d),h+=g+"<img style='width:"+this.display.width+"px;'src='"+f+"'></div>"));h+="</body></html>";(g=window.open(void 0,this.id,e))?g.document?(g.document.open(),g.document.write(h),g.document.close()):a.error("no method available for drawing image into print window"):a.error("could not create print window (check your pop-up blockers)")};
...
"").replace(/\n/g,"\\n")};if(c&&c.pub&&c.pub.wcsconfig&&c.pub.wcsconfig.wcssys)var w=c.pub.wcsconfig.wcssys;else w=this.params.wcssys,c=q;b=b||{};if(c.params.winid)var t=c.params.winid;else b.winid&&(t=b.winid);if(t){var u="#"+$(t).attr("id")+" .regionsConfigForm ";if($(u).length){n=$(u).data("multi")?!0:b.multi;$(u+"."+c.pub.shape).each(function(a,b){$(b).removeClass("nodisplay")});$(u+".val").each(function(b,m){h="";f=$(m).attr("name");switch(f){case "x":case "y":c.pub.lcs&&void 0!==c.pub.lcs[f]?
h=r(c.pub.lcs[f]):void 0!==c.pub[f]&&(h=r(c.pub[f]));break;case "radii":c.pub.radii&&(h=a.notWCS(w)||!c.pub.wcsconfig.wcsstr?c.pub.imstr.replace(/^annulus\(/,"").replace(/\)$/,"").split(",").slice(2).join(","):c.pub.wcsconfig.wcsstr.replace(/^annulus\(/,"").replace(/\)$/,"").split(",").slice(2).join(","));break;case "pts":c.pub.pts?c.pub.pts.forEach(function(a){h&&(h+=", ");h+=a.x.toFixed(2)+", "+a.y.toFixed(2)}):c.pub.imstr&&(h=c.pub.imstr.replace(/^.*\(/,"").replace(/\)$/,""));break;case "linelength":if(c.pub.pts&&

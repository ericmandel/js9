...
point:{name:"point"},polygon:{name:"polygon"},text:{name:"text"},sep2:"------",loadRegions:{name:"load regions"},saveRegions:{name:"save regions"},listRegions:{name:"list regions"},deleteRegions:{name:"delete regions"},listonchange:{name:"list on change"},xeqonchange:{name:"xeq on change"}};c&&c.params.listonchange&&(d.listonchange.icon="sun");c&&c.params.xeqonchange&&(d.xeqonchange.icon="sun");return{callback:function(c){b().forEach(function(b){var d=b.image;if(d)switch(c){case "deleteRegions":d.removeShapes("regions",
...
this.id,s.message,a.strace(s))}finally{this.params.xeqonchange=!0}if(this.params.xeqonchange&&n.show&&n.opts.onchange)try{this.params.xeqonchange=!1,a.xeqByName(n.opts.onchange,window,this,l)}catch(t){a.log("error in onchange: %s [%s]\n%s",this.id,t.message,a.strace(t))}finally{this.params.xeqonchange=!0}f="on"+c+"change";for(g in p.pluginInstances)if(p.pluginInstances.hasOwnProperty(g)&&(c=p.pluginInstances[g],b=c.plugin.opts,c.isActive(f)))try{b[f].call(c,this,l)}catch(v){c.errLog(f,v)}return l};
...
k+=this.params.wcssys,m="wcs"),k+="; "+j+e.wcsstr):e.imstr&&(m!==e.imsys&&("none"!==m&&(k+="; "),k+=e.imsys,m=e.imsys),k+="; "+j+e.imstr),p&&(k+=g);1<b&&this.displayMessage("regions",k);return k};a.Regions.parseRegions=function(c){var b=[],d,e,f,g,h,j,k,m,p,r,l,n,q=/(annulus)|(box)|(circle)|(ellipse)|(polygon)|(point)|(text)/,s=/(fk4)|(fk5)|(icrs)|(galactic)|(ecliptic)|(image)|(physical)/,t=/\(\s*([^)]+?)\s*\)/,v=/(\{[^}]*\})/,u=/\s*,\s*/,w=function(c){c=a.saostrtod(c);var b=String.fromCharCode(a.saodtype());
...
0<=g.indexOf("#")?g.split("#")[0].trim().toLowerCase():g.trim().toLowerCase();d.cmd&&(d.isregion=0<=d.cmd.search(q));d.comment=g.split("#")[1];d.comment&&(d.comment=d.comment.trim().toLowerCase());if((h=v.exec(g))&&h[1])try{d.opts=JSON.parse(h[1].trim())}catch(B){d.opts={}}if((h=t.exec(g))&&h[1])d.args=h[1].split(u);d.isregion&&0===d.cmd.indexOf("-")&&(d.cmd=d.cmd.slice(1),d.comment=d.comment?d.comment+",exclude":"exclude");h=d;n=h.args.length;if(h.isregion){g=$.extend(!0,{},h.opts);g.shape=h.cmd;

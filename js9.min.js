...
keyboardActions:{b:"make selected region a background region",e:"make selected region an exclude region",f:"toggle full screen mode",i:"make selected region an include region",l:"list regions",n:"display next image",p:"copy physical coords to clipboard",s:"make selected region a source region",v:"copy pixel value to clipboard",w:"copy wcs coords to clipboard",o:"open a local FITS file","+":"zoom in","-":"zoom out",">":"zoom in","<":"zoom out"},mousetouchZoom:!1,pinchWait:8,pinchThresh:6,extendedPlugins:!0,
...
this.binning={bin:1,obin:1};this.raws=[];this.blend={active:void 0,mode:"normal",opacity:1};this.updateshapes=!1;a.waiting(!0,this.display.divjq[0]);switch(typeof d){case "object":this.source="fits";this.mkRawDataFromHDU(d,$.extend({},{file:d.filename},h));"zscale"===this.params.scaleclipping&&this.zscale(!0);this.mkSection();e=j(this,h);e.length&&this.mkSection.apply(this,e);h&&h.rgbFile?(this.rgbFile=h.rgbFile,$(this.png.image).on("load",function(){var d;if(g.png.image.width!==g.raw.width||g.png.image.height!==
g.raw.height)d=sprintf("rgb dims [%s,%s] don't match image [%s,%s]",g.png.image.width,g.png.image.height,g.raw.width,g.raw.height),a.error(d);g.mkOffScreenCanvas();g.displayImage("all",h);k.call(g,c)}).on("error",function(){a.waiting(!1);g.status.load="error";a.error("could not load image: "+g.id)}),this.png.image.src=this.rgbFile):(this.displayImage("all",h),k.call(this,c));break;case "string":this.source="fits2png";this.imtab="image";this.file=d.replace(/\/\.\//,"/");this.id0=this.file.split("/").reverse()[0];
this.id=a.getImageID(this.id0,this.display.id);this.status.load="loading";$(this.png.image).on("load",function(){g.mkOffScreenCanvas();g.mkRawDataFromPNG();"zscale"===g.params.scaleclipping&&g.zscale(!0);g.mkSection();e=j(g,h);e.length&&g.mkSection.apply(g,e);g.displayImage("all",h);k.call(g,c);a.DEBUG&&a.log("JS9 image: %s dims(%d,%d) min/max(%d,%d)",g.file,g.raw.width,g.raw.height,g.raw.dmin,g.raw.dmax)}).on("error",function(){a.waiting(!1);g.status.load="error";a.error("could not load image: "+
g.id)});this.png.image.src=d;break;default:a.log("unknown specification type for Load: "+typeof d)}};a.Image.prototype.getImageData=function(a){var b=null;if(a)if("array"===a)b=Array.prototype.slice.call(this.raw.data);else if("base64"===a){var b="",c=new Uint8Array(this.raw.data.buffer),e=c.byteLength;for(a=0;a<e;a++)b+=String.fromCharCode(c[a]);b=window.btoa(b)}else a&&"false"!==a&&(b=this.raw.data);return{id:this.id,file:this.file,fits:this.fitsFile||"",source:this.source,imtab:this.imtab,width:this.raw.width,
height:this.raw.height,bitpix:this.raw.bitpix,header:this.raw.header,hdus:this.hdus,data:b}};a.Image.prototype.closeImage=function(){var d,b,c,e,f,g=!1;c=a.images.length;var h=this.display,j=function(d){d.stderr?a.error(d.stderr):d.stdout&&a.log(d.stdout)};for(d=0;d<c;d++)if(this===a.images[d]){c=a.images[d];c===c.display.image&&(g=!0);if(g){c.clearMessage();c.display.context.clear();for(b in c.layers)c.layers.hasOwnProperty(b)&&c.layers[b].canvas.clear();c.display.image=null}c.xeqPlugins("image",
"onimageclose");switch(c.cmapObj.name){case "red":a.globalOpts.rgb.rim=null;break;case "green":a.globalOpts.rgb.gim=null;break;case "blue":a.globalOpts.rgb.bim=null}if(a.fits.cleanupFITSFile)for(b=0;b<c.raws.length;b++)e=c.raws[b],e.hdu&&e.hdu.fits&&(f=a.lookupVfile(e.hdu.fits.vfile),1>=f.length&&a.fits.cleanupFITSFile(e.hdu.fits,!0));c.proxyFile&&a.Send("removeproxy",{cmd:"js9Xeq removeproxy "+c.proxyFile},j);c.rgb=null;c.offscreen=null;c.raw=null;c.colorData=null;c.colorCells=null;c.psColors=null;
c=c.psInverse=null;a.images.splice(d,1);break}if(g)for(d=0;d<a.images.length;d++)if(c=a.images[d],h===c.display){c.displayImage("all");c.refreshLayers();break}};a.Image.prototype.mkOffScreenCanvas=function(){if(!this.png||!this.png.image)return this;this.offscreen={};this.offscreen.canvas=document.createElement("canvas");this.offscreen.canvas.setAttribute("width",this.png.image.width);this.offscreen.canvas.setAttribute("height",this.png.image.height);this.offscreen.context=this.offscreen.canvas.getContext("2d");
...
alert("could not read off-screen image data [same-origin policy violation?]")}return this};a.Image.prototype.initLCS=function(a){var b=[[0,0,0],[0,0,0],[0,0,0]],c=function(a){var d,c,b=0,j=0,k=[[0,0,0],[0,0,0],[0,0,0]],m=a[0][0]*a[1][1];for(d=0;3>d;d++)for(c=0;2>c;c++)if(void 0===a[d][c]||isNaN(a[d][c]))return null;0<=m?b+=m:j+=m;m=-a[0][1]*a[1][0];0<=m?b+=m:j+=m;d=b+j;if(0===d||1E-15>Math.abs(d/(b-j)))return null;d=1/d;k[0][0]=a[1][1]*d;k[1][0]=-a[1][0]*d;k[0][1]=-a[0][1]*d;k[1][1]=a[0][0]*d;k[2][0]=
...
new Uint8Array(b);for(d=0;d<b;d++){switch(f){case 0:g=e[c];c+=1;f=1;break;case 1:g=e[c];c+=1;f=2;break;case 2:g=e[c];c+=2;f=0;break;case 3:g=e[c+1],c+=2,f=1}this.raw.data[d]=g}break;case 16:case -16:16===this.raw.bitpix?(this.raw.data=new Int16Array(b),h=DataView.prototype.getInt16):(this.raw.data=new Uint16Array(b),h=DataView.prototype.getUint16);for(d=0;d<b;d++){switch(f){case 0:k[0]=e[c];k[1]=e[c+1];g=h.call(m,0,j);c+=2;f=2;break;case 1:k[0]=e[c];k[1]=e[c+1];g=h.call(m,0,j);c+=3;f=0;break;case 2:k[0]=
e[c];k[1]=e[c+2];g=h.call(m,0,j);c+=3;f=1;break;case 3:k[0]=e[c+1],k[1]=e[c+2],g=h.call(m,0,j),c+=3,f=2}this.raw.data[d]=g}break;case 32:case -32:32===this.raw.bitpix?(this.raw.data=new Int32Array(b),h=DataView.prototype.getInt32):(this.raw.data=new Float32Array(b),h=DataView.prototype.getFloat32);for(d=0;d<b;d++){switch(f){case 0:k[0]=e[c];k[1]=e[c+1];k[2]=e[c+2];k[3]=e[c+4];g=h.call(m,0,j);c+=5;f=1;break;case 1:k[0]=e[c];k[1]=e[c+1];k[2]=e[c+3];k[3]=e[c+4];g=h.call(m,0,j);c+=5;f=2;break;case 2:k[0]=
e[c];k[1]=e[c+2];k[2]=e[c+3];k[3]=e[c+4];g=h.call(m,0,j);c+=6;f=0;break;case 3:k[0]=e[c+1],k[1]=e[c+2],k[2]=e[c+3],k[3]=e[c+5],g=h.call(m,0,j),c+=6,f=1}this.raw.data[d]=g}break;case -64:this.raw.data=new Float64Array(b);for(d=0;d<b;d++){switch(f){case 0:case 4:k[0]=e[c];k[1]=e[c+1];k[2]=e[c+2];k[3]=e[c+4];k[4]=e[c+5];k[5]=e[c+6];k[6]=e[c+8];k[7]=e[c+9];g=m.getFloat64(0,j);c+=10;f=2;break;case 1:case 5:k[0]=e[c];k[1]=e[c+1];k[2]=e[c+3];k[3]=e[c+4];k[4]=e[c+5];k[5]=e[c+7];k[6]=e[c+8];k[7]=e[c+9];g=
m.getFloat64(0,j);c+=11;f=0;break;case 2:case 6:k[0]=e[c];k[1]=e[c+2];k[2]=e[c+3];k[3]=e[c+4];k[4]=e[c+6];k[5]=e[c+7];k[6]=e[c+8];k[7]=e[c+10];g=m.getFloat64(0,j);c+=11;f=1;break;case 3:case 7:k[0]=e[c+1],k[1]=e[c+2],k[2]=e[c+3],k[3]=e[c+5],k[4]=e[c+6],k[5]=e[c+7],k[6]=e[c+9],k[7]=e[c+10],g=m.getFloat64(0,j),c+=11,f=2}this.raw.data[d]=g}break;default:a.error("unsupported bitpix in PNG file: "+this.raw.bitpix)}this.dataminmax();this.offscreen.img=null;this.initWCS();this.initLCS();return this}};a.Image.prototype.mkRawDataFromHDU=
...
e.display&&(e.resize?(e.resize.left+=g,e.resize.top+=h):e.resize={left:g,top:h},e===e.display.image))for(f in e.layers)e.layers.hasOwnProperty(f)&&(b=e.layers[f],"main"===b.dlayer.type&&!b.json&&(b.canvas.getObjects().forEach(j),b.canvas.renderAll()));a.bugs.webkit_resize&&this.divjq.css("width",this.width+a.RESIZEFUDGE).css("height",this.height+a.RESIZEFUDGE);if(this.image&&(a.globalOpts.resizeRedisplay||!this.resizing))this.image.displayImage("all",c),this.image.refreshLayers();return this};a.Display.prototype.inResize=
function(d){return a.globalOpts.resizeHandle&&d.x+a.RESIZEDIST>=this.divjq.width()&&d.y+a.RESIZEDIST>=this.divjq.height()?!0:!1};a.Display.prototype.nextImage=function(d){var b,c;d=d||1;if(this.image){for(b=0;b<a.images.length&&this.image!==a.images[b];b++);for(c=b+1;c!==b&&!(c>=a.images.length&&(c=0),a.images[c].display===this);c+=d);b!==c&&(d=a.images[c],d.displayImage("all"),d.refreshLayers(),d.clearMessage())}};a.Display.prototype.loadSession=function(d){var b=this,c,e=function(a){var d,e,f,m=
function(){a.refreshLayers()};if(c.layers&&c.layers.length)for(d=0;d<c.layers.length;d++)f=c.layers[d],e=b.newShapeLayer(f.name,f.dopts),a.addShapes(f.name,[]),e.canvas.loadFromJSON(f.json,m)},f=function(d){d.file||a.error("session does not contain a filename");c=d;c.params.onload=e;c.params.display&&delete c.params.display;a.Load(c.file,c.params,{display:b.id})};a.waiting(!0,this.divjq[0]);"object"===typeof d?f(d):$.ajax({url:d,cache:!1,dataType:"json",mimeType:"application/json",async:!1,success:function(a){f(a)},
...
this;d&&(this.type=d);if(this.socket){try{this.socket.disconnect()}catch(c){a.log("warning: can't disconnect from socket")}this.socket=null}this.url=a.globalOpts.helperURL?0<=a.globalOpts.helperURL.search(/:\/\//)?a.globalOpts.helperURL:a.globalOpts.helperProtocol+a.globalOpts.helperURL:document.domain?a.globalOpts.helperProtocol+document.domain:a.globalOpts.helperProtocol+"localhost";switch(this.type){case "none":this.connected=null;$(document).trigger("JS9:ready",{type:"none",status:"OK"});a.Preload(!0);
break;case "get":case "post":a.globalOpts.helperCGI||a.error("cgi script name missing for helper");this.url+="/"+a.globalOpts.helperCGI;this.helper=this.connected=!0;a.DEBUG&&a.log("JS9 helper: connect: "+this.type);$(document).trigger("JS9:ready",{type:"get",status:"OK"});a.Preload(!0);break;case "sock.io":case "nodejs":a.globalOpts.helperPort||a.error("port missing for helper");this.url+=":"+a.globalOpts.helperPort;this.sockurl=this.url+"/socket.io/socket.io.js";$.ajax({url:this.sockurl,dataType:"script",
timeout:a.globalOpts.htimeout,success:function(){var d,c;b.socket=io.connect(b.url,{reconnection:!0,reconnectionDelay:1E4,reconnectionDelayMax:1E4,reconnectionAttempts:6,timeout:a.globalOpts.htimeout});b.socket.on("connect",function(){b.connected=!0;b.helper=!0;c=[];for(d=0;d<a.displays.length;d++)c.push(a.displays[d].id);b.socket.emit("displays",{displays:c},function(a){b.pageid=a});$(document).trigger("JS9:ready",{type:"socket.io",status:"OK"});a.Preload(!0);a.DEBUG&&a.log("JS9 helper: connect: "+
...
c.dlayer.canvas;c.canvas.calcOffset()}return c};a.Fabric._parseShapeOptions=function(d,b,c){var e,f,g,h,j,k,m,n={},q={};h="main"===this.display.layers[d].dtype?this.rgb.sect.zoom:1;j=h/("main"===this.display.layers[d].dtype?this.binning.bin||1:1);if(b.remove)return{remove:b.remove};q.tags=[];if(b.tags)if("string"===typeof b.tags){f=b.tags.toLowerCase().split(",");for(d=0;d<f.length;d++)q.tags[d]=f[d].trim()}else if($.isArray(b.tags))for(d=0;d<b.tags.length;d++)q.tags[d]=b.tags[d].trim();void 0!==
b.angle&&(n.angle=-b.angle);void 0!==b.x&&void 0!==b.y&&(e=this.imageToDisplayPos(b),n.left=e.x,n.top=e.y);void 0!==b.left&&(n.left=b.left);void 0!==b.top&&(n.top=b.top);void 0===n.left&&(n.left=c&&void 0!==c.left?c.left:this.display.canvasjq.attr("width")/2-1);void 0===n.top&&(n.top=c&&void 0!==c.top?c.top:this.display.canvasjq.attr("height")/2-1+1);b.dx&&(n.left+=b.dx);b.dy&&(n.top-=b.dy);switch(b.shape){case "annulus":q.radii=[];if(void 0!==b.radii)if("string"===typeof b.radii){q.radii=b.radii.replace(/ /g,
"").split(",");for(e=d=0;d<q.radii.length;d++)""!==q.radii[d]&&(q.radii[e++]=parseInt(q.radii[d],10))}else q.radii=b.radii;else{b.ireg&&a.SCALEIREG&&(b.iradius&&(b.iradius/=j),b.oradius&&(b.oradius/=j));h=(b.oradius-b.iradius)/b.nannuli;j=b.nannuli+1;for(d=0;d<j;d++)f=b.iradius+h*d,q.radii.push(f)}break;case "box":b.ireg&&a.SCALEIREG&&(b.width&&(b.width/=j),b.height&&(b.height/=j));break;case "circle":b.ireg&&a.SCALEIREG&&b.radius&&(b.radius/=j);break;case "ellipse":b.ireg&&a.SCALEIREG&&(b.r1&&(b.r1/=
...
parseFloat(g[d]);b.pts=[];for(e=d=0;d<f;d+=2,e++)b.pts[e]={x:g[d],y:g[d+1]}}f=b.pts.length;for(d=0;d<f;d++)b.pts[d]=this.imageToDisplayPos(b.pts[d]);b.left&&b.top?g={x:b.left,y:b.top}:(g=a.centerPolygon(b.pts),n.left=g.x,n.top=g.y);b.points=[];for(d=0;d<f;d++)e={x:(b.pts[d].x-g.x)/h,y:(b.pts[d].y-g.y)/h},b.points.push(e)}else"polygon"===b.shape&&b.polypoints?b.points=b.polypoints:"line"===b.shape&&b.linepoints&&(b.points=b.linepoints);if(b.ireg&&a.SCALEIREG){f=b.points.length;for(d=0;d<f;d++)b.points[d].x/=
...
"onmouseover",d)))};a.mouseOutCB=function(d){var b=d.data.image;d.preventDefault();b&&(b.display.displayConjq.blur(),a.specialKey(d)||(b.pos=a.eventToDisplayPos(d),b.ipos=b.displayToImagePos(b.pos),a.specialKey(d)||b.xeqPlugins("mouse","onmouseout",d)))};a.wheelCB=function(d){var b=d.data,c=b.image;b.mousetouchZoom&&(c&&a.hasOwnProperty("MouseTouch")&&a.MouseTouch.Actions["wheel zoom"])&&(a.MouseTouch.Actions["wheel zoom"](c,d),d.preventDefault())};a.keyPressCB=function(a){var b=a.data.image;a.preventDefault();
b&&b.xeqPlugins("keypress","onkeypress",a)};a.keyDownCB=function(d){var b,c=d.data.image;a.hasOwnProperty("Keyboard")&&(b=c?c.ipos:{x:null,y:null},a.Keyboard.action(c,b,d));c&&(c.layer&&c.layers[c.layer].opts.usekeyboard&&a.Regions.keyDownCB(c,c.ipos,d,c.layer),c.xeqPlugins("keypress","onkeydown",d))};a.dragenterCB=function(a,b){b.stopPropagation();b.preventDefault()};a.dragoverCB=function(a,b){b.stopPropagation();b.preventDefault()};a.dragexitCB=function(a,b){b.stopPropagation();b.preventDefault()};

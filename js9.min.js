...
!1;a.waiting(!0,this.display.divjq[0]);switch(typeof d){case "object":this.source="fits";this.mkRawDataFromHDU(d,$.extend({},{file:d.filename},h));"zscale"===this.params.scaleclipping&&this.zscale(!0);this.mkSection();e=j(this,h);e.length&&this.mkSection.apply(this,e);h&&h.rgbFile?(this.rgbFile=h.rgbFile,$(this.png.image).on("load",function(){var d;if(g.png.image.width!==g.raw.width||g.png.image.height!==g.raw.height)d=sprintf("rgb dims [%s,%s] don't match image [%s,%s]",g.png.image.width,g.png.image.height,
g.raw.width,g.raw.height),a.error(d);g.mkOffScreenCanvas();g.displayImage("all",h);k.call(g,c)}).on("error",function(){a.waiting(!1);g.status.load="error";a.error("could not load image: "+g.id)}),this.png.image.src=this.rgbFile):(this.displayImage("all",h),k.call(this,c));break;case "string":this.source="fits2png";this.imtab="image";this.file=d.replace(/\/\.\//,"/");this.id0=this.file.split("/").reverse()[0];this.id=a.getImageID(this.id0,this.display.id);this.status.load="loading";$(this.png.image).on("load",
function(){g.mkOffScreenCanvas();g.mkRawDataFromPNG();"zscale"===g.params.scaleclipping&&g.zscale(!0);g.mkSection();e=j(g,h);e.length&&g.mkSection.apply(g,e);g.displayImage("all",h);k.call(g,c);a.DEBUG&&a.log("JS9 image: %s dims(%d,%d) min/max(%d,%d)",g.file,g.raw.width,g.raw.height,g.raw.dmin,g.raw.dmax)}).on("error",function(){a.waiting(!1);g.status.load="error";a.error("could not load image: "+g.id)});this.png.image.src=d;break;default:a.log("unknown specification type for Load: "+typeof d)}};
...
data:b}};a.Image.prototype.closeImage=function(){var d,b,c,e,f,g=!1;c=a.images.length;var h=this.display,j=function(d){d.stderr?a.error(d.stderr):d.stdout&&a.log(d.stdout)};for(d=0;d<c;d++)if(this===a.images[d]){c=a.images[d];c===c.display.image&&(g=!0);if(g){c.clearMessage();c.display.context.clear();for(b in c.layers)c.layers.hasOwnProperty(b)&&c.layers[b].canvas.clear();c.display.image=null}c.xeqPlugins("image","onimageclose");switch(c.cmapObj.name){case "red":a.globalOpts.rgb.rim=null;break;case "green":a.globalOpts.rgb.gim=
null;break;case "blue":a.globalOpts.rgb.bim=null}if(a.fits.cleanupFITSFile)for(b=0;b<c.raws.length;b++)e=c.raws[b],e.hdu&&e.hdu.fits&&(f=a.lookupVfile(e.hdu.fits.vfile),1>=f.length&&a.fits.cleanupFITSFile(e.hdu.fits,!0));c.proxyFile&&a.Send("removeproxy",{cmd:"js9Xeq removeproxy "+c.proxyFile},j);c.rgb=null;c.offscreen=null;c.raw=null;c.colorData=null;c.colorCells=null;c.psColors=null;c=c.psInverse=null;a.images.splice(d,1);break}if(g)for(d=0;d<a.images.length;d++)if(c=a.images[d],h===c.display){c.displayImage("all");
...
a.Image.prototype.initLCS=function(a){var b=[[0,0,0],[0,0,0],[0,0,0]],c=function(a){var d,c,b=0,j=0,k=[[0,0,0],[0,0,0],[0,0,0]],m=a[0][0]*a[1][1];for(d=0;3>d;d++)for(c=0;2>c;c++)if(void 0===a[d][c]||isNaN(a[d][c]))return null;0<=m?b+=m:j+=m;m=-a[0][1]*a[1][0];0<=m?b+=m:j+=m;d=b+j;if(0===d||1E-15>Math.abs(d/(b-j)))return null;d=1/d;k[0][0]=a[1][1]*d;k[1][0]=-a[1][0]*d;k[0][1]=-a[0][1]*d;k[1][1]=a[0][0]*d;k[2][0]=-(a[2][0]*k[0][0]+a[2][1]*k[1][0]);k[2][1]=-(a[2][0]*k[0][1]+a[2][1]*k[1][1]);return k};
...
this.raw.header.NAXIS2:a.error("NAXIS2 missing from PNG-based FITS header");this.raw.header.BITPIX?this.raw.bitpix=this.raw.header.BITPIX:a.error("BITPIX missing from PNG-based FITS header");"little"===this.raw.endian?j=!0:"big"===this.raw.endian?j=!1:a.error("js9Endian missing from PNG-based FITS header");this.object=this.raw.header.OBJECT;b=this.raw.width*this.raw.height;f=c%4;switch(this.raw.bitpix){case 8:this.raw.data=new Uint8Array(b);for(d=0;d<b;d++){switch(f){case 0:g=e[c];c+=1;f=1;break;
case 1:g=e[c];c+=1;f=2;break;case 2:g=e[c];c+=2;f=0;break;case 3:g=e[c+1],c+=2,f=1}this.raw.data[d]=g}break;case 16:case -16:16===this.raw.bitpix?(this.raw.data=new Int16Array(b),h=DataView.prototype.getInt16):(this.raw.data=new Uint16Array(b),h=DataView.prototype.getUint16);for(d=0;d<b;d++){switch(f){case 0:k[0]=e[c];k[1]=e[c+1];g=h.call(m,0,j);c+=2;f=2;break;case 1:k[0]=e[c];k[1]=e[c+1];g=h.call(m,0,j);c+=3;f=0;break;case 2:k[0]=e[c];k[1]=e[c+2];g=h.call(m,0,j);c+=3;f=1;break;case 3:k[0]=e[c+1],
k[1]=e[c+2],g=h.call(m,0,j),c+=3,f=2}this.raw.data[d]=g}break;case 32:case -32:32===this.raw.bitpix?(this.raw.data=new Int32Array(b),h=DataView.prototype.getInt32):(this.raw.data=new Float32Array(b),h=DataView.prototype.getFloat32);for(d=0;d<b;d++){switch(f){case 0:k[0]=e[c];k[1]=e[c+1];k[2]=e[c+2];k[3]=e[c+4];g=h.call(m,0,j);c+=5;f=1;break;case 1:k[0]=e[c];k[1]=e[c+1];k[2]=e[c+3];k[3]=e[c+4];g=h.call(m,0,j);c+=5;f=2;break;case 2:k[0]=e[c];k[1]=e[c+2];k[2]=e[c+3];k[3]=e[c+4];g=h.call(m,0,j);c+=6;
f=0;break;case 3:k[0]=e[c+1],k[1]=e[c+2],k[2]=e[c+3],k[3]=e[c+5],g=h.call(m,0,j),c+=6,f=1}this.raw.data[d]=g}break;case -64:this.raw.data=new Float64Array(b);for(d=0;d<b;d++){switch(f){case 0:case 4:k[0]=e[c];k[1]=e[c+1];k[2]=e[c+2];k[3]=e[c+4];k[4]=e[c+5];k[5]=e[c+6];k[6]=e[c+8];k[7]=e[c+9];g=m.getFloat64(0,j);c+=10;f=2;break;case 1:case 5:k[0]=e[c];k[1]=e[c+1];k[2]=e[c+3];k[3]=e[c+4];k[4]=e[c+5];k[5]=e[c+7];k[6]=e[c+8];k[7]=e[c+9];g=m.getFloat64(0,j);c+=11;f=0;break;case 2:case 6:k[0]=e[c];k[1]=
e[c+2];k[2]=e[c+3];k[3]=e[c+4];k[4]=e[c+6];k[5]=e[c+7];k[6]=e[c+8];k[7]=e[c+10];g=m.getFloat64(0,j);c+=11;f=1;break;case 3:case 7:k[0]=e[c+1],k[1]=e[c+2],k[2]=e[c+3],k[3]=e[c+5],k[4]=e[c+6],k[5]=e[c+7],k[6]=e[c+9],k[7]=e[c+10],g=m.getFloat64(0,j),c+=11,f=2}this.raw.data[d]=g}break;default:a.error("unsupported bitpix in PNG file: "+this.raw.bitpix)}this.dataminmax();this.offscreen.img=null;this.initWCS();this.initLCS();return this}};a.Image.prototype.mkRawDataFromHDU=function(d,b){var c,e,f,g,h,j,
k,m;b=b||{};$.isArray(d)||a.isTypedArray(d)||d instanceof ArrayBuffer?($.isArray(d[0])&&(d=d.reduce(function(a,d){return a.concat(d)})),g={image:d}):"object"===typeof d?g=d:a.error("unknown or missing input for HDU creation");g.data&&!g.image&&(g.image=g.data);g.image||a.error("data missing from JS9 FITS object"+JSON.stringify(g));2>g.naxis&&a.error("can't image a FITS file with less than 2 dimensions");this.raw&&(j=this.raw.width,k=this.raw.height,m=this.raw.bitpix);if(h=this.raws.length){b.rawid=
...
f=255*this.params.opacity):void 0!==this.params.alpha1&&void 0!==this.params.alpha2?(e=this.params.alpha1,f=this.params.alpha2):(e=255,f=0));j=b.y1-1;for(m=0;j>=b.y0;j--,m++){s=j*this.raw.width;q=m*b.zoom;h=b.x0;for(k=0;h<b.x1;h++,k++){if(E){if(g=y?y.colorData[s+h]:0,u=z?z.colorData[s+h]:0,D=A?A.colorData[s+h]:0,void 0===g||void 0===u||void 0===D)return a.globalOpts.rgb.active=!1,a.error("RGB images are incompatible. Turning off RGB mode.","",!1),a.Image.prototype.mkRGBImage.call(this),this}else c=
this.colorData[s+h];this.maskData&&(v=0<this.maskData[s+h]?e:f);n=k*b.zoom;for(l=0;l<b.zoom;l++){p=Math.floor(q+l);t=p*b.width;for(p=0;p<b.zoom;p++)r=Math.floor(n+p),r=4*(t+r),E?(d.data[r]=y?y.psColors[g][0]:0,d.data[r+1]=z?z.psColors[u][1]:0,d.data[r+2]=A?A.psColors[D][2]:0):(d.data[r]=this.psColors[c][0],d.data[r+1]=this.psColors[c][1],d.data[r+2]=this.psColors[c][2]),d.data[r+3]=v}}}return this};a.Image.prototype.blendImage=function(d,b,c){var e=/normal|multiply|screen|overlay|darken|lighten|color-dodge|color-burn|hard-light|soft-light|difference|exclusion|hue|saturation|color|luminosity|clear|copy|source-over|destination-over|source-in|destination-in|source-out|destination-out|source-atop|destination-atop|xor|lighter/i;
...
c||"image"===c)a.params.wcssys=a.params.wcssys0;break;case "physical":case "image":a.params.wcssys=d}return c};var m=function(a,d){var c;"table"===a.imtab?a.raw.hdu.table.filter&&!d.match(a.raw.hdu.table.filter)&&(d=d.match(/\]\[/)?d.slice(0,-1)+"&&"+a.raw.hdu.table.filter+"]":d+("["+a.raw.hdu.table.filter+"]")):"image"===a.imtab&&(c=a.file.match(/\[.*\]/))&&(d=d.match(/\[.*\]/)?d.replace(/\[.*\]/,c):d+c);return d},n=c.split("(");n[1]&&(n[1]=n[1].replace(/\)$/,""));switch(n[0]){case "id":k=f.display.divjq.attr("id");
...
e.name;break;default:b="runAnalysis"}a.waiting(!0,this.display.divjq[0]);a.helper.send(b,g,function(d){var b,g;a.waiting(!1);b="object"===typeof d?d:0<=d.search(a.analOpts.epattern)?{stderr:d}:{stdout:d};b.errcode=b.errcode||0;if(c)c(b.stdout,b.stderr,b.errcode,e);else{if(b.errcode||b.stderr)d=b.stderr?b.stderr:sprintf("ERROR: while executing %s [%s]",e.name,b.errcode),0<=d.search(/WARNING/i)?a.log(d):a.error(d,a.analOpts.epattern);switch(e.rtype){case "text":case void 0:f.displayAnalysis("text",
b.stdout,{divid:a.globalOpts.analysisDiv});break;case "plot":f.displayAnalysis("plot",b.stdout,{divid:a.globalOpts.analysisDiv});break;case "alert":b.stdout&&alert(b.stdout);break;case "fits":case "png":g=b.stdout.split(/\s+/);d=g[0].trim().replace(/\/\.\//,"/");"/"!==d.charAt(0)&&(d=a.InstallDir(d));b={proxyFile:d};g[1]&&(g=g[1].trim().replace(/\/\.\//,"/"),b.parentFile=g);a.Load(d,b,{display:f.display});break;case "none":break;default:a.error("unknown analysis result type: "+e.rtype)}}});return this}a.error("could not find analysis task: "+
d)}};a.Image.prototype.displayAnalysis=function(d,b,c){var e,f,g,h,j,k,m,n=a.lightOpts[a.LIGHTWIN],q=function(a,d){var c,b,e;if(!a.text)return null;b=a.x||0;if("%Y"===a.y.toUpperCase())for(c=1;c<d.length-1;c++){if(d[c][0]>b){e=Math.max(d[c-1][1],d[c][1],d[c+1][1]);break}}else e=a.y;return{x:b,y:e}},l=function(d,c,b){var e,f,g,h=b.data,k=b.annotations.color||a.plotOpts.annotateColor;d.find(".plotAnnotation").remove();for(e=0;e<b.annotations.data.length;e++)f=b.annotations.data[e],g=q(f,h),g=c.pointOffset({x:g.x,
y:g.y}),0>g.left||g.left>d.width()||(f=sprintf("<div class='plotAnnotation' style='position: absolute; left: %spx; top:%spx; color: %s; font-size: small'>%s</div>",g.left,g.top+-25,k,"&darr;"+f.text),d.append(f))};c=c||{};j=c.winformat;c.divid&&0<$("#"+c.divid).length&&(k=$("#"+c.divid));h=c.title||"";this&&!h&&(c=this.fitsFile||this.id,c=c.split("/").reverse()[0],h="AnalysisResults: "+c+sprintf(a.IDFMT,this.display.id));c="Analysis_"+a.uniqueID();switch(d){case "text":b="<div class='JS9Analysis'></div>"+
("<pre class='JS9AnalysisText'>"+(b||"")+"</pre>");b+="</div>";k?k.html(b):(j=j||n.textWin,e=a.lightWin(c,"inline",b,h,j));break;case "plot":try{f=JSON.parse(b)}catch(p){a.error("can't plot return data: "+b,p)}if(!f)return;b=sprintf("<div id='%s' class='JS9Analysis'><div id='%sPlot' class='JS9Plot' ></div></div>",c,c);k?k.html(b):(j=j||n.plotWin,e=a.lightWin(c,"inline",b,h,j));g=$("#"+c+" #"+c+"Plot");k&&(g.css("width",k.css("width")),g.css("height",k.css("height")),g.css("margin",0));if(f.data)switch(a.globalOpts.plotLibrary){case "plotly":j=
$.extend(!0,{},a.plotOpts,f.opts);f.label&&(j.title=f.label);m={x:[],y:[],type:"scatter"};3<=f.data[0].length&&(m.error_y={type:"data",array:[],visible:!0},f.points&&f.points.yerr&&f.points.yerr.color&&(m.error_y.color=f.points.yerr.color));for(b=0;b<f.data.length;b++)m.x.push(f.data[b][0]),m.y.push(f.data[b][1]),m.error_y&&m.error_y.array&&m.error_y.array.push(f.data[b][2]);if(a.plotOpts.annotate&&f.annotations){b=j;var n=f,r,s;c=n.data;h=n.annotations.color||a.plotOpts.annotateColor;var t=[];for(d=
0;d<n.annotations.data.length;d++)if(r=n.annotations.data[d],s=q(r,c))r={x:s.x,y:s.y,xref:"x",yref:"y",text:r.text,arrowcolor:h,font:{color:h},showarrow:!0,arrowhead:2,ax:0,ay:-30},t.push(r);b.annotations=t}try{Plotly.newPlot(g.attr("id"),[m],j)}catch(v){a.error("can't plot data (plotly)",v)}break;default:j=$.extend(!0,{},a.plotOpts,f.opts);a.plotOpts.annotate&&f.annotations&&(j.zoomStack.func=function(a){l(g,a,f)});f.color=f.color||j.color;try{m=$.plot(g,[f],j)}catch(u){a.error("can't plot data (flot)",
u)}a.plotOpts.annotate&&f.annotations&&l(g,m,f)}break;case "params":case "textline":k?a.allinone?k.html(b):$.ajax({url:b,cache:!1,dataType:"text",success:function(a){k.html(a)}}):(j="params"===d?j||n.paramWin:j||n.dpathWin,e=a.allinone?"inline":"ajax",e=a.lightWin(c,e,b,h,j))}return e};a.Image.prototype.loadAuxFile=function(d,b){var c=this,e,f,g,h,j=a.auxFiles.length,k=[],m=function(){c.aux[f.name]=f;a.Image.prototype.mkOffScreenCanvas.call(h);a.Image.prototype.mkRawDataFromPNG.call(h);if(b)try{a.xeqByName(b,
window,c,f)}catch(d){a.error("in aux mask onload callback",d)}a.DEBUG&&a.log("JS9 %s: %s dims(%d,%d) min/max(%d,%d)",h.type,h.file,h.raw.width,h.raw.height,h.raw.dmin,h.raw.dmax)},n=function(d){c.aux[f.name]=f;f.regions=d;if(b)try{a.xeqByName(b,window,c,f)}catch(e){a.error("in aux region onload callback",e)}},q=function(d){c.aux[f.name]=f;f.text=d;if(b)try{a.xeqByName(b,window,c,f)}catch(e){a.error("in aux text onload callback",e)}},l=function(d,c){a.log(sprintf("could not load auxiliary file: %s [%s]",
...
"onsetcolormap");return this};a.Image.prototype.getScale=function(){if(this.params.scale)return{scale:this.params.scale,scalemin:this.params.scalemin,scalemax:this.params.scalemax}};a.Image.prototype.setScale=function(d,b,c){var e=this,f=function(d){0<=a.scales.indexOf(d)?e.params.scale=d:a.error("unknown scale: "+d)};if(arguments.length){switch(arguments.length){case 1:f(d);break;case 2:this.params.scalemin=parseInt(d,10);this.params.scalemax=parseInt(b,10);this.mkColorData();break;default:f(d),
this.params.scalemin=parseInt(b,10),this.params.scalemax=parseInt(c,10),this.mkColorData()}this.displayImage("scaled")}a.globalOpts.extendedPlugins&&this.xeqPlugins("image","onsetscale");return this};a.Image.prototype.dataminmax=function(){var a,b,c=isNaN(this.params.scalemin)||void 0===this.params.scalemin,e=isNaN(this.params.scalemax)||void 0===this.params.scalemax;if("dataminmax"===this.params.scaleclipping){if(this.raw.dmin===this.params.scalemin||void 0===this.raw.dmin)c=!0;if(this.raw.dmax===
...
0<=q.search(/\[struct stat="OK"/)&&(b.cmdswitches=b.cmdswitches||"",b.cmdswitches+=" -i "+g)),r.test(d.raw.wcsinfo.ptype)||(j="owcs_"+a.uniqueID()+".txt",a.vfile(j,a.raw2FITS(n,!0)),h="awcs_"+a.uniqueID()+".txt",q=a.tanhdr(j,h,""),1<a.DEBUG&&a.log("tanhdr (reproj): %s %s -> %s",j,h,q),a.vunlink(j),0<=q.search(/\[struct stat="OK"/)&&(a.vunlink(e),e=h))}catch(y){}c.raw.hdu&&c.raw.hdu.vfile?(h=c.raw.hdu.vfile,c.raw.hdu.fits.extname?h+=sprintf("[%s]",c.raw.hdu.fits.extname):c.raw.hdu.fits.extnum&&0<c.raw.hdu.fits.extnum&&
...
parseInt(this.divjq.css("width"),10),this.oheight=parseInt(this.divjq.css("height"),10),this.divjq.css("width",this.width+a.RESIZEFUDGE).css("height",this.height+a.RESIZEFUDGE)),this.resizeSensor=new ResizeSensor(this.divjq,function(){var d=b.divjq.width(),e=b.divjq.height();a.bugs.webkit_resize&&(d-=a.RESIZEFUDGE,e-=a.RESIZEFUDGE);b.resize(d,e)}));this.context=this.canvas.getContext("2d");a.ANTIALIAS||(this.context.imageSmoothingEnabled=!1,this.context.mozImageSmoothingEnabled=!1,this.context.webkitImageSmoothingEnabled=
!1,this.context.msImageSmoothingEnabled=!1);this.tooltip=$("<div>").attr("id","tooltip_"+this.id).addClass("JS9Tooltip").appendTo(this.divjq);this.image=null;this.pluginInstances={};this.layers={};this.initMessages();this.blendMode=!1;this.mouseActions=a.globalOpts.mouseActions.slice(0);this.touchActions=a.globalOpts.touchActions.slice(0);this.mousetouchZoom=a.globalOpts.mousetouchZoom;this.divjq.on("mouseover",this,function(d){return a.mouseOverCB(d)});this.divjq.on("mousedown touchstart",this,function(d){return a.mouseDownCB(d)});
this.divjq.on("mousemove touchmove",this,function(d){return a.mouseMoveCB(d)});this.divjq.on("mouseup touchend",this,function(d){return a.mouseUpCB(d)});this.divjq.on("mouseout",this,function(d){return a.mouseOutCB(d)});this.divjq.on("keypress",this,function(d){return a.keyPressCB(d)});this.divjq.on("keydown",this,function(d){return a.keyDownCB(d)});this.divjq.on("wheel",this,function(d){return a.wheelCB(d)});this.divjq.on("dragenter",this,function(d){return a.dragenterCB(this.id,d)});this.divjq.on("dragover",
this,function(d){return a.dragoverCB(this.id,d)});this.divjq.on("dragexit",this,function(d){return a.dragexitCB(this.id,d)});this.divjq.on("drop",this,function(d){return a.dragdropCB(this.id,d,a.NewFITSImage)});this.divjq.on("contextmenu",this,function(){return!1});this.divjq.append('<div style="visibility:hidden; position:relative; top:-50;left:-50"> <input type="file" id="openLocalFile-'+this.id+'" multiple="true" onchange="javascript:for(var i=0; i<this.files.length; i++){JS9.Load(this.files[i], {display:\''+
...
(d+="<p>"+a.helper.pageid),d):sprintf("notConnected %s",a.helper.type)};a.Helper.prototype.connect=function(d){var b=this;d&&(this.type=d);if(this.socket){try{this.socket.disconnect()}catch(c){a.log("warning: can't disconnect from socket")}this.socket=null}this.url=a.globalOpts.helperURL?0<=a.globalOpts.helperURL.search(/:\/\//)?a.globalOpts.helperURL:a.globalOpts.helperProtocol+a.globalOpts.helperURL:document.domain?a.globalOpts.helperProtocol+document.domain:a.globalOpts.helperProtocol+"localhost";
...
this.url+=":"+a.globalOpts.helperPort;this.sockurl=this.url+"/socket.io/socket.io.js";$.ajax({url:this.sockurl,dataType:"script",timeout:a.globalOpts.htimeout,success:function(){var d,c;b.socket=io.connect(b.url,{reconnection:!0,reconnectionDelay:1E4,reconnectionDelayMax:1E4,reconnectionAttempts:6,timeout:a.globalOpts.htimeout});b.socket.on("connect",function(){b.connected=!0;b.helper=!0;c=[];for(d=0;d<a.displays.length;d++)c.push(a.displays[d].id);b.socket.emit("displays",{displays:c},function(a){b.pageid=
...
function(){b.connected=!0;b.helper=!0;1<a.DEBUG&&a.log("JS9 helper: reconnect")});b.socket.on("msg",a.msgHandler)},error:function(d,c,g){b.connected=!1;b.helper=!1;$(document).trigger("JS9:ready",{type:"socket.io",status:"error"});a.Preload(!0);a.DEBUG&&a.log("JS9 helper: connect failure: "+c+" "+g)}});break;default:a.error("unknown helper type: "+this.type)}};a.Helper.prototype.send=function(d,b,c){if(!this.connected)return null;b&&"object"===typeof b?(b.cookie=document.cookie,a.globalOpts.dataPath&&
...
1<b&&this.displayMessage("regions",k);return k};a.Regions.parseRegions=function(d){var b=[],c,e,f,g,h,j,k,m,n,q,l,p=/(annulus)|(box)|(circle)|(ellipse)|(line)|(polygon)|(point)|(text)/,r=/(fk4)|(fk5)|(icrs)|(galactic)|(ecliptic)|(image)|(physical)/,s=/\(\s*([^)]+?)\s*\)/,t=/(\{[^}]*\})/,v=/\s*,\s*/,u=function(b){b=a.saostrtod(b);var d=String.fromCharCode(a.saodtype());switch(d){case '"':b/=3600;break;case "'":b/=60;break;case "r":b*=180/Math.PI}return{dval:b,dtype:d}},w=function(b,d){var c,e;e=u(b);
c=u(d);if(":"===e.dtype||":"===c.dtype)n=!0;m||n?(":"===e.dtype&&("galactic"!==k&&"ecliptic"!==k)&&(e.dval*=15),c=a.wcs2pix(this.raw.wcs,e.dval,c.dval).split(/ +/),e=parseFloat(c[0]),c=parseFloat(c[1])):"physical"===k?(c=this.logicalToImagePos({x:e.dval,y:c.dval}),e=c.x,c=c.y):(e=e.dval,c=c.dval);return[e,c]},y=function(a,b){var d=u(a),c=this.raw.wcsinfo||{cdelt1:1,cdelt2:1};if((m||n)&&d.dtype&&"."!==d.dtype)d.dval=Math.abs(d.dval/c["cdelt"+b]);return d.dval},x=function(a){a=u(a);var b=this.raw.wcsinfo||
...
c.cmd&&(c.isregion=0<=c.cmd.search(p));c.comment=g.split("#")[1];c.comment&&(c.comment=c.comment.trim().toLowerCase());if((h=t.exec(g))&&h[1])try{c.opts=JSON.parse(h[1].trim())}catch(A){c.opts={}}if((h=s.exec(g))&&h[1])c.args=h[1].split(v);c.isregion&&0===c.cmd.indexOf("-")&&(c.cmd=c.cmd.slice(1),c.comment=c.comment?c.comment+",exclude":"exclude");h=c;l=h.args.length;if(h.isregion){g=$.extend(!0,{},h.opts);g.shape=h.cmd;2<=l&&(q=w.call(this,h.args[0],h.args[1]),g.x=q[0],g.y=q[1]);switch(h.cmd){case "annulus":g.radii=
[];for(c=2;c<l;c++)g.radii.push(y.call(this,h.args[c],1));break;case "box":3<=l&&(g.width=y.call(this,h.args[2],1));4<=l&&(g.height=y.call(this,h.args[3],2));5<=l&&(g.angle=x.call(this,h.args[4]));break;case "circle":3<=l&&(g.radius=y.call(this,h.args[2],1));break;case "ellipse":3<=l&&(g.r1=y.call(this,h.args[2],1));4<=l&&(g.r2=y.call(this,h.args[3],2));5<=l&&(g.angle=x.call(this,h.args[4]));break;case "line":case "polygon":g.pts=[];for(e=c=0;c<l;c+=2,e++)q=w.call(this,h.args[c],h.args[c+1]),g.pts[e]=
...
"");for(e=0;e<j;e++)f=a.images[e],f.display.id===b&&(f!==c&&d===f.id0)&&(0<=f.id.search(k)&&(f=f.id.replace(k,"$1"),h=Math.max(h,parseInt(f,10))),g++);return g?d+"<"+String(h+1)+">":d};var G=1;a.uniqueID=function(){return G++};a.waiting=function(d,b){var c;switch(d){case !0:window.hasOwnProperty("Spinner")&&"spinner"===a.globalOpts.waitType?(b=b||$("body").get(0),a.spinner||(a.spinner={},c={color:a.globalOpts.spinColor,opacity:a.globalOpts.spinOpacity},a.spinner.spinner=new Spinner(c)),a.spinner.spinner.spin(b)):
$("body").addClass("waiting");break;case !1:window.hasOwnProperty("Spinner")&&"spinner"===a.globalOpts.waitType?a.spinner&&a.spinner.spinner.stop():$("body").removeClass("waiting")}};a.msgHandler=function(d,b){var c,e,f,g=[],h=d.cmd,j=d.id;b&&(a.globalOpts.alerts=!1);if(a.publics[h]){$.isArray(d.args)||(d.args=[d.args]);g=$.extend(!0,[],d.args);j&&g.push({display:j});"RunAnalysis"===h&&b&&(1===g.length&&g.push(null),g.push(b),b=null);try{f=a.publics[h].apply(null,g)}catch(k){f=sprintf("ERROR: %s",
k.message)}b&&(a.globalOpts.alerts=!0,b(f));return f}if(!h||"#"===h)b&&b(""),b&&(a.globalOpts.alerts=!0);else{c=a.lookupCommand(h);e=a.lookupDisplay(j);if(c&&e)switch(c.getDisplayInfo(e),d.args?g=$.extend(!0,[],d.args):d.paramlist&&(g=d.paramlist.split(/ +/)),c.getWhich(g)){case "get":try{f=c.get(g)||""}catch(m){f="ERROR: "+m.message}break;case "set":try{f=c.set(g)||"OK"}catch(n){f="ERROR: "+n.message}break;default:f=sprintf("ERROR: unknown cmd type for '%s'",h)}else c||(f=sprintf("ERROR: unknown cmd '%s'",
h)),e||(f=sprintf("ERROR: unknown display (%s)",j));b&&(a.globalOpts.alerts=!0,b(f));return f}};a.lightWin=function(d,b,c,e,f){var g;switch(a.LIGHTWIN){case "dhtml":g=dhtmlwindow.open(d,b,c,e,f),/iPad|iPhone|iPod/.test(navigator.platform)&&$("#"+d+" "+a.lightOpts.dhtml.drag).css("-webkit-overflow-scrolling","touch").css("overflow-y","scroll"),$("#"+d+" ."+a.lightOpts.dhtml.dragBar).doubletap(function(){g.close()},null,400),$("#"+d+" ."+a.lightOpts.dhtml.dragBar).on("touchend",this,function(){!dhtmlwindow.distancex&&
...
c.image;return b};a.lookupVfile=function(d){var b,c,e,f,g=[];if(!d)return g;for(b=0;b<a.images.length;b++){e=a.images[b];for(c=0;c<e.raws.length;c++)f=e.raws[c],f.hdu&&(f.hdu.fits&&d===f.hdu.fits.vfile)&&g.push({im:e,raw:f,idx:c})}return g};a.onFileList=function(d,b,c){var e,f=function(e){var f;e=d[e];var j=$.extend({},b);if(a.fits.handleFITSFile){e.name&&(j.filename=e.name);j.display&&(f=a.lookupDisplay(j.display))&&(f=f.divjq[0]);a.waiting(!0,f);try{a.fits.handleFITSFile(e,j,c)}catch(k){a.error("can't process FITS file from file list",
...
(a.globalOpts.resizeHandle=!1);a.globalOpts.helperProtocol+="//";if(window.hasOwnProperty("localStorage")){try{d=localStorage.getItem("images")}catch(c){d=null}if(d){try{a.userOpts.images=JSON.parse(d)}catch(e){}a.userOpts.images&&$.extend(!0,a.imageOpts,a.userOpts.images)}try{d=localStorage.getItem("regions")}catch(f){d=null}if(d){try{a.userOpts.regions=JSON.parse(d)}catch(g){}a.userOpts.regions&&$.extend(!0,a.Regions.opts,a.userOpts.regions)}try{d=localStorage.getItem("fits")}catch(h){d=null}if(d)try{a.userOpts.fits=
JSON.parse(d)}catch(j){}try{d=localStorage.getItem("displays")}catch(k){d=null}if(d){try{a.userOpts.displays=JSON.parse(d)}catch(m){}a.userOpts.displays&&$.extend(!0,a.globalOpts,a.userOpts.displays)}}window.addEventListener("message",function(b){var d,c=b.data;b=b.origin||b.originalEvent.origin;"null"===b&&(b="unknown");if(a.globalOpts.postMessage){if("string"===typeof c)try{d=JSON.parse(c)}catch(e){a.error("can't parse msg: "+c,e)}else"object"===typeof c?d=c:a.error("invalid msg from postMessage");
a.msgHandler(d,function(a,b,c,e){e=e||{};parent.postMessage({cmd:d.cmd,res:{name:e.name,rtype:e.rtype,rdata:a,stdout:a,stderr:b,errcode:c}},"*")})}else a.DEBUG&&(b=sprintf("JS9 ignoring postMessage, origin: %s",b),b="string"===typeof c?b+sprintf(" data: %s",c):"object"===typeof c?b+sprintf(" obj: %s",JSON.stringify(Object.keys(c))):b+sprintf(" typeof: %s",typeof c),a.log(b))},!1);a.DEBUG=a.DEBUG||a.globalOpts.debug||0;window.hasOwnProperty("ImageFilters")&&(a.ImageFilters=ImageFilters);window.hasOwnProperty("Astroem")&&

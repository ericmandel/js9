...
var clientSlim = undefined;
var clientSlimMap = undefined;
...
Server.prototype.serveClient = function(v){
  ...
  if (v && !clientSlim) {
    clientSlim = read(resolvePath( 'socket.io-client/dist/socket.io.slim.js'), 'utf-8');
    try {
      clientSlimMap = read(resolvePath( 'socket.io-client/dist/socket.io.slim.js.map'), 'utf-8');
    } catch(err) {
      debug('could not load slim sourcemap file');
    }
  }
  return this;
};
...
Server.prototype.attachServe = function(srv){
  ...
  var url1 = this._path + '/socket.io.slim.js';
  var urlMap1 = this._path + '/socket.io.slim.js.map';
  var evs = srv.listeners('request').slice(0);
  var self = this;
  srv.removeAllListeners('request');
  srv.on('request', function(req, res) {
    if (0 === req.url.indexOf(urlMap)) {
      self.serveMap(req, res, 0);
    } else if (0 === req.url.indexOf(url)) {
      self.serve(req, res, 0);
    } else if (0 === req.url.indexOf(urlMap1)) {
      self.serveMap(req, res, 1);
    } else if (0 === req.url.indexOf(url1)) {
      self.serve(req, res, 1);
    } else {
...
Server.prototype.serve = function(req, res, which){
  ...
  res.writeHead(200);
  res.end(which ? clientSlim : clientSource);
};
...
Server.prototype.serveMap = function(req, res, which){
  // Per the standard, ETags must be quoted:
  // https://tools.ietf.org/html/rfc7232#section-2.3
  var expectedEtag = '"' + clientVersion + '"';
  ...
  res.setHeader('ETag', expectedEtag);
  res.writeHead(200);
  res.end(which ? clientSlimMap : clientSourceMap);
};
